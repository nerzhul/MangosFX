...
void WorldSession::HandleWhoOpcode( WorldPacket & recv_data )
{
    ...
    data << uint32(clientcount);                                    // clientcount place holder
    data << uint32(clientcount);                                    // clientcount place holder
...
void WorldSession::HandleLogoutRequestOpcode( WorldPacket & /*recv_data*/ )
{
	...
	if (GetPlayer()->isInCombat())
		...
		reason = 2;
	else if(GetPlayer()->GetVehicleGUID() ||		                //...is in vehicle
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket & recv_data)
{
    ...
    if( quest_id && pl->isAlive() && GetPlayer()->IsActiveQuest(quest_id) )
    {
    ...
    if(!GetPlayer()->isGameMaster())
    {
		...
		if(OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr.GetOutdoorPvPToZoneId(4197))
		{
			if(Trigger_ID == 5258)
			{
				if(pvpWG->isWarTime())
				{
					SendAreaTriggerMessage("Vous ne pouvez entrez dans le Caveau d'Archavon durant la bataille !");
					return;
				}

				if(!(pvpWG->getDefenderTeam() == BG_TEAM_ALLIANCE && pl->GetTeam() == ALLIANCE ||
					pvpWG->getDefenderTeam() == BG_TEAM_HORDE && pl->GetTeam() == HORDE))
				{
					SendAreaTriggerMessage("Vous ne pouvez entrez dans le Caveau d'Archavon alors que vous n'avez pas le controle du Joug d'Hiver !");
					return;
				}
			}
		}
...
void WorldSession::HandleSetRaidDifficultyOpcode( WorldPacket & recv_data )
{
    ...
    if(map && map->IsDungeon())
    {
        sLog.outDebug("WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", _player->GetGUIDLow());
        return;
    }
