...
Map::PlayerRelocation(Player *player, float x, float y, float z, float orientation)
{
    ...
    if( old_cell.DiffGrid(new_cell) || old_cell.DiffCell(new_cell) )
    {
        ...
        // update player position for group
		...
		case 2531:
            ...
            if (x > 5568.0f && x < 6116.0f && y > 282.0f && y < 982.0f && z > 563.0f)
            {
                ...
                if (x > 5758.77f && x < 5869.03f && y < 555.75f)
                {
                    ...
                    {
                        areaflag = 2633;                    // Note: also 2633, possible one flight allowed and other not allowed case
                        break;
                    }
                }
				if(z < 613.0f && z > 609.0f && x > 5750.0f && x < 5860.0f)
				{
					// zone de duel de dalaran
					areaflag = 2549;
				}
				else
				{
...
void Map::SendInitTransports( Player * player )
{
	...
	transData.m_map = uint16(player->GetMapId());
...
bool InstanceMap::CanEnter(Player *player)
{
    if(player->GetMapRef().getTarget() == this)
    {
		...
		player->SendTransferAborted(GetId(), TRANSFER_ABORT_TOO_MANY_INSTANCES);
        return false;
    }
...
bool InstanceMap::Add(Player *player)
{
    ...
    if (i_data)
	{
		...
		if(player->GetLfgRoles() && dId)
		{
			...
			else if(player->getLevel() < 80 && player->getLevel() > 70)
				dId = 261;
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld.GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                switch(step.script->talk.chatType)
                {
                    ...
                    case CHAT_TYPE_WHISPER:
                        ...
                        {
							sLog.outError("SCRIPT_COMMAND_TALK (script id %u) attempt to whisper (%u) to %u, skipping.", step.script->id, step.script->talk.chatType, unitTarget ? unitTarget->GetGUID() : 0);
                            break;
                        }
						pSource->MonsterWhisper(textId, unitTarget->GetGUID());
                        break;
            ...
            case SCRIPT_COMMAND_EMOTE:
                ...
                ((Creature*)source)->HandleEmoteCommand(step.script->emote.emoteId);
                break;
            ...
            case SCRIPT_COMMAND_MOVE_TO:
                ...
                ((Creature*)source)->SendMonsterMoveWithSpeed(step.script->x, step.script->y, step.script->z, step.script->moveTo.travelTime);
...
bool Map::IsWintergraspBuffValidMap(uint32 mapid)
{
	switch(mapid)
	{
		case 571:
