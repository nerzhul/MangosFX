...
void Object::BuildMovementUpdate(ByteBuffer * data, uint16 updateFlags) const
{
    ...
    *data << uint16(updateFlags);                           // update flags
    ...
    if (updateFlags & UPDATEFLAG_LIVING)
    {
        ...
        switch(GetTypeId())
        {
            ...
            case TYPEID_PLAYER:
            {
				...
				if(player->GetTransport() || player->GetVehicleGUID() || player->GetVehicle())
        ...
        // Write movement info
		unit->m_movementInfo.SetGuid((unit->GetVehicle()) ? unit->GetVehicle()->GetBase()->GetGUID() : unit->m_movementInfo.GetTransportGuid());
    ...
    if(updateFlags & UPDATEFLAG_VEHICLE)                    // unused for now
    {
        ...
        *data << float(((WorldObject*)this)->GetOrientation());                                  // facing adjustment
    }
...
void WorldObject::Relocate(float x, float y, float z, float orientation)
{
    ...
    if(isType(TYPEMASK_UNIT))
	{
		...
		if(((Unit*)this)->GetTypeId() == TYPEID_UNIT)
			if(IsInWorld() && ((Creature*)this)->isVehicle())
				((Unit*)this)->GetVehicleKit()->RelocatePassengers(x,y,z,GetOrientation(),GetMap());
	}
}
...
bool WorldObject::HasInArc(const float arcangle, const WorldObject* obj) const
{
	...
	{
		if(i>1500)
			sLog.outError("has in arc freeze !");
        arc -=  2.0f * M_PI;
	}
...
void WorldObject::GetNearPoint(WorldObject const* searcher, float &x, float &y, float &z, float searcher_size, float distance2d, float absAngle ) const
{
    ...
    ObjectPosSelector selector(GetPositionX(),GetPositionY(),GetObjectBoundingRadius(),distance2d+searcher_size);
	...
	int i = 0;
    while(selector.NextAngle(angle) && i < 750)                        // angle for free pos
    {
		i++;
        GetNearPoint2D(x,y,distance2d,absAngle+angle);
        z = GetPositionZ();
    ...
    // select in positions after current nodes (selection one by one)
	i = 0;
    while(selector.NextUsedAngle(angle) && i < 750)                    // angle for used pos but maybe without LOS problem
    {
		i++;
        GetNearPoint2D(x,y,distance2d,absAngle+angle);
        z = GetPositionZ();
