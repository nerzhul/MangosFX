...
class MANGOS_DLL_SPEC WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        {
            return IsInWorld() && obj->IsInWorld() && GetMap() && (GetMap() == obj->GetMap()) && InSamePhase(obj);
        }
		...
		template<class NOTIFIER> void VisitNearbyWorldObject(const float &radius, NOTIFIER &notifier) const { if(GetMap()) GetMap()->VisitWorld(GetPositionX(), GetPositionY(), radius, notifier); }
