...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
    switch (timer)
    {
        case FATIGUE_TIMER:
            ...
            return sWorld.getConfig(CONFIG_TIMERBAR_FATIGUE_MAX)*IN_MILLISECONDS;
        case BREATH_TIMER:
        {
            if (!isAlive() || HasAuraType(SPELL_AURA_WATER_BREATHING) ||
                GetSession()->GetSecurity() >= sWorld.getConfig(CONFIG_TIMERBAR_BREATH_GMLEVEL))
                return DISABLED_MIRROR_TIMER;
            int32 UnderWaterTime = sWorld.getConfig(CONFIG_TIMERBAR_BREATH_MAX)*IN_MILLISECONDS;
        ...
        case FIRE_TIMER:
        {
            if (!isAlive() || GetSession()->GetSecurity() >= sWorld.getConfig(CONFIG_TIMERBAR_FIRE_GMLEVEL))
                return DISABLED_MIRROR_TIMER;
            return sWorld.getConfig(CONFIG_TIMERBAR_FIRE_MAX)*IN_MILLISECONDS;
        }
        default:
            return 0;
    }
...
bool Player::BuildEnumData( QueryResult * result, WorldPacket * p_data )
{
    ...
    *p_data << uint32(fields[13].GetUInt32());              // guild id
	...
	if(sWorld.IsLocked() || atLoginFlags & AT_LOGIN_BLOCK_PERSO)
...
void Player::GiveXP(uint32 xp, Unit* victim)
{
	...
	{
		// go there
		QueryResult *TakeXp = CharacterDatabase.PQuery("SELECT xp_allowed FROM character_fss_addon WHERE guid = '%u'",GetGUIDLow());
		...
		if (TakeXp)
		{
			do
			{	
				Field *fields = TakeXp->Fetch();
				if(fields[0].GetUInt32() == 1)
					XpAllowed = 2;
				else
					XpAllowed = 1;
			} while (TakeXp->NextRow());
			delete TakeXp;
		}
		else
			XpAllowed = 2;
    ...
    if(victim)
    {
		...
		if(((Creature*)victim)->GetEntry() == (23777 | 26678))
			return;
    }
    else
    {
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_AFK | PLAYER_FLAGS_DND | PLAYER_FLAGS_GM | PLAYER_FLAGS_GHOST | PLAYER_FLAGS_DEVELOPER);
    ...
    if(spellLearnSkill)
    {
        ...
        else
        {
            ...
            while(!prevSkill && prev_spell)
            {
				i++;
				if(i > 2500)
					sLog.outError("Boucle removeSpell !");
...
void Player::RepopAtGraveyard()
{
	...
	else if(GetZoneId() == 4197 && WintergraspOn)
	{
		ClosestGrave = pvpWG->GetClosestGraveyard(this);
		pvpWG->AddPlayerToResurrectQueue(0,this->GetGUID());
		if(!ClosestGrave)
            ...
            else
            {
                ...
                if (diff < -5)
                {
					if(GetSession()->GetSecurity() > 0)
						XP = uint32(sObjectMgr.GetBaseXP(getLevel()+5)*sWorld.getRate(RATE_XP_EXPLORE)*float(1.5));
					else
                ...
                else if (diff > 5)
                {
					...
					if(GetSession()->GetSecurity() > 0)
						XP = uint32(sObjectMgr.GetBaseXP(p->area_level)*exploration_percent/100*sWorld.getRate(RATE_XP_EXPLORE)*float(1.5));
					else
                ...
                {
                    if(GetSession()->GetSecurity() > 0)
						XP = uint32(sObjectMgr.GetBaseXP(p->area_level)*sWorld.getRate(RATE_XP_EXPLORE)*1.5);
					else
...
void Player::RewardReputation(Unit *pVictim, float rate)
{
	...
	if(getLevel() == 80 && this->GetMap() && this->GetMap()->GetDifficulty())
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
	...
	if (IsTwoHandUsedInDualWield() && CanTitanGrip())
    {
		if(HasAura(12163))
			RemoveAurasDueToSpell(12163);
		if(HasAura(12711))
			RemoveAurasDueToSpell(12711);
		if(HasAura(12712))
			RemoveAurasDueToSpell(12712);
		if(!HasAura(49152))
					...
					if(Group* group = GetGroup())
					{
						...
						switch (group->GetLootMethod())
						{
							case GROUP_LOOT:
								...
								group->GroupLoot(GetGUID(), loot, go);
								permission = GROUP_PERMISSION;
								break;
							case NEED_BEFORE_GREED:
								group->NeedBeforeGreed(GetGUID(), loot, go);
								permission = GROUP_PERMISSION;
								break;
							case MASTER_LOOT:
								group->MasterLoot(GetGUID(), loot, go);
...
void Player::SendInitWorldStates(uint32 zoneid, uint32 areaid)
{
    ...
    FillInitialWorldState(data, count, 0xC77, sWorld.getConfig(CONFIG_ARENA_SEASON_IN_PROGRESS));
                                                            // 8 Arena season id
    FillInitialWorldState(data, count, 0xF3D, sWorld.getConfig(CONFIG_ARENA_SEASON_ID));

	// May be send timer to start Wintergrasp
    if(sWorld.GetWintergrapsState() == 4354)
        FillInitialWorldState(data, count, 0x1102, sWorld.GetWintergrapsTimer());
    else
		FillInitialWorldState(data, count, 0xEC5, sWorld.GetWintergrapsTimer());
    // ---
    ...
    switch(zoneid)
    {
		...
		case 4378:                                          // Dalaran Sewers
            ...
            {
                FillInitialWorldState(data,count,3601,0x0);// 7 gold
                FillInitialWorldState(data,count,3600,0x0);// 8 green
                FillInitialWorldState(data,count,3610,0x0);// 9 show
            }
            break;
        case 3703: 
        case 4384:                                          // SA
            ...
            {
                FillInitialWorldState(data,count, 0xe10, 0x0);           // 7 gold
                FillInitialWorldState(data,count, 0xe11, 0x0);           // 8 green
        ...
        if (pSource->GetTypeId() == TYPEID_UNIT)
        {
            ...
            switch(itr->second.option_id)
            {
				...
				case GOSSIP_OPTION_LEARNDUALSPEC:
                    if(!(this->GetSpecsCount() == 1 && pCreature->isCanTrainingAndResetTalentsOf(this) && this->getLevel() >= sWorld.getConfig(CONFIG_MIN_DUALSPEC_LEVEL)))
                        bCanTalk = false;
                    break;
...
void Player::RewardQuest( Quest const *pQuest, uint32 reward, Object* questGiver, bool announce )
{
	...
	if(GetSession()->GetSecurity() > 0)
		 XP = q_status.m_rewarded ? 0 : uint32(pQuest->XPValue( this )*sWorld.getRate(RATE_XP_QUEST)*1.5);
	else
...
void Player::SendQuestReward( Quest const *pQuest, uint32 XP, Object * questGiver )
{
	...
	// FSS mod (send video portal)
	if(questid == 12500 || questid == 12499)
		SendMovieStart(14);
...
void Player::_LoadTitles(const char* data)
{
    if(!data)
		return;
...
bool Player::LoadFromDB( uint32 guid, SqlQueryHolder *holder )
{
	...
	_LoadTitles(fields[63].GetString());
	SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS, DEFAULT_WORLD_OBJECT_SIZE);
	SetFloatValue(UNIT_FIELD_COMBATREACH, 1.5f);
    ...
    if( HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GM) || HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER))
    ...
    {
        m_activeSpec = 0;
        sLog.outDebug("Player %s(GUID: %u) has SpecCount = %u and ActiveSpec = %u.", GetName(), GetGUIDLow(), m_specsCount, m_activeSpec);
    }
	...
	if(QueryResult *result2 = loginDatabase.PQuery("SELECT gmlevel from account where id = '%u' and vip_end < NOW()",m_session->GetAccountId()))
	{
		int gmleveldb = 0;
		gmleveldb = result2->Fetch()->GetInt32();
		if(gmleveldb == 1)
		{
			loginDatabase.PExecute("UPDATE account SET gmlevel = '0' WHERE id = '%u'", m_session->GetAccountId());
		}
		delete result2;
	}
	...
	if(QueryResult* result2 = CharacterDatabase.PQuery("SELECT is_ctp FROM character_fss_addon where guid = '%u'", guid))
	{
		uint8 ctp = result2->Fetch()->GetUInt8();
		SetCTP(ctp ? true : false);
	}
	else
		CharacterDatabase.PExecute("INSERT IGNORE INTO character_fss_addon VALUES ('%u',1,NULL,0,NULL)",guid);
    return true;
}
...
void Player::_LoadInventory(QueryResult *result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if(!proto)
            {
				CharacterDatabase.PExecute("DELETE FROM character_inventory_bak WHERE item = '%u'", item_guid);
        ...
        while(!problematicItems.empty())
        {
            ...
            // fill mail
            MailDraft draft(subject, "Une erreur est survenue sur l'un de vos objets.");
...
void Player::_SaveInventory()
{
    ...
    for (uint8 i = BUYBACK_SLOT_START; i < BUYBACK_SLOT_END; ++i)
    {
        ...
        CharacterDatabase.PExecute("DELETE FROM character_inventory_bak WHERE item = '%u'", item->GetGUIDLow());
	...
	if(mode != PET_SAVE_AS_CURRENT && !InBattleGround())
    {
		...
		if(SpellEntry const *spellInfo = sSpellStore.LookupEntry(spellId))
        {
			...
			if (spellInfo->Attributes & SPELL_ATTR_DISABLED_WHILE_ACTIVE)// && pet->getDeathState() != CORPSE)
			{
				...
				{
					if (spellInfo->Id == 46585 || spellInfo->Id == 52150)
					{
						SendCooldownEvent(sSpellStore.LookupEntry(46584));
					}
				}
			}
        }
...
bool Player::RewardPlayerAndGroupAtKill(Unit* pVictim)
{
    ...
    {
        xp = (PvP || GetVehicle()) ? 0 : MaNGOS::XP::Gain(this, pVictim);
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank)
{
	...
	sLog.outDebug("TalentID: %u Rank: %u Spell: %u Spec: %u\n", talentId, talentRank, spellid, m_activeSpec);
            ...
            *data << uint8(talentIdCount);                  // [PH], talentIdCount
            ...
            for(uint32 i = 0; i < 3; ++i)
            {
                ...
                for(uint32 talentId = 0; talentId < sTalentStore.GetNumRows(); ++talentId)
                {
					...
					TalentTabNb[i] += curtalent_maxrank;
			...
			if (specIdx == m_activeSpec)
			{
				uint8 spec = 0;
				for(uint8 i = 1; i < 3; ++i)
				{
					if (TalentTabNb[i] >= TalentTabNb[spec])
					{
						spec = i;
					}
				}
    ...
    for(uint8 i = 0; i < 3; ++i)
    {
        ...
        for(uint32 talentId = 0; talentId < sTalentStore.GetNumRows(); ++talentId)
        {
            ...
            for(int8 rank = MAX_TALENT_RANK-1; rank >= 0; --rank)
            {
                ...
                {
                    learnSpell(talentInfo->RankID[rank], 0, false);
                    spentTalents += (rank + 1);

					TalentTabNb[i] += (rank + 1);
                }
            }
        }
    }
...
void Player::AddPlayerToWintergraspQueue(uint64 guid)
{
	if (this->GetZoneId() == 4197)
    {
        OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr.GetOutdoorPvPToZoneId(4197);
        if (pvpWG && pvpWG->isWarTime())
		{
			pvpWG->SendAreaSpiritHealerQueryOpcode(this, guid);
            pvpWG->AddPlayerToResurrectQueue(guid, this->GetGUID());
		}
    }
}

void Player::AddWintergraspBuffIfCan(uint32 _mapid)
{
	bool Wintergrasp = false;
	OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr.GetOutdoorPvPToZoneId(4197);
	if(pvpWG && !pvpWG->isWarTime())
	{
		if(pvpWG->getDefenderTeam() == BG_TEAM_ALLIANCE && GetTeam() == ALLIANCE ||
			pvpWG->getDefenderTeam() == BG_TEAM_HORDE && GetTeam() == HORDE)
			Wintergrasp = true;
	}

	if(Map::IsWintergraspBuffValidMap(_mapid) && Wintergrasp)
	{
		SetAuraStack(57940,this,1);
	}
	else
...
bool Player::IsWintergraspDefender()
{
