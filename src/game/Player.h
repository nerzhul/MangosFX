...
typedef UNORDERED_MAP<uint32, PlayerTalent*> PlayerTalentMap; // dual spec
...
struct TradeData
{
	...
	uint32     m_tradeGold;
	uint32     m_tradeSpell;
	uint64	   m_tradeItems[TRADE_SLOT_COUNT];
...
class MANGOS_DLL_SPEC Player : public Unit
{
    ...
    public:
        ...
        bool TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options = 0, uint32 dungeonId = 0);
        ...
        Item* GetItemByTradeSlot(uint32 slot) const { return m_trade && m_trade->m_tradeItems[slot] ? GetItemByGuid(m_trade->m_tradeItems[slot]) : NULL; }
		...
		void AddPlayerToWintergraspQueue(uint64 guid);
