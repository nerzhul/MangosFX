    ...
    if(retval>=0)
    {
        if(plr->HasAura(ZM_BATTLE_STANDARD_A) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
            if(m_GraveYardState == ZM_GRAVEYARD_H)
				sWorld.SendZoneText(ZM_GRAVEYARD_ZONE,sObjectMgr.GetMangosStringForDBCLocale(0/*LANG_OPVP_ZM_LOOSE_GY_H*/));
            ...
            m_PvP->TeamApplyBuff(BG_TEAM_ALLIANCE, ZM_CAPTURE_BUFF);
            plr->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_A);
            sWorld.SendZoneText(ZM_GRAVEYARD_ZONE,sObjectMgr.GetMangosStringForDBCLocale(0/*LANG_OPVP_ZM_CAPTURE_GY_A*/));
        }
        else if(plr->HasAura(ZM_BATTLE_STANDARD_H) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
            if(m_GraveYardState == ZM_GRAVEYARD_A)
                sWorld.SendZoneText(ZM_GRAVEYARD_ZONE,sObjectMgr.GetMangosStringForDBCLocale(0/*LANG_OPVP_ZM_LOOSE_GY_A*/));
            ...
            m_PvP->TeamApplyBuff(BG_TEAM_HORDE, ZM_CAPTURE_BUFF);
            plr->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_H);
            sWorld.SendZoneText(ZM_GRAVEYARD_ZONE,sObjectMgr.GetMangosStringForDBCLocale(0/*LANG_OPVP_ZM_CAPTURE_GY_H*/));
        }
        UpdateTowerState();
    }
    return retval;
...
bool OPvPCapturePointZM_GraveYard::HandleGossipOption(Player *plr, uint64 guid, uint32 gossipid)
{
    ...
    if(itr != m_CreatureTypes.end())
    {
        Creature * cr = sMapMgr.FindMap(530)->GetCreature(guid);
        if(!cr)
            return true;
