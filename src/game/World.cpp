...
void World::LoadConfigSettings(bool reload)
{
	...
	m_configs[CONFIG_QUEST_DAILY_RESET_HOUR] = sConfig.GetBoolDefault("Quests.Daily.ResetHour", 4);
	m_configs[CONFIG_QUEST_WEEKLY_RESET_WEEK_DAY] = sConfig.GetBoolDefault("Quests.Weekly.ResetWeekDay", 2);
	m_configs[CONFIG_QUEST_WEEKLY_RESET_HOUR] = sConfig.GetBoolDefault("Quests.Daily.ResetHour", 4);
	...
	// Dungeon finder
    m_configs[CONFIG_DUNGEON_FINDER] = sConfig.GetIntDefault ("Dungeon_Finder", 1);
...
void World::SetInitialWorldSettings()
{
	...
	sLog.outString("Loading PlayerBot Mounts...");
...
//	sLFGMgr.InitLFG(); // Merging
    ...
    sLog.outString("Calculate next daily & weekly quest reset time..." );
    ...
    sLog.outError( "SERVER STARTUP TIME: %i minutes %i seconds", uStartInterval / 60000, (uStartInterval % 60000) / 1000 );

	if(QueryResult *result = CharacterDatabase.Query("SELECT value FROM needed_variables WHERE needed_variables.key = '5';"))
	{
		...
		if(fields[0].GetUInt32() == 1)
		{
			if(QueryResult *result2 = CharacterDatabase.Query("SELECT value FROM needed_variables WHERE needed_variables.key = '3';"))
			{
				...
				m_savedUptime = fields2[0].GetUInt32();
			}
...
void World::ShutdownMsg(bool show, Player* player)
{
	...
	{
		CharacterDatabase.PExecute("UPDATE needed_variables set value = '1' where needed_variables.key = '5'");
		CharacterDatabase.PExecute("UPDATE needed_variables set value = '%u' where needed_variables.key = '3'",GetUptime());
		CharacterDatabase.PExecute("UPDATE needed_variables set value = '%u' where needed_variables.key = '4'",m_maxActiveSessionCount);
	}
}
...
void World::InitWeeklyQuestResetTime()
{
	...
	int week_day_offset = localTm.tm_wday - int(getConfig(CONFIG_QUEST_WEEKLY_RESET_WEEK_DAY));
	
	// current week reset time
	localTm.tm_hour = getConfig(CONFIG_QUEST_WEEKLY_RESET_HOUR);
	localTm.tm_min  = 0;
	localTm.tm_sec  = 0;
...
{
    m_maxActiveSessionCount = std::max(m_maxActiveSessionCount,uint32(m_sessions.size()-m_QueuedPlayer.size()) + sWorld.GetCountBot());
