...
uint32 Group::RemoveMember(const uint64 &guid, const uint8 &method)
{
    ...
    if(GetMembersCount() > ((isBGGroup() || IsWGGroup()) ? 1 : 2))           // in BG group case allow 1 members group
    {
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold) && !i->freeforall)
        {
            ...
            for(GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    if (member->IsWithinDist(object, sWorld.getConfig(CONFIG_GROUP_XP_DISTANCE), false))
                    {
            ...
            if(object->GetTypeId() == TYPEID_UNIT)
			{
				...
				((Creature*)object)->lootingGroupLeaderGUID = GetLeaderGUID();
			}
			/*else if(object->GetTypeId() == TYPEID_GAMEOBJECT)
			{
				((GameObject*)object)->m_groupLootTimer = 60000;
				((GameObject*)object)->lootingGroupLeaderGUID = GetLeaderGUID();   
			}*/
    ...
    for(std::vector<LootItem>::iterator i=loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold) && !i->freeforall)
        {
            ...
            for(GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    if (playerToRoll->IsWithinDist(object, sWorld.getConfig(CONFIG_GROUP_XP_DISTANCE), false))
                    {
...
void Group::MasterLoot(const uint64& playerGUID, Loot* /*loot*/, WorldObject *object)
{
    ...
    for(GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (looter->IsWithinDist(object, sWorld.getConfig(CONFIG_GROUP_XP_DISTANCE), false))
        {
            data << looter->GetGUID();
            ++real_count;
        }
