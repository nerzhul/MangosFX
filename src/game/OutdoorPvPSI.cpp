/* Outdoor PvP Redesigned by Ner'zhul
 */
...
bool OutdoorPvPSI::HandleAreaTrigger(Player *plr, uint32 trigger)
{
    ...
    case SI_AREATRIGGER_A:
        if(plr->GetTeam() == ALLIANCE && plr->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            {
                TeamApplyBuff(BG_TEAM_ALLIANCE, SI_CENARION_FAVOR);
                sWorld.SendZoneText(OutdoorPvPSIBuffZones[0],sObjectMgr.GetMangosStringForDBCLocale(0/*LANG_OPVP_SI_CAPTURE_A*/));
    ...
    case SI_AREATRIGGER_H:
        if(plr->GetTeam() == HORDE && plr->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            {
                TeamApplyBuff(BG_TEAM_HORDE, SI_CENARION_FAVOR);
                sWorld.SendZoneText(OutdoorPvPSIBuffZones[0],sObjectMgr.GetMangosStringForDBCLocale(0/*LANG_OPVP_SI_CAPTURE_H*/));
