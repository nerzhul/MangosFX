...
ChatCommand * ChatHandler::getCommandTable()
{
    ...
    static ChatCommand debugCommandTable[] =
    {
		...
		{ "dualspec",		SEC_GAMEMASTER,		false, &ChatHandler::HandleDebugDualSpecCommand,            "", NULL },
        { NULL,             0,                  false, NULL,                                                "", NULL }
    };
...
bool ChatHandler::SetDataForCommandInTable(ChatCommand *table, const char* text, uint32 security, std::string const& help, std::string const& fullcommand )
{
    ...
    for(uint32 i = 0; table[i].Name != NULL; i++)
    {
        ...
        if(table[i].SecurityLevel != security)
            sLog.outDebug("Table `command` overwrite for command '%s' default security (%u) by %u",fullcommand.c_str(),table[i].SecurityLevel,security);
