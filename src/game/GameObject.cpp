            ...
            switch(GetGoType())
            {
				...
				/*case GAMEOBJECT_TYPE_CHEST:
					if (m_groupLootTimer && lootingGroupLeaderGUID)
					{
						...
						{
							if (Group* group = sObjectMgr.GetGroupByLeader(lootingGroupLeaderGUID))
								group->EndRoll();
							m_groupLootTimer = 0;
							lootingGroupLeaderGUID = 0;
						}
    ...
    switch(GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if(user->GetTypeId()==TYPEID_PLAYER)
            {
				...
				// titan relic on wintergrasp
				if(GetEntry() == 192829)
				{
					OutdoorPvP * pvp = sOutdoorPvPMgr.GetOutdoorPvPToZoneId(GetZoneId());
					if(pvp && pvp->GetTypeId() == OUTDOOR_PVP_WG)
					{
						if((player->GetTeam() == ALLIANCE && ((OutdoorPvPWG*)pvp)->getAttackerTeam() == BG_TEAM_ALLIANCE
							|| player->GetTeam() == HORDE && ((OutdoorPvPWG*)pvp)->getAttackerTeam() == BG_TEAM_HORDE)
							&& !player->isInCombat())
							((OutdoorPvPWG*)pvp)->ProcessEvent(this,1,player);
					}
					break;
				}
...
void GameObject::TakenDamage(uint32 damage)
{
	...
	uint8 life = (uint8)ceil((double)m_goValue->building.health / (m_goInfo->building.intactNumHits + m_goInfo->building.damagedNumHits) * 255/100);
