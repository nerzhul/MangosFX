...
void ThreatManager::addThreat(Unit* pVictim, float pThreat, bool crit, SpellSchoolMask schoolMask, SpellEntry const *pThreatSpell)
{
    ...
    if (threat > 0.0f && pVictim->GetReducedThreatPercent())
    {
        ...
        if (unit)
			...
			{
				SpellEffectEntry const* spellEffect = pAura->GetSpellProto()->GetSpellEffect(EFFECT_INDEX_0);
				reducedThreadPercent += (spellEffect) ? spellEffect->EffectBasePoints : 0;
			}
...
void ThreatManager::_addThreat(Unit *pVictim, float fThreat)
{
    ...
    if(!ref)                                                // there was no ref => create a new one
    {
        ...
        hostileReference->addThreat(fThreat);                // now we add the real threat
        iUpdateNeed = true;
