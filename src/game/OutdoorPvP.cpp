/*
 * Copyright (C) 2008-2009 Mangos <http://www.trinitycore.org/>
 *
...
: m_PvP(pvp), m_value(0), m_maxValue(0), m_team(BG_TEAM_NEUTRAL),
...
bool OPvPCapturePoint::HandlePlayerEnter(Player * plr)
{
	...
	return m_activePlayers[BattleGround::GetTeamIndexByTeamId(plr->GetTeam())].insert(plr).second;
}
...
bool OPvPCapturePoint::SetCapturePointData(uint32 entry, uint32 map, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3)
{
	...
	m_capturePoint = ObjectAccessor::GetGameObjectInWorld(m_capturePointGUID);
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    MaNGOS::PlayerListSearcher<MaNGOS::AnyPlayerInObjectRangeCheck> searcher(m_capturePoint, players, checker);
	...
	for (std::list<Player*>::iterator itr = players.begin(); itr != players.end(); ++itr)
    {
        ...
        {
            if(m_activePlayers[BattleGround::GetTeamIndexByTeamId((*itr)->GetTeam())].insert(*itr).second)
...
{
    return m_activePlayers[BattleGround::GetTeamIndexByTeamId(plr->GetTeam())].find(plr) != m_activePlayers[BattleGround::GetTeamIndexByTeamId(plr->GetTeam())].end();
}
...
{
    return m_players[BattleGround::GetTeamIndexByTeamId(plr->GetTeam())].find(plr) != m_players[BattleGround::GetTeamIndexByTeamId(plr->GetTeam())].end();
}

void OutdoorPvP::TeamCastSpell(BattleGroundTeamId team, int32 spellId)
{
    if(spellId > 0)
