...
bool AssistDelayEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    if(Unit* victim = Unit::GetUnit(m_owner, m_victim))
    {
        ...
        while (!m_assistants.empty())
        {
			i++;
			if(i>2500)
				sLog.outError("AssistDelayEvent Boucle");
...
, m_reactState(REACT_AGGRESSIVE), m_splineFlags(SPLINEFLAG_WALKMODE)
{
    m_regenTimer = 200;
...
void Creature::AddToWorld()
{
    ...
    if(!IsInWorld() && (GetGUIDHigh()==HIGHGUID_UNIT || GetGUIDHigh()==HIGHGUID_VEHICLE))
...
bool Creature::InitEntry(uint32 Entry, uint32 team, const CreatureData *data )
{
	...
	// normally the same as native, see above for the exception
    ...
    {                                                       // use default from the template
		if (cinfo->equipmentId == 0)
			LoadEquipment(normalInfo->equipmentId);
		else
...
bool Creature::Create(uint32 guidlow, Map *map, uint32 phaseMask, uint32 Entry, uint32 team, const CreatureData *data, uint32 vehId)
{
    if(!map)
		return false;
    ...
    const bool bResult = CreateFromProto(guidlow, Entry, team, data, vehId);
