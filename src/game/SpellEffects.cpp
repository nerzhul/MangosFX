...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS]=
{
    ...
    &Spell::EffectSummonWild,                         // 76 SPELL_EFFECT_SUMMON_OBJECT_WILD
...
void Spell::EffectInstaKill(SpellEffectEntry const* /*effect*/)
{
	...
	data << uint64(m_caster->GetTypeId() != TYPEID_GAMEOBJECT ? m_caster->GetGUID() : 0);
...
void Spell::EffectSchoolDMG(SpellEffectEntry const* effect)
{
    if(unitTarget && unitTarget->isAlive())
    {
        switch(m_spellInfo->GetSpellFamilyName())
        {
            ...
            {
				sClassSpellHandler.HandleSchoolDmg(this,damage,SpellEffectIndex(effect->EffectIndex));
                break;
            }
        }
...
void Spell::EffectDummy(SpellEffectEntry const* effect)
{
    ...
    switch(m_spellInfo->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 29858:                                 // Soulshatter
					if (unitTarget && unitTarget->GetTypeId() == TYPEID_UNIT && unitTarget->IsHostileTo(m_caster) && unitTarget->isInCombat())
                ...
                */
				...
				{
					if(m_caster->GetTypeId() == TYPEID_PLAYER)
						((Player*)m_caster)->KilledMonsterCredit(29124,0);
					return;
				}
				case 43375: //fss
				{	
					if(m_caster->GetTypeId() == TYPEID_PLAYER)
					{
						DoCreateItem(0,33617);
					}

					return;
				}
				case 42793: // FSS mod (ne marche pas)
				{
					// Fallen Combatant Credit
					m_caster->CastSpell(unitTarget, 43297, false, NULL);
					return;
				}
				case 47170: // FSS mod
				{
				...
				{
					// Quest 11889
					if(unitTarget->GetEntry() == 25582)
					{
						// todo : ajouter l'aura du filet
						m_caster->AddThreat(unitTarget,50);
					}
					return;
				...
				{
					//Fly Spiritual form
					m_caster->CastSpell(m_caster, 40308, false);
				...
				case 56033: //FSS mod
				{
					...
					{
						if(m_caster->GetTypeId() == TYPEID_PLAYER)
							((Player*)m_caster)->KilledMonsterCredit(30146,unitTarget->GetGUID());
					}
					else
						unitTarget->AddThreat(m_caster,250.0f);
				...
				{
					if(m_caster->GetTypeId() == TYPEID_PLAYER)
						((Player*)m_caster)->KilledMonsterCredit(27921,m_caster->GetGUID());
					return;
				}
                ...
                case 45980: // Re-Cursive Transmatter Injection
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER && unitTarget)
                    {
                        if (const SpellEntry *pSpell = sSpellStore.LookupEntry(46022))
                        {
							...
							((Player*)m_caster)->KilledMonsterCredit(killSpellEffect ? killSpellEffect->EffectMiscValue : 0,m_caster->GetGUID());
                        }
				...
				case 72040:
					...
					if(!unitTarget || !spellEffect)
						return;
					
					m_caster->CastSpell(unitTarget,spellEffect->EffectBasePoints+1,true);
					return;
            }
            break;
        }
        case SPELLFAMILY_WARRIOR:
            // Charge
			if ((m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x1)) && m_spellInfo->SpellVisual[0] == 867)
            {
            ...
            // Execute
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x20000000))
            {
                if(!unitTarget)
                    return;
            ...
            // Slam
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000200000))
            {
                if(!unitTarget)
                    return;
            ...
            // Concussion Blow
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000004000000))
            {
        ...
        case SPELLFAMILY_WARLOCK:
            // Life Tap
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000040000))
            {
...
//              damage = m_caster->SpellDamageBonus(m_caster, m_spellInfo,uint32(damage > 0 ? damage : 0), SPELL_DIRECT_DAMAGE);
                if (unitTarget && (int32(unitTarget->GetHealth()) > damage))
                {
					...
					if(m_caster->HasAura(63320))
					{
						m_caster->CastSpell(m_caster,63321,false);
					}
                }
                else
				...
				case 27217:
					...
					if(m_caster->GetTypeId() == TYPEID_PLAYER)
						if(m_caster->HasAura(58070))
							if(roll_chance_i(5))
								m_caster->CastSpell(m_caster,58068,false);
					break;
			}
            break;
        case SPELLFAMILY_PRIEST:
            // Penance
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0080000000000000))
            {
                if (!unitTarget)
                    return;
        ...
        case SPELLFAMILY_DRUID:
            // Starfall
            if (m_spellInfo->GetSpellFamilyFlags2() & UI64LIT(0x00000100))
            {
		...
		case SPELLFAMILY_HUNTER:
            // Steady Shot
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x100000000))
            {
                if (!unitTarget || !unitTarget->isAlive())
                    return;
            ...
            // Disengage
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000400000000000))
            {
        ...
        case SPELLFAMILY_PALADIN:
			// Divine Storm
			if (m_spellInfo->GetSpellFamilyFlags2() & 0x00020000 && effect->EffectIndex == 1)
            {
        ...
        case SPELLFAMILY_SHAMAN:
            // Cleansing Totem
            if ((m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000004000000)) && m_spellInfo->SpellIconID==1673)
            {
                if (unitTarget)
            ...
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000002000))
            {
                if (unitTarget)
                {
            ...
            // Mana Spring Totem
            if (m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000004000))
            {
            ...
            // Lava Lash
            if (m_spellInfo->GetSpellFamilyFlags2() & 0x00000004)
            {
                if (m_caster->GetTypeId()!=TYPEID_PLAYER)
                    return;
                ...
                if (item)
                {
                    ...
                    for(Unit::AuraList::const_iterator itr = auraDummy.begin(); itr != auraDummy.end(); ++itr)
                    {
                        if ((*itr)->GetSpellProto()->GetSpellFamilyName()==SPELLFAMILY_SHAMAN &&
							((*itr)->GetSpellProto()->GetSpellFamilyFlags() & UI64LIT(0x0000000000200000)) &&
        ...
        case SPELLFAMILY_DEATHKNIGHT:
			if(!sClassSpellHandler.HandleEffectDummy(this,m_damage,SpellEffectIndex(effect->EffectIndex)))
				return;
            break;
    }
    ...
    if (gameObjTarget)
		Script->EffectDummyGameObj(m_caster, m_spellInfo->Id, effect->EffectIndex, gameObjTarget);
    else if (unitTarget && unitTarget->GetTypeId()==TYPEID_UNIT)
		Script->EffectDummyCreature(m_caster, m_spellInfo->Id, effect->EffectIndex, (Creature*)unitTarget);
    else if (itemTarget)
		Script->EffectDummyItem(m_caster, m_spellInfo->Id, effect->EffectIndex, itemTarget);
}
...
void Spell::EffectTriggerSpell(SpellEffectEntry const* effect)
{
    ...
    switch(triggered_spell_id)
    {
        ...
        case 29284:
        {
			...
			for (int j=0; j < spell->GetStackAmount(); ++j)
...
void Spell::EffectTriggerMissileSpell(SpellEffectEntry const* effect)
{
    ...
    if(!spellInfo)
    {
		if(m_spellInfo->Id == 66313)
		{
			m_caster->SummonCreature(34854,m_targets.m_destX, m_targets.m_destY, m_targets.m_destZ,0,TEMPSUMMON_TIMED_DESPAWN,60000);
			return;
		}
    ...
    //remove cooldown from triggered spell for Freezing Arrow
...
void Spell::EffectApplyAura(SpellEffectEntry const* effect)
{
	...
	Aura* Aur = CreateAura(m_spellInfo, effect->EffectIndex, &m_currentBasePoints[effect->EffectIndex], unitTarget, caster, m_CastItem);
...
void Spell::EffectSendEvent(SpellEffectEntry const* effect)
{
    ...
    */
    sLog.outDebug("Spell ScriptStart %u for spellid %u in EffectSendEvent ", effect->EffectMiscValue, m_spellInfo->Id);
...
void Spell::EffectHeal(SpellEffectEntry const* effect)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
		...
		else if (m_spellInfo->GetTargetAuraState() == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND))
        {
            ...
            for(Unit::AuraList::const_iterator i = RejorRegr.begin(); i != RejorRegr.end(); ++i)
            {
                if ((*i)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DRUID &&
					...
					((*i)->GetSpellProto()->GetSpellFamilyFlags() & UI64LIT(0x0000000000000050)))
                {
			...
			int32 tickcount = (targetSpellEffect ? targetSpellEffect->EffectAmplitude : 1) - 1;
        ...
        // Chain Healing
		if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000000100))
        {
            // check for Riptide
...
void Spell::DoCreateItem(SpellEffectEntry const* effect, uint32 itemtype)
{
    ...
    if( msg != EQUIP_ERR_OK )
    {
        ...
        {
			SpellEffectEntry const* spellEffect = m_spellInfo->GetSpellEffect(EFFECT_INDEX_1);
			// ignore mana gem case (next effect will recharge existed example)
			if (effect->EffectIndex == 0 && spellEffect && spellEffect->Effect == SPELL_EFFECT_DUMMY )
                return;
...
void Spell::EffectPersistentAA(SpellEffectEntry const* effect)
{
	...
	if (!dynObj->Create(m_caster->GetMap()->GenerateLocalLowGuid(HIGHGUID_DYNAMICOBJECT), m_caster, m_spellInfo->Id, effect->EffectIndex, m_targets.m_destX, m_targets.m_destY, m_targets.m_destZ, duration, radius))
    {
        delete dynObj;
        return;
    }
...
void Spell::EffectEnergize(SpellEffectEntry const* effect)
{
    ...
    switch (m_spellInfo->Id)
    {
		...
		case 68082:											// fss fix for glyphe de sceau d'autorite
...
void Spell::EffectProficiency(SpellEffectEntry const* /*effect*/)
{
	...
	if (m_spellInfo->GetEquippedItemClass() == ITEM_CLASS_WEAPON && !(p_target->GetWeaponProficiency() & eqItems->EquippedItemSubClassMask))
    {
		p_target->AddWeaponProficiency(eqItems->EquippedItemSubClassMask);
        p_target->SendProficiency(ITEM_CLASS_WEAPON, p_target->GetWeaponProficiency());
    }
	if (m_spellInfo->GetEquippedItemClass() == ITEM_CLASS_ARMOR && !(p_target->GetArmorProficiency() & eqItems->EquippedItemSubClassMask))
    {
...
void Spell::EffectApplyAreaAura(SpellEffectEntry const* effect)
{
	...
	AreaAura* Aur = new AreaAura(m_spellInfo, effect->EffectIndex, &m_currentBasePoints[effect->EffectIndex], unitTarget, m_caster, m_CastItem);
    unitTarget->AddAura(Aur);
}
...
void Spell::EffectSummon(SpellEffectEntry const* effect)
{
	...
	for(int32 count = 0; count < amount; ++count)
	{
		...
		if (m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION)
		{
			...
			summoner->GetClosePoint(x, y, z, creature->GetObjectBoundingRadius());
...
void Spell::EffectDispel(SpellEffectEntry const* effect)
{
    ...
    {
        Aura *aur = (*itr).second;
		if (aur && (1<<aur->GetSpellProto()->GetDispel()) & dispelMask)
        {
			if(aur->GetSpellProto()->GetDispel() == DISPEL_MAGIC)
            {
                bool positive = true;
                if (!aur->IsPositive())
                    positive = false;
			...
			else if(aur->GetSpellProto()->GetDispel() == DISPEL_DISEASE)
			{
				// Unholy Blight hack
...
void Spell::EffectAddFarsight(SpellEffectEntry const* effect)
{
	...
	if(!dynObj->Create(m_caster->GetMap()->GenerateLocalLowGuid(HIGHGUID_DYNAMICOBJECT), m_caster, m_spellInfo->Id, effect->EffectIndex, m_targets.m_destX, m_targets.m_destY, m_targets.m_destZ, duration, 0))
    {
        delete dynObj;
        return;
    }
...
void Spell::EffectSummonWild(SpellEffectEntry const* effect, uint32 forceFaction)
{
    ...
    for(int32 count = 0; count < amount; ++count)
    {
        ...
        if(Creature *summon = m_caster->SummonCreature(creature_entry, px, py, pz, m_caster->GetOrientation(), summonType, duration))
        {
            ...
            if(creature_entry == 31216)
            {
				...
				summon->SetMaxHealth(28 + 30*m_caster->getLevel());
...
void Spell::EffectSummonGuardian(SpellEffectEntry const* effect, uint32 forceFaction)
{
	...
	switch(effect->EffectMiscValueB)
	{
		case 2081: // Engineering Dragonlings
		case 2141: // Winterfin Horn of Distress
		{
			if (level > m_spellInfo->GetMaxLevel())
				level = m_spellInfo->GetMaxLevel();
			amount = 1;
			break;
		}
	}
...
void Spell::EffectLearnSkill(SpellEffectEntry const* effect)
{
	...
	((Player*)unitTarget)->SetSkill(skillid, m_spellInfo->CalculateSimpleValue(SpellEffectIndex(effect->EffectIndex)), skillval ? skillval : 1, damage * 75);
}
...
void Spell::EffectEnchantItemTmp(SpellEffectEntry const* effect)
{
    ...
    if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000400000))
    {
        uint32 spell_id = 0;
    ...
    if(!pEnchant)
    {
		sLog.outError("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have not existed enchanting id %u ",m_spellInfo->Id,effect->EffectIndex,enchant_id);
        return;
    }
...
void Spell::EffectSummonAllTotems(SpellEffectEntry const* effect)
{
    ...
    switch(m_spellInfo->Id)
    {
        ...
        case 66844:         // Call of the Spirits
        {
            ...
            {
                uint8 button = effect->EffectMiscValue+slot+132;
...
void Spell::EffectTaunt(SpellEffectEntry const* /*effect*/)
{
    ...
    if (unitTarget->CanHaveThreatList() && unitTarget->getThreatManager().getCurrentVictim())
		if(!((Creature*)unitTarget)->HasReactState(REACT_PASSIVE))
...
void Spell::EffectWeaponDmg(SpellEffectEntry const* effect)
{
    ...
    for (uint8 j = 0; j < 3; ++j)
    {
        ...
        switch(spellEffect->Effect)
        {
            ...
            case SPELL_EFFECT_WEAPON_PERCENT_DAMAGE:
                if (j < effect->EffectIndex)                                  // we must calculate only at last weapon effect
                    return;
            break;
        }
    }
		...
		case SPELLFAMILY_GENERIC:
		{
			switch(m_spellInfo->Id)                         // for spells with divided damage to targets
			{
				...
				case 71021:                                 // Saber Lash
				{
					...
					// For all this spells damages could be divided by 2 only
					if(count > 2)
						count = 2;
    ...
    if(m_spellInfo->GetSpellFamilyName()==SPELLFAMILY_PALADIN)
    {
        ...
        if(m_spellInfo->GetSpellFamilyFlags() & 0x0000000800000000LL && m_spellInfo->SpellIconID==153)
        {
...
void Spell::EffectInterruptCast(SpellEffectEntry const* effect)
{
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
    {
        if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
            ...
            {
				uint32 zero = 0;
				unitTarget->ProhibitSpellSchool(GetSpellSchoolMask(curSpellInfo), unitTarget->CalculateSpellDuration(m_spellInfo, effect->EffectIndex, unitTarget,zero));
...
void Spell::EffectSummonWild(SpellEffectEntry const* effect)
{
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
				...
				case 44455:                                 // Character Script Effect Reverse Cast
                {
                    ...
                    if (const SpellEntry *pSpell = sSpellStore.LookupEntry(m_spellInfo->CalculateSimpleValue(EFFECT_INDEX_0)))
                    {
                        // if we used item at least once...
						SpellEffectEntry const* spellEffect = pSpell->GetSpellEffect(EFFECT_INDEX_0);
                        if (pTarget->isTemporarySummon() && pTarget->GetEntry() == effect->EffectMiscValue)
                        {
                ...
                case 50238:                                 // The Cleansing: Your Inner Turmoil's On Death Cast on Master
                {
                    ...
                    if (((Creature*)m_caster)->isTemporarySummon())
                    {
						...
						if (pSummon->GetSummoner()->GetTypeId() == TYPEID_PLAYER)
                        {
				...
				case 51864:                                 // Player Summon Nass
                {
                    ...
                    if (const SpellEntry* pSpell = sSpellStore.LookupEntry(51865))
                    {
						...
						SpellEffectEntry const* spellEffect = pSpell->GetSpellEffect(EFFECT_INDEX_0);
                        if (!m_caster->FindGuardianWithEntry(spellEffect->EffectMiscValue))
                        {
                            m_caster->CastSpell(m_caster, pSpell, true);
							SpellEffectEntry const* spellEffect = pSpell->GetSpellEffect(EFFECT_INDEX_0);
                            if (Pet* pPet = m_caster->FindGuardianWithEntry(spellEffect->EffectMiscValue))
                            {
               ...
               case 58591:                                 // Stoneclaw Totem X
               {
                ...
                case 58420:                                 // Portal to Stormwind
                {
					if(!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER || effect->EffectIndex!=0)
                        return;
				...
				{
					if(m_caster)
						m_caster->RemoveAurasDueToSpell(effect->EffectBasePoints + 1);
					return;
				}
                ...
                case 47422:
                {
                    ...
                    for(Unit::AuraMap::iterator itr = suAuras.begin(); itr != suAuras.end(); ++itr)
                    {
                        ...
                        if(spellInfo->GetSpellFamilyName() == SPELLFAMILY_WARLOCK &&
							(spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000000002)) &&
                ...
                case 47948:
                {
                    ...
                    for(Unit::AuraMap::iterator itr = auras.begin(); itr != auras.end(); ++itr)
                    {
                        ...
                        if (spellInfo->GetSpellFamilyName() == SPELLFAMILY_PRIEST &&
							(spellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000008000)) &&
                            (*itr).second->GetCasterGUID() == m_caster->GetGUID())
                        {
                            (*itr).second->RefreshAura();
                            return;
            ...
            switch(m_spellInfo->Id)
            {
                ...
                case 53209:
                {
                    ...
                    for(Unit::AuraMap::iterator i = Auras.begin(); i != Auras.end(); ++i)
                    {
                        ...
                        uint64 familyFlag = aura->GetSpellProto()->GetSpellFamilyFlags();
            ...
            switch(m_spellInfo->Id)
            {
                ...
                case 50842:
                {
					...
					if (m_caster->HasAura(63334))
					{
						...
						for(Unit::AuraList::const_iterator itr = auras.begin(); itr!=auras.end(); ++itr)
							if ((*itr)->GetSpellProto()->GetDispel() == DISPEL_DISEASE)
...
void Spell::EffectSanctuary(SpellEffectEntry const* /*effect*/)
{
    ...
    if(m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_ROGUE && (m_spellInfo->GetSpellFamilyFlags() & SPELLFAMILYFLAG_ROGUE_VANISH))
    {
...
void Spell::EffectSummonTotem(SpellEffectEntry const* effect, uint8 slot)
{
	...
	// stone claw shield
	if(m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && m_spellInfo->GetSpellFamilyFlags() & 0x00008)
	{
		int32 bp0 = 0;
		switch(m_spellInfo->Id)
		{
...
void Spell::EffectReputation(SpellEffectEntry const* effect)
{
	...
	int32  rep_change = m_currentBasePoints[effect->EffectIndex];           // field store reputation change -1
...
void Spell::EffectSummonCritter(SpellEffectEntry const* effect, uint32 forceFaction)
{
    if(m_caster->GetTypeId() != TYPEID_PLAYER)
        return;
    Player* player = (Player*)m_caster;
...
void Spell::EffectKnockBack(SpellEffectEntry const* effect)
{
	...
	// Thunderstorm Glyph
	if(m_spellInfo->GetSpellFamilyName() & SPELLFAMILY_SHAMAN && unitTarget->HasAura(62132) && m_spellInfo->GetSpellFamilyFlags() & UI64LIT(0x200000000000))
		return;

    unitTarget->KnockBackFrom(m_caster,float(effect->EffectMiscValue)/10,float(damage)/10);
}

void Spell::EffectQuestClear(SpellEffectEntry const* effect)
{
    Player *pPlayer = NULL;
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
        pPlayer = (Player*)m_caster;
...
void Spell::EffectPlayerPull(SpellEffectEntry const* effect)
{
    ...
    unitTarget->KnockBackFrom(m_caster,-dist,float(effect->EffectMiscValue)/30);
}
...
void Spell::EffectDispelMechanic(SpellEffectEntry const* effect)
{
    ...
    for(Unit::AuraMap::iterator iter = Auras.begin(), next; iter != Auras.end(); iter = next)
    {
		...
		SpellEffectEntry const* spellEffect = spell->GetSpellEffect(SpellEffectIndex(iter->second->GetEffIndex()));
        if(spell->GetMechanic() == mechanic || spellEffect->EffectMechanic == mechanic)
        {
...
void Spell::EffectTitanGrip(SpellEffectEntry const* effect)
{
    ...
    {
		if(((Player*)unitTarget)->IsTwoHandUsed())
        {
...
void Spell::EffectRestoreItemCharges(SpellEffectEntry const* effect)
{
    ...
    if (Item* item = player->GetItemByEntry(effect->EffectItemType))
        item->RestoreCharges();
}
