...
//              damage = m_caster->SpellDamageBonus(m_caster, m_spellInfo,uint32(damage > 0 ? damage : 0), SPELL_DIRECT_DAMAGE);
        ...
        case SPELLFAMILY_PALADIN:
			// Divine Storm
            if (m_spellInfo->SpellFamilyFlags2 & 0x00020000 && i == 1)
            {
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
				...
				case 44455:                                 // Character Script Effect Reverse Cast
                {
                    ...
                    if (const SpellEntry *pSpell = sSpellStore.LookupEntry(m_spellInfo->CalculateSimpleValue(0)))
                    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            switch(m_spellInfo->Id)
            {
				...
				case 46584:
                {
					...
					bool allow_cast = unitTarget && unitTarget != m_caster;
					
					if(!allow_cast)
					{
						if (p_caster->HasAura(60200))
							allow_cast = true;
                    ...
                    uint32 spell = uint32(p_caster->HasSpell(52143) ? 52150 : 46585);
