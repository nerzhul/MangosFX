    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if(spellInfo)
        {
            ...
            if(loc < MAX_LOCALE)
            {
                ...
                uint32 rank = talentCost ? talentCost : sSpellMgr.GetSpellRank(learn ? spellEffect && spellEffect->EffectTriggerSpell : id);
...
bool ChatHandler::HandleDieCommand(const char* /*args*/)
{
    ...
    if(target->GetTypeId()==TYPEID_PLAYER)
    {
        /*if(HasLowerSecurity((Player*)target,0,false))
            return false;*/
		if(m_session->GetSecurity() <= ((Player*)target)->GetSession()->GetSecurity() && ((Player*)target) != m_session->GetPlayer())
			return false;
    }
...
bool ChatHandler::HandleNpcInfoCommand(const char* /*args*/)
{
	...
	PSendSysMessage("Phase : %u",target->GetPhaseMask());
