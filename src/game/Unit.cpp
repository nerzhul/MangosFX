...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
	...
	if(GetTypeId() == TYPEID_PLAYER &&
		spellProto && (spellProto->GetDmgClass() == SPELL_DAMAGE_CLASS_MELEE || spellProto->GetDmgClass() == SPELL_DAMAGE_CLASS_RANGED) &&
		damagetype == SPELL_DIRECT_DAMAGE && !(spellProto->GetAuraInterruptFlags() & AURA_INTERRUPT_FLAG_DAMAGE))
    ...
    if (!(damageInfo->HitInfo & HITINFO_MISS))
    {
        ...
        for(AuraList::const_iterator i = vDamageShields.begin(); i != vDamageShields.end();)
        {
           if (alreadyDone.find(*i) == alreadyDone.end())
           {
			   ...
			   // Thorns
			   if (i_spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && i_spellProto->GetSpellFamilyFlags() & UI64LIT(0x00000100))
			   {
...
void Unit::CalcAbsorbResist(Unit *pVictim,SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, bool canReflect)
{
    ...
    for(AuraList::const_iterator i = vAbsorb.begin(); i != vAbsorb.end() && RemainingDamage > 0; ++i)
    {
        ...
        if(i_spellProto->GetSpellFamilyName() == SPELLFAMILY_MAGE && i_spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000000000108))
        {
            int chance = 0;
    ...
    for(AuraList::const_iterator i = vSchoolAbsorb.begin(); i != vSchoolAbsorb.end() && RemainingDamage > 0; ++i)
    {
        ...
        switch(spellProto->GetSpellFamilyName())
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                {
					SpellEffectEntry const* spellEffect = spellProto->GetSpellEffect(EFFECT_INDEX_1);
                    // Max absorb stored in 1 dummy effect
                    if (spellEffect && spellEffect->EffectBasePoints < currentAbsorb)
                        currentAbsorb = spellEffect->EffectBasePoints;
                    break;
                }
                break;
            }
            ...
            case SPELLFAMILY_PRIEST:
            {
                ...
                // Power Word: Shield
                if (spellProto->GetSpellFamilyFlags() & UI64LIT(00000001) && spellProto->GetMechanic() == MECHANIC_SHIELD)
                {
                ...
                // Reflective Shield
                if (spellProto->GetSpellFamilyFlags() == 0x1 && canReflect)
                {
                    if (pVictim == this)
                        break;
    ...
    for(AuraList::const_iterator i = vManaShield.begin(), next; i != vManaShield.end() && RemainingDamage > 0; i = next)
    {
		...
		if(SpellEffectEntry const* spellEffectM = (*i)->GetSpellProto()->GetSpellEffect(SpellEffectIndex((*i)->GetEffIndex())))
			if (float manaMultiplier = spellEffectM->EffectMultipleValue)
			{
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
	...
	if(spell->GetCategory() == SPELLCATEGORY_JUDGEMENT || 
    ...
    if (spell->GetDmgClass() == SPELL_DAMAGE_CLASS_RANGED && !isSpecialPlayerSpell)
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
	...
	// explosive shot
	if(spell->GetSpellFamilyFlags() == SPELLFAMILY_HUNTER && spell->SpellIconID == 3407)
	{
    ...
    if (i != m_Auras.end())
    {
        ...
        if (!Aur->IsPassive() && !Aur->IsPersistent())
        {
            for(AuraMap::iterator i2 = m_Auras.lower_bound(spair); i2 != m_Auras.upper_bound(spair); ++i2)
            {
                ...
                bool stop = false;
				SpellEffectEntry const* effectA = aurSpellInfo->GetSpellEffect(SpellEffectIndex(Aur->GetEffIndex()));
                // m_auraname can be modified to SPELL_AURA_NONE for area auras, use original
				AuraType aurNameReal = AuraType(effectA ? effectA->EffectApplyAuraName : 0);
...
bool Unit::RemoveNoStackAurasDueToAura(Aura *Aur)
{
    ...
    for (i = m_Auras.begin(); i != m_Auras.end(); i = next)
    {
        ...
        for(int j = 0; j < 3; ++j)
			if(SpellEffectEntry const* effectJ = i_spellProto->GetSpellEffect(SpellEffectIndex(j)))
				if (effectJ->EffectTriggerSpell == spellId)
					is_triggered_by_spell = true;

        // prevent triggered aura of removing aura that triggering it (triggered effect early some aura of parent spell
        for(int j = 0; j < 3; ++j)
			if(SpellEffectEntry const* effectJ = spellProto->GetSpellEffect(SpellEffectIndex(j)))
				if (effectJ->EffectTriggerSpell == i_spellId)
					is_triggered_by_spell = true;
...
void Unit::RemoveSingleAuraDueToSpellByDispel(uint32 spellId, uint64 casterGUID, Unit *dispeler)
{
    ...
    else if (spellEntry->GetSpellFamilyName() == SPELLFAMILY_DRUID && (spellEntry->GetSpellFamilyFlags() & UI64LIT(0x0000001000000000)))
    {                    
        if (Aura* dotAura = GetAura(SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, UI64LIT(0x0000001000000000), 0x00000000, casterGUID))
        {
            ...
            {
                int32 returnmana = (spellEntry->GetManaCostPercentage() * caster->GetCreateMana() / 100)/ 2 ;// * stackAmount / 2;
...
bool Unit::HandleDummyAuraProc(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
	...
	switch(dummySpell->GetSpellFamilyName())
    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 28764:
                {
                    ...
                    for(AuraList::const_iterator iter = mRegenInterupt.begin(); iter != mRegenInterupt.end(); ++iter)
                    {
                        ...
                        {
							if(iterSpellProto->GetSpellFamilyName()==SPELLFAMILY_MAGE && (iterSpellProto->GetSpellFamilyFlags() & UI64LIT(0x10000000)))
                            {
                                found=true;
                                break;
                            }
                ...
                case 31872:
                {
                    ...
                    for(AuraMap::const_iterator iter = Auras.begin(); iter != Auras.end();)
                    {
						...
						SpellEffectEntry const* effectIter = spell->GetSpellEffect(SpellEffectIndex(iter->second->GetEffIndex()));
                        if( spell->GetMechanic() == MECHANIC_STUN || effectIter &&
                            effectIter->EffectMechanic == MECHANIC_STUN)
                        {
                ...
                case 71903:
                {
                    ...
                    if (aur && aur->GetStackAmount() + 1 >= aur->GetSpellProto()->GetStackAmount())
                    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000008000))
            {
                if(GetTypeId() != TYPEID_PLAYER)
                    return false;
                    ...
                    if (Unit* caster = triggeredByAura->GetCaster())
                    {
                        ...
                        for (Unit::AuraList::const_iterator i = auras.begin(); i != auras.end(); i++)
                        {
                            switch((*i)->GetId())
                            {
                                ...
                                case 31657:
                                case 31658:
                                    if(roll_chance_i((*i)->GetSpellProto()->GetProcChance()))
                                    {
        ...
        {
            // Seed of Corruption
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x0000001000000000))
            {
            ...
            if (dummySpell->GetSpellFamilyFlags() == UI64LIT(0x0) && dummySpell->SpellIconID == 1932)
            {
                Modifier* mod = triggeredByAura->GetModifier();
        ...
        {
            // Vampiric Touch
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x0000040000000000))
            {
                if(!pVictim || !pVictim->isAlive())
                    return false;
            ...
            switch(dummySpell->Id)
            {
                ...
                {
                    // Shadow Word: Pain
                    if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000008000))
                        triggered_spell_id = 40441;
                    // Renew
                    else if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000010))
        ...
        case SPELLFAMILY_DRUID:
        {
            switch(dummySpell->Id)
            {
                ...
                {
                    // mana back
                    basepoints0 = int32(procSpell->GetManaCost() * 30 / 100);
                    target = this;
                ...
                case 40442:
                {
                    ...
                    // Starfire
                    if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000004))
                    {
                    ...
                    else if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000044000000000))
                    {
	            ...
	            case 54821:
	            {
					SpellEffectEntry const* effect1 = procSpell->GetSpellEffect(EFFECT_INDEX_1);
	                if (procSpell->SpellVisual[0] == 750 && effect1 && effect1->EffectApplyAuraName == 3)
	                {
		...
		// Item - Druid T10 Balance 4P Bonus
            ...
            if (dummySpell->SpellIconID == 2850)
            {
                ...
                if (procSpell->GetSpellFamilyFlags2() & UI64LIT(0x00000800)  && triggeredByAura->GetMiscValue() != 126)
                {
                    basepoints0 = triggerAmount;
                    triggered_spell_id = 51178;
                    target = this;
            ...
            else if (dummySpell->SpellIconID == 2856)
            {
                ...
                if (effIndex!=0)
                    return true;
                // Wrath crit
                if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000001))
                {
                    if (HasAura(48517))
                        return false;
                    if (!roll_chance_i(60))
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            if (dummySpell->SpellIconID == 513)
            {
                ...
                if (triggeredByAura->GetStackAmount() < dummySpell->GetStackAmount())
                    return false;
            ...
            if (dummySpell->SpellIconID == 2909)
            {
                ...
                for(AuraList::const_iterator itr = sd.begin(); itr != sd.end(); ++itr)
                {
                    ...
                    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_ROGUE &&
                        (spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000000040000)))
                    {
            ...
            if (dummySpell->SpellIconID == 2116)
            {
                ...
                // energy cost save
                basepoints0 = procSpell->GetManaCost() * triggerAmount/100;
                if (basepoints0 <= 0)
                    return false;
        ...
        {
            // Aspect of the Viper
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x4000000000000))
            {
            ...
            if ( dummySpell->SpellIconID == 3560 )
            {
                ...
                if (!(procSpell->GetSpellFamilyFlags() & UI64LIT(0x0100000000000000)))
                    return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if ((dummySpell->GetSpellFamilyFlags() & UI64LIT(0x000000008000000)) && effIndex==0)
            {
            ...
            // Sacred Shield
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x0008000000000000))
            {
				if (procFlag & PROC_FLAG_TAKEN_POSITIVE_SPELL)
				{
					if (procSpell->GetSpellFamilyName() == SPELLFAMILY_PALADIN
						&& (procSpell->GetSpellFamilyFlags() & 0x40000000))
					{
            ...
            switch(dummySpell->Id)
            {
                ...
                case 40470:
                {
                    ...
                    if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x00000000C0000000))
                    {
                ...
                case 54939:
                {
                    ...
                    for (int i=0; i<3;++i)
						if(SpellEffectEntry const* effectI = procSpell->GetSpellEffect(SpellEffectIndex(i)))
							if (effectI->Effect == SPELL_EFFECT_ENERGIZE)
							{
								int32 mana = effectI->EffectBasePoints;
                ...
                case 58597:
                {
					...
					SpellEffectEntry const* effect0 = procSpell->GetSpellEffect(EFFECT_INDEX_0);
					if(!effect0)
						return false;

                    basepoints0 = int32(damage / (GetSpellDuration(triggeredEntry) / effect0->EffectAmplitude));
                    target = this;
                    break;
                }
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 33757:
                {
					...
					SpellEffectEntry const* effect1 = windfurySpellEntry->GetSpellEffect(EFFECT_INDEX_1);
					int32 extra_attack_power = effect1 ? CalculateSpellDamage(windfurySpellEntry, 1, effect1->EffectBasePoints, pVictim) : 0;
                ...
                case 40463:
                {
                    ...
                    else if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000080))
                    {
                        triggered_spell_id = 40465;         // Lesser Healing Wave
                        chance = 10.0f;
                    }
                    else if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000001000000000))
                    {
			...
			if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000200000))
			{
            ...
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x200000))
            {
                if(GetTypeId()!=TYPEID_PLAYER)
                    return false;
				...
				SpellEffectEntry const* effect0 = dummySpell->GetSpellEffect(EFFECT_INDEX_0);
                //  firehit =  dummySpell->EffectBasePoints[0] / ((4*19.25) * 1.3);
				float fire_onhit = effect0 ? effect0->EffectBasePoints / 100.0 : 0.0f;
            ...
            if (dummySpell->SpellIconID == 2287)
            {
                ...
                if ((procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000080)) && !roll_chance_i(60))
                    return false;

				// Chain Heal needs additional 30% roll
				if ((procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000100)) && !roll_chance_i(30))
					return false;
                // lookup water shield
                ...
                for(AuraList::const_iterator itr = vs.begin(); itr != vs.end(); ++itr)
                {
                    ...
                    {
						SpellEffectEntry const* effectItr = (*itr)->GetSpellProto()->GetSpellEffect(SpellEffectIndex((*itr)->GetEffIndex()));
						uint32 spell = effectItr ? effectItr->EffectTriggerSpell : 0;
            ...
            if (dummySpell->SpellIconID == 2018)            // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
                ...
                if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000002))
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            // Vendetta
            if (dummySpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000010000))
            {
            ...
            if (dummySpell->SpellIconID == 138)
            {
				...
				if (!(procFlag & PROC_FLAG_SUCCESSFUL_MELEE_HIT) && procSpell->Id != 56815)
					return false;
				SpellEffectEntry const* effectIdx = dummySpell->GetSpellEffect(SpellEffectIndex(effIndex));
                triggered_spell_id = effectIdx->EffectTriggerSpell;
                break;
            }
			// Rune strike
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
	...
	SpellEffectEntry const* effectA = auraSpellInfo->GetSpellEffect(SpellEffectIndex(triggeredByAura->GetEffIndex()));
	uint32 trigger_spell_id = effectA ? effectA->EffectTriggerSpell : 0;
    Unit*  target = NULL;
	...
	switch (auraSpellInfo->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
            switch(auraSpellInfo->Id)
            {
                ...
                case 43820:                                 // Charm of the Witch Doctor (Amani Charm of the Witch Doctor trinket)
				{
					...
					SpellEffectEntry const* effect1 = auraSpellInfo->GetSpellEffect(EFFECT_INDEX_1);
					basepoints[0] = effect1 ? pVictim->GetCreateHealth() * effect1->EffectBasePoints / 100 : 0;
                    target = pVictim;
					break;
				}
        ...
        {
            // Drain Soul
            if (auraSpellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000004000))
            {
        ...
        case SPELLFAMILY_HUNTER:
            ...
            else if (auraSpellInfo->Id == 53228 || auraSpellInfo->Id == 53232)
            {
                ...
                if (!(procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000020)))
                    return false;
            }
            break;
        case SPELLFAMILY_PALADIN:
        {
            ...
            */
            ...
            else if (auraSpellInfo->SpellIconID==241)
            {
                ...
                // Holy Shock heal
                if (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0001000000000000))
                {
                    switch(procSpell->Id)
                    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (auraSpellInfo->GetSpellFamilyFlags() & UI64LIT(0x0000000000000400))
            {
                switch(auraSpellInfo->Id)
                {
                    case 324:                           // Rank 1
    ...
    switch(trigger_spell_id)
    {
        ...
        case 57761:
        {
            ...
            if (procSpell->GetSpellFamilyName()==SPELLFAMILY_MAGE && (procSpell->GetSpellFamilyFlags() & UI64LIT(0x0000000000000080)))
            {
                bool found = false;
        ...
        case 56453:
        {
            ...
            if (!(procFlags & PROC_FLAG_ON_TRAP_ACTIVATION) ||
                !(procSpell->GetSpellFamilyFlags() & 0x00000008 || procSpell->GetSpellFamilyFlags2() & 0x40000) || !roll_chance_i(triggerAmount))
                return false;
            break;
        }
...
uint32 Unit::SpellDamageBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (!(spellProto->AttributesEx6 & SPELL_ATTR_EX6_NO_DMG_PERCENT_MODS))
    {
        ...
        {
			SpellEquippedItemsEntry const* eqItems = (*i)->GetSpellProto()->GetSpellEquippedItems();
            if( ((*i)->GetModifier()->m_miscvalue & GetSpellSchoolMask(spellProto)) &&
                (*i)->GetSpellProto()->GetEquippedItemClass() == -1 &&
                                                                // -1 == any item class (not wand then)
                eqItems && eqItems->EquippedItemInventoryTypeMask == 0 )
    ...
    for(AuraList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch((*i)->GetModifier()->m_miscvalue)
        {
            ...
            case 4993:
            {
                ...
                // effect 0 m_amount
				SpellEffectEntry const* effectItr = (*i)->GetSpellProto()->GetSpellEffect(EFFECT_INDEX_0);
				int32 stepPercent = CalculateSpellDamage((*i)->GetSpellProto(), 0, effectItr ? effectItr->EffectBasePoints : 0, this);
			...
			{
				// ebony plaguebringer
				if(spellProto->GetDispel() == DISPEL_DISEASE)
    ...
    for(AuraList::const_iterator i = mDummy.begin(); i != mDummy.end(); ++i)
    {
        SpellEntry const *spell = (*i)->GetSpellProto();
        //Fire and Brimstone
        if (spell->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && spell->SpellIconID == 3173)
        {
            if (pVictim->HasAuraState(AURA_STATE_CONFLAGRATE) && (spellProto->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && spellProto->GetSpellFamilyFlags() & UI64LIT(0x0002004000000000)))
            {
                DoneTotalMod *= ((*i)->GetModifier()->m_amount+100.0f) / 100.0f;
                break;
            }
    ...
    else if (DoneAdvertisedBenefit || TakenAdvertisedBenefit)
    {
        ...
        {
			SpellEffectEntry const* effectJ = spellProto->GetSpellEffect(SpellEffectIndex(j));
            if(effectJ && (effectJ->Effect == SPELL_EFFECT_HEALTH_LEECH ||
                (effectJ->Effect == SPELL_EFFECT_APPLY_AURA &&
                effectJ->EffectApplyAuraName == SPELL_AURA_PERIODIC_LEECH)))
            {
                CastingTime /= 2;
                break;
            }
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask)
{
    ...
    for(AuraList::const_iterator i = mDamageDone.begin();i != mDamageDone.end(); ++i)
    {
        ...
        if (((*i)->GetModifier()->m_miscvalue & schoolMask) != 0 &&
            ...
            eqItems && eqItems->EquippedItemInventoryTypeMask == 0)          //  0 == any inventory type (not wand then)
...
bool Unit::isSpellCrit(Unit *pVictim, SpellEntry const *spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType)
{
	...
	case SPELL_DAMAGE_CLASS_NONE:
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (pVictim)
            {
                ...
                switch(spellProto->GetSpellFamilyName())
                {
                    case SPELLFAMILY_PRIEST:
                        // Flash Heal
                        if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000000000800))
                        {
					...
					case SPELLFAMILY_DRUID:
						...
						if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000000000004))
						{
                    ...
                    case SPELLFAMILY_PALADIN:
                        // Sacred Shield
                        if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000040000000))
                        {
                    ...
                    case SPELLFAMILY_SHAMAN:
                        // Lava Burst
                        if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000100000000000))
                        {
                            // Flame Shock
					...
					case SPELLFAMILY_DEATHKNIGHT:
					{
						if(Aura* aur = sClassSpellHandler.GetAuraByName(this,DK_RIME))
							if(spellProto->GetSpellFamilyFlags() & UI64LIT(0x2000000000002))
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if(spellProto->GetSpellFamilyName() == SPELLFAMILY_PALADIN)
            {
                if(spellProto->GetSpellFamilyFlags() & 0x0000000000800000LL && spellProto->SpellIconID == 561)
                {
            ...
            else if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID)
            {
                if(spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000000800000))
                {
...
uint32 Unit::SpellCriticalDamageBonus(SpellEntry const *spellProto, uint32 damage, Unit *pVictim)
{
	...
	// Mage Spell Power
	if(spellProto->GetSpellFamilyName() == SPELLFAMILY_MAGE)
	{
		if(HasAura(35578))
			critPctDamageMod += 25;
		...
		if(HasAura(56363) && spellProto->GetSpellFamilyFlags() & 0x200000)
			critPctDamageMod += 25;
	}
...
uint32 Unit::SpellHealingBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto->GetDmgClass() == SPELL_DAMAGE_CLASS_NONE && spellProto->Id != 64844) // divine hymn hack
    {
            ...
            case 8477: // Nourish Heal Boost
            {
                ...
                for (AuraMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                {
                    ...
                    if (m_spell->GetSpellFamilyName() != SPELLFAMILY_DRUID ||
                        !(m_spell->GetSpellFamilyFlags() & UI64LIT(0x0000001000000050)))
                        continue;
                    modPercent += stepPercent * itr->second->GetStackAmount();
					if(HasAura(62971))
						modPercent += 6;
	...
	// Nourish cast bonus
    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && spellProto->GetSpellFamilyFlags() & 0x200000000000000)
    {
	...
	// Twin disciplines
	else if(spellProto->GetSpellFamilyName() == SPELLFAMILY_PRIEST && (spellProto->GetSpellFamilyFlags() & UI64LIT(200204008000)))
	{
    ...
    // Taken mods
    // Healing Wave cast
    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && (spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000000000040)))
    {
...
int32 Unit::SpellBaseHealingBonus(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for(AuraList::const_iterator i = mHealingDoneOfStatPercent.begin();i != mHealingDoneOfStatPercent.end(); ++i)
        {
			...
			Stats usedStat = Stats(effectItr ? effectItr->EffectMiscValue : 0);
...
bool Unit::IsImmunedToSpellEffect(SpellEntry const* spellInfo, uint32 index) const
{
    ...
    if(uint32 aura = spellEffect->EffectApplyAuraName)
    {
		...
		if(HasAura(47585) && (
			spellEffect->EffectMechanic & IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK ||
        ...
        for(AuraList::const_iterator i = immuneMechanicAuraApply.begin(); i != immuneMechanicAuraApply.end(); ++i)
		{
            ...
            if ((spellEffect->EffectMechanic & (*i)->GetMiscValue() ||
...
uint32 Unit::MeleeDamageBonus(Unit *pVictim, uint32 pdamage,WeaponAttackType attType, SpellEntry const *spellProto, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto && spellProto->GetSpellFamilyName()==SPELLFAMILY_DRUID && spellProto->GetSpellFamilyFlags() & UI64LIT(0x00008000))
	...
	if (GetTypeId() == TYPEID_PLAYER)
		if(spellProto && spellProto->GetSpellFamilyName() == SPELLFAMILY_HUNTER || !spellProto)
		{
			// Improved Tracking
	...
	// Frost Strike
	if (spellProto && spellProto->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT && spellProto->GetSpellFamilyFlags() & UI64LIT(0x0000000400000000))
	{   
		// search disease
		bool found = false;
...
int32 Unit::CalculateSpellDamage(SpellEntry const* spellProto, uint8 effect_index, int32 effBasePoints, Unit const* target)
{
    ...
    if (level > (int32)spellProto->GetMaxLevel() && spellProto->GetMaxLevel() > 0)
        level = (int32)spellProto->GetMaxLevel();
	else if (level < (int32)spellProto->GetBaseLevel())
		level = (int32)spellProto->GetBaseLevel();
    level-= (int32)spellProto->GetSpellLevel();

	SpellEffectEntry const* spellEffect = spellProto->GetSpellEffect(SpellEffectIndex(effect_index));

	float basePointsPerLevel = spellEffect ? spellEffect->EffectRealPointsPerLevel : 0;
	int32 basePoints = effBasePoints ? effBasePoints - 1 : spellEffect ? spellEffect->EffectBasePoints: 0;
    basePoints += int32(level * basePointsPerLevel);
	int32 randomPoints = int32(spellEffect ? spellEffect->EffectDieSides : 0);
	float comboDamage = spellEffect ? spellEffect->EffectPointsPerComboPoint: 0;
    ...
    if(spellProto->Attributes & SPELL_ATTR_LEVEL_DAMAGE_CALCULATION && spellProto->GetSpellLevel() &&
        ...
        value = int32(value*0.25f*exp(getLevel()*(70-spellProto->GetSpellLevel())/1000.0f));

	// Frostbite trigger aura: if Fingers of Frost is active, it has saved a roll:
	if(spellEffect->EffectTriggerSpell == 12494)
	{
...
int32 Unit::CalculateSpellDuration(SpellEntry const* spellProto, uint8 effect_index, Unit const* target, uint32 &periodicTime)
{
    ...
    {	
		...
		if (periodicTime)
        {
			...
			if(HasAura(15473) && spellProto->SpellIconID == 2213 && spellProto->GetSpellFamilyName() == SPELLFAMILY_PRIEST)
				applyHaste = true;

			if(HasAura(44401) && spellProto->GetSpellFamilyName() == SPELLFAMILY_MAGE && spellProto->GetSpellFamilyFlags() & 0x800)
			{
				periodicTime = 500;
				duration = 2500;
			}
        ...
        if (!IsPositiveSpell(spellProto->Id))
             durationMod_always += target->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_DURATION_OF_MAGIC_EFFECTS, spellProto->GetDispel());
		...
		if (unitPlayer && target == this)
        {
            ...
            {
                case SPELLFAMILY_DRUID:
                    if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x100))
                    {
                        // Glyph of Thorns
                        if (Aura *aur = GetAura(57862, 0))
                            duration += aur->GetModifier()->m_amount * MINUTE * IN_MILLISECONDS;
                    }
					else if(spellProto->GetSpellFamilyFlags() & UI64LIT(0x800000))
					{
						// Glyph of rip
                    ...
                    break;
                case SPELLFAMILY_PALADIN:
                    if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x00000002))
                    {
                        // Glyph of Blessing of Might
                        if (Aura * aur = GetAura(57958, 0))
                            duration += aur->GetModifier()->m_amount * MINUTE * IN_MILLISECONDS;
                    }
                    else if (spellProto->GetSpellFamilyFlags() & UI64LIT(0x00010000))
                    {
...
uint32 Unit::GetCreatureType() const
{
    if(GetTypeId() == TYPEID_PLAYER)
    {
		SpellShapeshiftFormEntry const* ssFormEntry = sSpellShapeshiftFormStore.LookupEntry(m_form);
        if(ssFormEntry && ssFormEntry->creatureType > 0)
            return ssFormEntry->creatureType;
        else
    ...
    for(uint32 x = 0; x < CREATURE_MAX_SPELLS; ++x)
    {
        ...
        else
        {
            ...
            for(uint32 i = 0;i<3 && onlyselfcast;++i)       //non existent spell will not make any problems as onlyselfcast would be false -> break right away
            {
                ...
                if(effectI->EffectImplicitTargetA != TARGET_SELF && effectI->EffectImplicitTargetA != 0)
                    onlyselfcast = false;
            }
...
uint32 Unit::GetCastingTimeForBonus( SpellEntry const *spellProto, DamageEffectType damagetype, uint32 CastingTime )
{
    ...
    {
		SpellEffectEntry const* effectI = spellProto->GetSpellEffect(SpellEffectIndex(i));
        switch ( effectI->Effect )
        {
        ...
        if(IsAreaEffectTarget(Targets(effectI->EffectImplicitTargetA)) || IsAreaEffectTarget(Targets(effectI->EffectImplicitTargetB)))
            AreaEffect = true;
    }
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit *pVictim, Aura* aura, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const*& spellProcEvent )
{
    ...
    if(procSpell && procSpell->Id == spellProto->Id && !(spellProto->GetProcFlags() & PROC_FLAG_ON_TAKE_PERIODIC) && aura->GetModifier()->m_auraname != SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE)
        return false;
    ...
    if(!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        if(spellProto->GetEquippedItemClass() == ITEM_CLASS_WEAPON)
        {
            ...
            if(!item || item->IsBroken() || !IsUseEquipedWeapon(attType) || item->GetProto()->Class != ITEM_CLASS_WEAPON || eqItems && !((1<<item->GetProto()->SubClass) & eqItems->EquippedItemSubClassMask))
                return false;
        }
        else if(spellProto->GetEquippedItemClass() == ITEM_CLASS_ARMOR)
        {
            ...
            if(!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_ARMOR || eqItems && !((1<<item->GetProto()->SubClass) & eqItems->EquippedItemSubClassMask))
                return false;
        }
    }
	...
	if(SpellEffectEntry const* effectA = aura->GetSpellProto()->GetSpellEffect(SpellEffectIndex(aura->GetEffIndex())))
	if(effectA && effectA->EffectTriggerSpell == 44544)
	{
...
bool Unit::HandleMendingAuraProc( Aura* triggeredByAura )
{
    ...
    {
		SpellEffectEntry const* effectIdx = spellProto->GetSpellEffect(SpellEffectIndex(effIdx));
        float radius;
        if (effectIdx->EffectRadiusIndex)
            radius = GetSpellRadius(sSpellRadiusStore.LookupEntry(effectIdx->EffectRadiusIndex));
        else
        ...
        if(Player* caster = ((Player*)triggeredByAura->GetCaster()))
        {
            ...
            if(Player* target = ((Player*)this)->GetNextRandomRaidMember(radius))
            {
                ...
                SpellModifier *mod = new SpellModifier(SPELLMOD_CHARGES,SPELLMOD_FLAT,jumps-5,spellProto->Id,spellProto->GetSpellFamilyFlags(),spellProto->GetSpellFamilyFlags2());
...
bool Unit::isIgnoreUnitState(SpellEntry const *spell)
{
	...
	if(spell->GetSpellFamilyName() == SPELLFAMILY_MAGE)
	{
		// Ice Lance
...
void Unit::AddAura(uint32 spellId, Unit *target)
{
    ...
    {
		SpellEffectEntry const* effectI = spellInfo->GetSpellEffect(SpellEffectIndex(i));
        if (effectI && (effectI->Effect == SPELL_EFFECT_APPLY_AURA || IsAreaAuraEffect(effectI->Effect)))
        {
            // Area auras applied as linked should have target as source (otherwise they'll be removed after first aura update)
            if (effectI->Effect != SPELL_EFFECT_APPLY_AURA)
                source = target;
