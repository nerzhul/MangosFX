...
void WorldSession::HandleCreatureQueryOpcode( WorldPacket & recv_data )
{
    ...
    if (ci)
    {
        ...
        data << uint32(ci->ModelId[0]);                 // modelid_male1
        data << uint32(ci->ModelId[1]);                 // modelid_female1 ?
        data << uint32(ci->ModelId[2]);                 // modelid_male2 ?
        data << uint32(ci->ModelId[3]);                 // modelid_femmale2 ?
        ...
        if(questOk)
        {
            ...
            if(POI)
            {
                ...
                for(QuestPOIVector::const_iterator itr = POI->begin(); itr != POI->end(); ++itr)
                {
					...
					data << uint32(itr->mapAreaId);              // unknown
					data << uint32(itr->floodId);              // unknown
