...
void WorldSession::HandleGroupInviteOpcode( WorldPacket & recv_data )
{
    ...
    // can't group with
	if(!sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && GetPlayer()->GetTeam() != player->GetTeam() && !GetPlayer()->isGameMaster())
    {
...
void WorldSession::HandleLootRoll( WorldPacket &recv_data )
{
	...
	if(!group->CountRollVote(GetPlayer()->GetGUID(), Guid, NumberOfPlayers, RollVote(rollType)))
		return;
...
void WorldSession::HandleRequestPartyMemberStatsOpcode( WorldPacket &recv_data )
{
    ...
    WorldPacket data(SMSG_PARTY_MEMBER_STATS_FULL, 200);
    data << uint8(0);                                       // only for SMSG_PARTY_MEMBER_STATS_FULL, probably arena/bg related
    data.append(player->GetPackGUID());
	...
	if(player->GetVehicleGUID() || player->GetVehicle())	// for vehicles
        mask1 = 0xFFFFFFFF;                                 
    ...
    if(pet)
    {
		...
		if(player->GetVehicle() || player->GetVehicleGUID())
			data << uint32(player->m_movementInfo.GetTransportDBCSeat());
    }
    else
    {
