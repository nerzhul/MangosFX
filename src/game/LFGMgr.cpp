...
{
    if (!m_update || sWorld.getConfig(CONFIG_DUNGEON_FINDER) == 0)
        return;
...
void LFGMgr::Join(Player* plr)
{
    ...
    {
        if (grp->GetMembersCount() > MAX_GROUP_SIZE) //Merging
...
{
    if ((plr && (!plr->GetLfgUpdate() || !plr->isUsingLfg())) || sWorld.getConfig(CONFIG_DUNGEON_FINDER) == 0)
        return;
...
LfgProposal* LFGMgr::FindNewGroups(LfgGuidList check, LfgGuidList all)
{
    ...
    LfgProposal* pProposal = NULL;
    if (!check.size() || check.size() > MAX_GROUP_SIZE || !CheckCompatibility(check, pProposal)) //Merging
        return NULL;
...
bool LFGMgr::CheckCompatibility(LfgGuidList check, LfgProposal*& pProposal)
{
    ...
    if (check.size() > MAX_GROUP_SIZE || !check.size()) // Merging
    {
    ...
    LfgQueueInfoMap::iterator itQueue;
    for (LfgGuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups < 2 && numPlayers <= MAX_GROUP_SIZE; ++it) //Merging
    {
    ...
    if (check.size() == 1)
    {
        if (numPlayers != MAX_GROUP_SIZE)                      // Single group with less than MAX_GROUP_SIZE - Compatibles //Merging
            return true;
    }

    // Do not match - groups already in a lfgDungeon or too much players
    if (numLfgGroups > 1 || numPlayers > MAX_GROUP_SIZE) //Merging
    {
        pqInfoMap.clear();
        SetCompatibles(strGuids, false);
        if (numLfgGroups > 1)
...
//                else if (plr->GetTeam() != (*itPlayer)->GetTeam() && !sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP)) // Merging
    ...
    if (numPlayers != MAX_GROUP_SIZE) //Merging
    {
        players.clear();
        rolesMap.clear();
    ...
    if (numAccept == MAX_GROUP_SIZE) //Merging
...
void LFGMgr::UpdateProposal(uint32 proposalId, uint32 lowGuid, bool accept)
{
    ...
    else
    {
        ...
        for (LfgPlayerList::const_iterator it = players.begin(); it != players.end(); ++it)
		{
			(*it)->SetAuraStack(LFG_SPELL_DUNGEON_COOLDOWN,(*it),1);
...
void LFGMgr::RemoveProposal(LfgProposalMap::iterator itProposal, LfgUpdateType type)
{
    ...
    for (LfgProposalPlayerMap::const_iterator it = pProposal->players.begin(); it != pProposal->players.end(); ++it)
    {
        ...
        else                                                 // Readd to queue
        {
            ...
            {
                sLog.outError("LFGMgr::RemoveProposal: Impossible to readd [" UI64FMTD "] to queue. Missing queue info!", guid);
    ...
    else
    {
        ...
        else if (plr->GetMapId() != uint32(dungeon->map))    // Do not teleport players in dungeon to the entrance
        {
            ...
            if (!mapid)
            {
                ...
                if (!at)
                {
                    sLog.outError("LfgMgr::TeleportPlayer: Failed to teleport [" UI64FMTD "]: No areatrigger found for map: %u difficulty: %u", plr->GetGUID(), dungeon->map, dungeon->heroic); //Merging
                    error = LFG_TELEPORTERROR_INVALID_LOCATION;
                }
                else
                {
...
LfgLockStatusSet* LFGMgr::GetPlayerLockStatusDungeons(Player* plr, LfgDungeonSet* dungeons /* = NULL */, bool useEntry /* = true */)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons->begin(); it != dungeons->end(); ++it)
    {
        ...
        ar = sObjectMgr.GetAccessRequirement(dungeon->map, Difficulty(dungeon->heroic));
 ...
 /*       else if (sDisableMgr.IsDisabledFor(DISABLE_TYPE_MAP, dungeon->map, plr))
            locktype = LFG_LOCKSTATUS_RAID_LOCKED;*/ // For test purpose
        else if (dungeon->heroic > DUNGEON_DIFFICULTY_NORMAL && plr->GetBoundInstance(dungeon->map, Difficulty(dungeon->heroic)))
        ...
        {
            /*if (ar->achievement && !plr->GetAchievementMgr().HasAchievement(sAchievementStore.LookupEntry(ar->achievement))) // Merging
			...
			if (plr->GetTeam() == ALLIANCE && ar->quest_A && !plr->GetQuestRewardStatus(ar->quest_A))
