...
bool BattleGroundSA::ResetObjs()
{
  ...
  for(uint8 i = 0;i < BG_SA_MAX_GY; i++)
      ...
      if(i == BG_SA_BEACH_GY)
	{
	  ...
	  AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z, BG_SA_GYOrientation[i], ((attackers == BG_TEAM_HORDE )? HORDE : ALLIANCE));
	}
      else
	{
	  ...
	  if(!AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z, BG_SA_GYOrientation[i], ((attackers == BG_TEAM_HORDE )? ALLIANCE : HORDE)  ))
	    {
...
void BattleGroundSA::OverrideGunFaction()
{
  if(!m_BgCreatures[0])
	...
	{
	  gun->setFaction(BG_SA_Factions[attackers? BG_TEAM_ALLIANCE : BG_TEAM_HORDE]);
	}
...
void BattleGroundSA::CaptureGraveyard(BG_SA_Graveyards i)
{
  ...
  WorldSafeLocsEntry const *sg = NULL;
  sg = sWorldSafeLocsStore.LookupEntry(BG_SA_GYEntries[i]);
  AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z, BG_SA_GYOrientation[i], (GraveyardStatus[i] == BG_TEAM_ALLIANCE?  ALLIANCE : HORDE ));
  uint32 npc = 0;
