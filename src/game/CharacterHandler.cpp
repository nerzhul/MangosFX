...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADFROM,            "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags,"
        ...
        "health, power1, power2, power3, power4, power5, power6, power7, power8, power9, specCount, activeSpec, exploredZones, equipmentCache, ammoId, titleCache, actionBars FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADGLYPHS,          "SELECT spec, glyph1, glyph2, glyph3, glyph4, glyph5, glyph6, glyph7, glyph8, glyph9 FROM character_glyphs WHERE guid='%u'", GUID_LOPART(m_guid));
	...
	res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADCALENDAREVENTS,  "SELECT eventid FROM character_calendar_events WHERE guid = '%u'", GUID_LOPART(m_guid));
	res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADTALENTBRANCHSPECS, "SELECT branchSpec, spec FROM character_branchspec WHERE guid = '%u'",GUID_LOPART(m_guid));
