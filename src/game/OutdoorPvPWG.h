...
{
    explicit BuildingState(uint32 _worldState, BattleGroundTeamId _team, bool asDefault)
    ...
    uint32 worldState;
    uint32 health;
    BattleGroundTeamId defaultTeam;
    ...
    BattleGroundTeamId GetTeam() const { return team; }
    void SetTeam(BattleGroundTeamId t)
    {
        team = t;
        if(graveTeam)
...
class OutdoorPvPWG : public OutdoorPvP
{
    ...
    public:
        ...
        void ModifyWorkshopCount(BattleGroundTeamId team, bool add);
    ...
    protected:
		...
		std::vector<uint64> BuildingDestroyables;
		std::vector<uint64> NESpawnsCr;
		std::vector<uint64> SESpawnsCr;
		std::vector<uint64> NWSpawnsCr;
		std::vector<uint64> SWSpawnsCr;
		std::vector<uint64> NESpawnsGo;
		std::vector<uint64> SESpawnsGo;
		std::vector<uint64> NWSpawnsGo;
		std::vector<uint64> SWSpawnsGo;
		std::vector<uint64> ToDespawnInWarCr;
		std::vector<uint64> FortressGuards;
		std::vector<uint64> FortressEngineer;
		uint64 DoorColisionGUID;
		...
		void ChangeFortressSpawns(BattleGroundTeamId owner);
};
