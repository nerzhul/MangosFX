...
#define MAX_VENDOR_ITEMS 255                                // Limitation in 3.x.x item count in SMSG_LIST_INVENTORY
...
class MANGOS_DLL_SPEC Creature : public Unit
{
    ...
    public:
        ...
        bool Create(uint32 guidlow, Map *map, uint32 phaseMask, uint32 Entry, uint32 team, const CreatureData *data = NULL, uint32 vehId = 0);
        ...
        {
			if (pos >= CREATURE_MAX_SPELLS || (m_charmInfo && m_charmInfo->GetCharmSpell(pos) && m_charmInfo->GetCharmSpell(pos)->GetType() != ACT_ENABLED))
                return 0;
            else
