...
enum SpellAuraInterruptFlags
{
    ...
    AURA_INTERRUPT_FLAG_CAST                        = 0x00000800,   // 11
    ...
    public:
		...
		bool IsUseEquipedWeapon( WeaponAttackType attackType ) const
		{
			...
			switch(attackType)
			{
				...
				case OFF_ATTACK:    haveweapon = !HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISARM_OFFHAND); break;
        ...
        void CastSpell(Unit* Victim, uint32 spellId, bool triggered = false, Item *castItem = NULL, Aura* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        void RemoveAura(uint32 spellId, uint32 effindex, Aura* except = NULL,AuraRemoveMode removeMode = AURA_REMOVE_BY_DEFAULT );

        // removing specific aura stacks by diff reasons and selections
        void RemoveAurasDueToSpell(uint32 spellId, Aura* except = NULL,AuraRemoveMode removeMode = AURA_REMOVE_BY_DEFAULT );
		...
		void RemoveSpellsCausingAuraWithInterruptFlags(AuraType auraType, uint32 flags);
        ...
        int32 CalculateSpellDuration(SpellEntry const* spellProto, uint8 effect_index, Unit const* target, uint32 &periodicTime);
