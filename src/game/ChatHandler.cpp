...
void WorldSession::HandleMessagechatOpcode( WorldPacket & recv_data )
{
    ...
    switch(type)
    {
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if (!sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHAT) && tSecurity <= SEC_MODERATOR && pSecurity <= SEC_MODERATOR )
            {
...
void WorldSession::HandleTextEmoteOpcode( WorldPacket & recv_data )
{
	...
	if(unit && unit->GetTypeId() == TYPEID_PLAYER && em->Id == 9 && GetPlayer()->HasAura(70877)	&& !unit->HasAura(70867) && !unit->HasAura(70877)
		&& GetPlayer()->GetDistance2d(unit) < 5.0f && GetPlayer()->GetMapId() == 631)
	{
