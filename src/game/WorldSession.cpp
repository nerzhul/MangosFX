...
void WorldSession::SendPacket(WorldPacket const* packet)
{
	...
	{
		error_log("SendPacket Error... aborted.");
	}
}
    ...
    if (_player)
    {
        ...
        for (int i=0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
        {
            if(BattleGroundQueueTypeId bgQueueTypeId = _player->GetBattleGroundQueueTypeId(i))
            {
				...
				sBattleGroundMgr.RemovePlayerFromQueue(_player->GetGUID(),bgQueueTypeId,true);
            }
        }
...
uint32 WorldSession::getVoteTime()
{
	uint32 time = 0;
	if(QueryResult* result = loginDatabase.PQuery("SELECT (TIMESTAMP( NOW( ) ) - `last_vote1`) FROM vote_user WHERE uid = %d",GetAccountId()))
	{
		time = result->Fetch()->GetUInt32();
		delete result;
	}
	if(time >= 7200)
		time = 0;
...
void WorldSession::ExecuteOpcode(OpcodeHandler const& opHandle, WorldPacket* packet)
{
    ...
    if (_player)
    {
		...
		if (_player->IsHasDelayedTeleport() && !_player->GetVehicle())
