...
void WorldSession::HandleMoveWorldportAckOpcode()
{
	...
	bool bgSAf = false;
	/*if(GetPlayer()->GetBattleGround())
		if(GetPlayer()->GetBattleGround()->GetTypeID(false) == BATTLEGROUND_SA)
		{
			BattleGroundSA* bgSA = (BattleGroundSA*)GetPlayer()->GetBattleGround();
			if(!bgSA)
				return;
    ...
    if(!GetPlayer()->GetMap()->Add(GetPlayer()))
    {
		...
		GetPlayer()->ResurrectPlayer(100.0f);
        GetPlayer()->ResetMap();
...
void WorldSession::HandleMoveNotActiveMover(WorldPacket &recv_data)
{
    ...
    {
        //Log.outError("HandleMoveNotActiveMover: incorrect mover guid: mover is " I64FMT " and should be " I64FMT " instead of " UI64FMTD, _player->m_mover->GetGUID(), _player->GetGUID(), old_mover_guid);
        recv_data.rpos(recv_data.wpos());                   // prevent warnings spam
        return;
    }
    ...
    switch (recv_data.GetOpcode())
    {
		...
		case CMSG_CHANGE_SEATS_ON_CONTROLLED_VEHICLE:
		{
			...
			if(!vehicle_base)
			 ...
			 if(!recv_data.readPackGUID(guid) || vehicle_base->GetGUID() != guid)
				 return;
			_player->m_movementInfo = MovementInfo(recv_data);
		...
		case CMSG_REQUEST_VEHICLE_SWITCH_SEAT:
		{
		    ...
		    if(!vehicleGUID)                                        // something wrong here...
			 ...
			 if(Unit *vehUnit = Unit::GetUnit(*GetPlayer(), vehicleGUID))
                if(Vehicle *vehicle = vehUnit->GetVehicleKit())
				{
					uint64 guid;
					if(!recv_data.readPackGUID(guid))
					...
					if(guid)
					{
						if(vehicleGUID != guid)
						{
							if(Unit *vehUnit = Unit::GetUnit(*GetPlayer(), guid))
								if(Vehicle *veh = vehUnit->GetVehicleKit())
								{
