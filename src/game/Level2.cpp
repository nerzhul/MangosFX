...
bool ChatHandler::HandleKickPlayerCommand(const char *args)
{
	...
	if(m_session && (m_session->GetSecurity() <= ((Player*)target)->GetSession()->GetSecurity()))
		return false;
...
bool ChatHandler::HandleTicketCommand(const char* args)
{
	...
	{
		if(Player* pl = sObjectMgr.GetPlayer(target_guid))
			pl->GetSession()->SendGMTicketGetTicket(0x6,ticket->GetText(),true);
	}
    return true;
}
...
bool ChatHandler::HandleGetSpellInfoCommand(const char* args)
{
	...
	PSendSysMessage("Spell %i / Nom : %s / Icone : %u / SpellVisual[0] %u / SpellVisual[1] %u / activeIconId %u / FamilyFlags1 : " UI64FMTD " / FamilyFlags2 : " UI64FMTD "",
		...
		if(argstr == "forcetank")
		{
			switch(player->getClass())
			{
				...
				case CLASS_PALADIN:
					...
					player->AddItem(41238);
					player->AddItem(40671);
					player->AddItem(40675);
		...
		else if(argstr == "forcedps")
		{
			switch(player->getClass())
			{
				...
				case CLASS_PRIEST:
					...
					player->AddItem(43974);
					player->AddItem(41516);
		...
		else if(argstr == "forceheal")
		{
			switch(player->getClass())
			{
				case CLASS_DRUID:
					...
					player->AddItem(43264);
					player->AddItem(43265);
				...
				case CLASS_SHAMAN:
					...
					player->AddItem(43452);
					player->AddItem(43454);
				...
				case CLASS_PALADIN:
					...
					player->AddItem(42723);
					player->AddItem(42724);
...
bool ChatHandler::HandleDiamondCommand(const char* args)
{
	...
	int diamant = 0;
	QueryResult *result = loginDatabase.PQuery("SELECT credit_diamond FROM account WHERE id = '%u'", player->GetSession()->GetAccountId());
	if(result)
	{
	...
	if(argstr == "del")
	{
		if(diamant > 0)
		{
			SendSysMessage("Vous lui avez retire 1 diamant!");
			if(loginDatabase.PExecute("Update account set credit_diamond = credit_diamond - 1 where id = %u"
		...
		{
			SendSysMessage("Il n'a pas de diamant!");
			return true;
		}
	}
	else if(argstr == "get")
	{
		PSendSysMessage("%s a %d diamants.",player->GetName(),diamant);
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleLookPlayerInventory(const char* args)
{
	...
	if(!player)
		return false;
	/*QueryResult *result = CharacterDatabase.PQuery("SELECT bag FROM character_inventory WHERE guid = '%u' group by bag", player->GetGUID());
	if(result)
	{
		do
		{
			Field *fields = result->Fetch();
			uint64 bag = fields[0].GetUInt64();*/
			QueryResult *result2 = CharacterDatabase.PQuery("SELECT item_template FROM character_inventory WHERE guid = '%u' group by item_template", player->GetGUID()/*,bag*/);
			if(result2)
			{
				do
				{
					Field *fields2 = result2->Fetch();
					uint32 id = fields2[0].GetUInt32();
					...
					if ( loc_idx >= 0 )
					{
						ItemLocale const *il = sObjectMgr.GetItemLocale(id);
						if (il)
						{
