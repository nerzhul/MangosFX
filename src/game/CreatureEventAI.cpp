...
bool CreatureEventAI::ProcessEvent(CreatureEventAIHolder& pHolder, Unit* pActionInvoker)
{
    ...
    switch (event.event_type)
    {
        ...
        case EVENT_T_SUMMONED_JUST_DESPAWN:
        {
            ...
            //Repeat Timers
            pHolder.UpdateRepeatTimer(me,event.summoned.repeatMin,event.summoned.repeatMax);
            break;
        }
        ...
        case EVENT_T_BUFFED:
        {
            ...
            Aura* aura = me->GetAura(event.buffed.spellId, EFFECT_INDEX_0);
    ...
    switch (action.type)
    {
        ...
        case ACTION_T_CAST_EVENT:
            if (Unit* target = GetTargetByType(action.cast_event.target, pActionInvoker))
                if (target->GetTypeId() == TYPEID_PLAYER)
                    ((Player*)target)->CastedCreatureOrGO(action.cast_event.creatureId, me->GetGUID(), action.cast_event.spellId);
            break;
            ...
            if (pActionInvoker && pActionInvoker->GetTypeId() == TYPEID_PLAYER)
            {
                if (Unit* Temp = Unit::GetUnit(*me,pActionInvoker->GetGUID()))
                    if (Temp->GetTypeId() == TYPEID_PLAYER)
                        ((Player*)Temp)->GroupEventHappens(action.quest_event_all.questId,me);
            }
            break;
        case ACTION_T_CAST_EVENT_ALL:
        {
            ...
            for (ThreatList::const_iterator i = threatList.begin(); i != threatList.end(); ++i)
                if (Unit* Temp = Unit::GetUnit(*me,(*i)->getUnitGuid()))
                    if (Temp->GetTypeId() == TYPEID_PLAYER)
                        ((Player*)Temp)->CastedCreatureOrGO(action.cast_event_all.creatureId, me->GetGUID(), action.cast_event_all.spellId);
            break;
        }
...
void CreatureEventAI::MoveInLineOfSight(Unit *who)
{
    ...
    if (me->isCivilian() || me->IsNeutralToAll())
        return;

    if (!me->hasUnitState(UNIT_STAT_STUNNED | UNIT_STAT_DIED) && who->isTargetableForAttack() &&
        me->IsHostileTo(who) && who->isInAccessablePlaceFor(me))
    {
...
void CreatureEventAI::UpdateAI(const uint32 diff)
{
    ...
    bool Combat = me->SelectHostileTarget() && me->getVictim();
            ...
            for (std::list<CreatureEventAIHolder>::iterator i = CreatureEventAIList.begin(); i != CreatureEventAIList.end(); ++i)
            {
                ...
                switch ((*i).Event.event_type)
                {
                    ...
                    case EVENT_T_RANGE:
                        if (Combat)
                        {
                            if (me->getVictim() && me->IsInMap(me->getVictim()))
...
void CreatureEventAI::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    MaNGOS::CreatureListSearcher<MaNGOS::FriendlyCCedInRange> searcher(me, _list, u_check);
...
void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    ...
    MaNGOS::CreatureListSearcher<MaNGOS::FriendlyMissingBuffInRange> searcher(me, _list, u_check);
...
bool CreatureEventAI::CanCast(Unit* Target, SpellEntry const *Spell, bool Triggered)
{
    ...
    if (!Triggered && (me->hasUnitState(UNIT_STAT_CAN_NOT_REACT) ||
        me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED)))
        return false;
