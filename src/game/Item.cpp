...
void Item::UpdateDuration(Player* owner, uint32 diff)
{
    ...
    if (GetUInt32Value(ITEM_FIELD_DURATION)<=diff)
    {
		...
		{
			for(std::vector<LootItem>::iterator itr = loot.items.begin(); itr != loot.items.end(); ++itr)
			{
...
{
	CharacterDatabase.PExecute("DELETE FROM character_inventory_bak WHERE item = '%u'",GetGUIDLow());
...
bool Item::IsFitToSpellRequirements(SpellEntry const* spellInfo) const
{
	...
	if(proto->IsVellum() && spellEffect && spellEffect->Effect == SPELL_EFFECT_ENCHANT_ITEM)
	{
	...
	//Lava Lash
	if (spellInfo->Id==60103 && equippedItems->EquippedItemClass==ITEM_CLASS_WEAPON)
		return true;
