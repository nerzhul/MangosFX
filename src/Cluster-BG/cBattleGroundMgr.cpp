...
GroupQueueInfo * BattleGroundQueue::AddGroup(Player *leader, Group* grp, BattleGroundTypeId BgTypeId, PvPDifficultyEntry const*  backetEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 arenaRating, uint32 arenateamid)
{
    ...
    //add players from group to ginfo
    {
        ...
        if (!isRated && !isPremade)
        {
            if (cBattleGround* bg = sClusterBGMgr.GetBattleGroundTemplate(ginfo->BgTypeId))
            {
...
void BattleGroundQueue::RemovePlayer(const uint64& guid, bool decreaseInvitedCount)
{
    ...
    {
        cBattleGround* bg = sClusterBGMgr.GetBattleGround(group->IsInvitedToBGInstanceGUID, group->BgTypeId);
        if (bg)
    ...
    else if (!group->IsInvitedToBGInstanceGUID && group->IsRated)
    {
        ...
        {
            cBattleGround * bg = sClusterBGMgr.GetBattleGroundTemplate(group->BgTypeId);
            BattleGroundQueueTypeId bgQueueTypeId = sClusterBGMgr.BGQueueTypeId(group->BgTypeId, group->ArenaType);
            uint32 queueSlot = plr2->GetBattleGroundQueueIndex(bgQueueTypeId);
            plr2->RemoveBattleGroundQueueId(bgQueueTypeId); // must be called this way, because if you move this call to
                                                            // queue->removeplayer, it causes bugs
            WorldPacket data;
            sClusterBGMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_NONE, 0, 0, 0);
...
bool BattleGroundQueue::InviteGroupToBG(GroupQueueInfo * ginfo, cBattleGround * bg, uint32 side)
{
    // set side if needed
    if (side)
        ginfo->Team = side;
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        BattleGroundQueueTypeId bgQueueTypeId = sClusterBGMgr.BGQueueTypeId(bgTypeId, bg->GetArenaType());
        ...
        for(std::map<uint64,PlayerQueueInfo*>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            // send status packet
            sClusterBGMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, 0, ginfo->ArenaType);
...
*/
void BattleGroundQueue::FillPlayersToBG(cBattleGround* bg, BattleGroundBracketId bracket_id)
{
...
bool BattleGroundQueue::CheckPremadeMatch(BattleGroundBracketId bracket_id, uint32 MaxPlayersPerTeam)
{
    ...
    uint32 time_before = getMSTime() - 30 * MINUTE * IN_MILLISECONDS;
    for(uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE + i].empty())
        {
            GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE + i].begin();
            if (!(*itr)->IsInvitedToBGInstanceGUID && ((*itr)->JoinTime < time_before || (*itr)->Players.size() < MaxPlayersPerTeam))
            {
...
bool BattleGroundQueue::CheckNormalMatch(cBattleGround* bg_template, BattleGroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers)
{
...
void BattleGroundQueue::Update(BattleGroundTypeId bgTypeId, BattleGroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    for (itr = sClusterBGMgr.BGFreeSlotQueue[bgTypeId].begin(); itr != sClusterBGMgr.BGFreeSlotQueue[bgTypeId].end(); itr = next)
    {
        next = itr;
        ++next;
        ...
        {
            cBattleGround* bg = *itr; //we have to store battleground pointer here, because when battleground is full, it is removed from free queue (not yet implemented!!)
    ...
    cBattleGround * bg_template = sClusterBGMgr.GetBattleGroundTemplate(bgTypeId);
    if (!bg_template)
    {
    ...
    if (bg_template->isBattleGround())
    {
        ...
        if (CheckPremadeMatch(bracket_id, MaxPlayersPerTeam))
        {
            ...
            cBattleGround * bg2 = sClusterBGMgr.CreateNewBattleGround(bgTypeId, bracketEntry, 0, false);
            if (!bg2)
            {
    ...
    if (!isRated)
    {
            ...
            || (bg_template->isArena() && CheckSkirmishForSameFaction(bracket_id, MinPlayersPerTeam)) )
        {
            ...
            cBattleGround * bg2 = sClusterBGMgr.CreateNewBattleGround(bgTypeId, bracketEntry, arenaType, false);
            if (!bg2)
            {
                sLog.outError("BattleGroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
    ...
    else if (bg_template->isArena())
    {
        ...
        //set rating range
        uint32 arenaMinRating = (arenaRating <= sClusterBGMgr.GetMaxRatingDifference()) ? 0 : arenaRating - sClusterBGMgr.GetMaxRatingDifference();
        uint32 arenaMaxRating = arenaRating + sClusterBGMgr.GetMaxRatingDifference();
        // if max rating difference is set and the time past since server startup is greater than the rating discard time
        // (after what time the ratings aren't taken into account when making teams) then
        // the discard time is current_time - time_to_discard, teams that joined after that, will have their ratings taken into account
        // else leave the discard time on 0, this way all ratings will be discarded
        uint32 discardTime = getMSTime() - sClusterBGMgr.GetRatingDiscardTimer();
        ...
        {
            cBattleGround* arena = sClusterBGMgr.CreateNewBattleGround(bgTypeId, bracketEntry, arenaType, true);
            if (!arena)
            {
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    cBattleGround* bg = sClusterBGMgr.GetBattleGround(m_BgInstanceGUID, m_BgTypeId);
    //if battleground ended and its instance deleted - do nothing
    if (!bg)
        return true;

    BattleGroundQueueTypeId bgQueueTypeId = sClusterBGMgr.BGQueueTypeId(bg->GetTypeID(), bg->GetArenaType());
    uint32 queueSlot = plr->GetBattleGroundQueueIndex(bgQueueTypeId);
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            sClusterBGMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME - INVITATION_REMIND_TIME, 0, m_ArenaType);
            plr->GetSession()->SendPacket(&data);
        }
    }
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
    {
        ...
        BattleGroundQueue &bgQueue = sClusterBGMgr.m_BattleGroundQueues[m_BgQueueTypeId];
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            if (bg && bg->isBattleGround() && bg->GetStatus() != STATUS_WAIT_LEAVE)
                sClusterBGMgr.ScheduleQueueUpdate(0, 0, m_BgQueueTypeId, m_BgTypeId, bg->GetBracketId());
...
cBattleGroundMgr::cBattleGroundMgr(): m_AutoDistributionTimeChecker(0)
{
	for(uint32 i = BATTLEGROUND_TYPE_NONE; i < MAX_BATTLEGROUND_TYPE_ID; i++)
        m_BattleGrounds[i].clear();
	m_NextRatingDiscardUpdate = 10 * MINUTE * IN_MILLISECONDS;
}
...
void cBattleGroundMgr::DeleteAllBattleGrounds()
{
	for(uint32 i = BATTLEGROUND_TYPE_NONE; i < MAX_BATTLEGROUND_TYPE_ID; i++)
    {
        for(cBattleGroundSet::iterator itr = m_BattleGrounds[i].begin(); itr != m_BattleGrounds[i].end();)
        {
            cBattleGround * bg = itr->second;
...
void cBattleGroundMgr::ScheduleQueueUpdate(uint32 arenaRating, uint8 arenaType, BattleGroundQueueTypeId bgQueueTypeId, BattleGroundTypeId bgTypeId, BattleGroundBracketId bracket_id)
{
...
{
	cBattleGroundSet::iterator itr, next;
    for(uint32 i = BATTLEGROUND_TYPE_NONE; i < MAX_BATTLEGROUND_TYPE_ID; i++)
    {
        ...
        for(int qtype = BATTLEGROUND_QUEUE_2v2; qtype <= BATTLEGROUND_QUEUE_5v5; ++qtype)
            for(int bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
                m_BattleGroundQueues[qtype].Update(
                    ...
                    sClusterBGMgr.BGArenaType(BattleGroundQueueTypeId(qtype)), true, 0);
    ...
    {
        if (sClusterBG.GetGameTime() > m_NextAutoDistributionTime)
        {
            DistributeArenaPoints();
            m_NextAutoDistributionTime = time_t(sClusterBG.GetGameTime() + BATTLEGROUND_ARENA_POINT_DISTRIBUTION_DAY * 7);
...
void cBattleGroundMgr::BuildPvpLogDataPacket(WorldPacket *data, cBattleGround *bg)
{
    ...
    *data << uint8(type);                                   // type (battleground=0/arena=1)
    ...
    if(type)                                                // arena
    {
        ...
        for(int i = 1; i >= 0; --i)
        {
			*data << uint32(bg->GetArenaTeamRatingChangeForTeam((i==0)?ALLIANCE:HORDE));
    ...
    *data << (int32)(bg->GetPlayerScoresSize());
	...
	for(cBattleGround::BattleGroundScoreMap::const_iterator itr = bg->GetPlayerScoresBegin(); itr != bg->GetPlayerScoresEnd(); ++itr)
    {
...
void cBattleGroundMgr::BuildGroupJoinedBattlegroundPacket(WorldPacket *data, GroupJoinBattlegroundResult result)
{
...
cBattleGround * cBattleGroundMgr::GetBattleGroundThroughClientInstance(uint32 instanceId, BattleGroundTypeId bgTypeId)
{
    ...
    for(cBattleGroundSet::iterator itr = m_BattleGrounds[bgTypeId].begin(); itr != m_BattleGrounds[bgTypeId].end(); ++itr)
    {
...
cBattleGround * cBattleGroundMgr::GetBattleGround(uint32 InstanceID, BattleGroundTypeId bgTypeId)
{
    //search if needed
    cBattleGroundSet::iterator itr;
...
cBattleGround * cBattleGroundMgr::GetBattleGroundTemplate(BattleGroundTypeId bgTypeId)
{
...
uint32 cBattleGroundMgr::CreateClientVisibleInstanceId(BattleGroundTypeId bgTypeId, BattleGroundBracketId bracket_id)
{
...
cBattleGround * cBattleGroundMgr::CreateNewBattleGround(BattleGroundTypeId bgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    // get the template BG
    cBattleGround *bg_template = GetBattleGroundTemplate(bgTypeId);
    if (!bg_template)
    {
    ...
    if (bg_template->isArena())
    {
        BattleGroundTypeId arenas[] = {BATTLEGROUND_NA, BATTLEGROUND_BE, BATTLEGROUND_RL, BATTLEGROUND_RV, BATTLEGROUND_DS};
    ...
    {
        /*case BATTLEGROUND_AV:
            bg = new cBattleGroundAV(*(cBattleGroundAV*)bg_template);
            break;*/
        case BATTLEGROUND_WS:
            bg = new cBattleGroundWS(*(cBattleGroundWS*)bg_template);
            break;
        case BATTLEGROUND_AB:
            bg = new cBattleGroundAB(*(cBattleGroundAB*)bg_template);
            break;
        case BATTLEGROUND_NA:
            bg = new cBattleGroundNA(*(cBattleGroundNA*)bg_template);
            break;
        case BATTLEGROUND_BE:
            bg = new cBattleGroundBE(*(cBattleGroundBE*)bg_template);
            break;
        case BATTLEGROUND_AA:
            bg = new cBattleGroundAA(*(cBattleGroundAA*)bg_template);
            break;
        /*case BATTLEGROUND_EY:
            bg = new cBattleGroundEY(*(cBattleGroundEY*)bg_template);
            break;*/
        case BATTLEGROUND_RL:
            bg = new cBattleGroundRL(*(cBattleGroundRL*)bg_template);
            break;
        case BATTLEGROUND_SA:
            bg = new cBattleGroundSA(*(cBattleGroundSA*)bg_template);
            break;
        case BATTLEGROUND_DS:
            bg = new cBattleGroundDS(*(cBattleGroundDS*)bg_template);
            break;
        case BATTLEGROUND_RV:
            bg = new cBattleGroundRV(*(cBattleGroundRV*)bg_template);
            break;
        /*case BATTLEGROUND_IC:
            bg = new cBattleGroundIC(*(cBattleGroundIC*)bg_template);
            break;*/
        case BATTLEGROUND_RB:
			bg = new cBattleGroundABG(*(cBattleGroundABG*)bg_template);
            break;
        default:
    ...
    bg->SetInstanceID(sClusterObjectMgr.GenerateInstanceId()); // set instance id
...
uint32 cBattleGroundMgr::CreateBattleGround(BattleGroundTypeId bgTypeId, bool IsArena, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam, uint32 LevelMin, uint32 LevelMax, uint32 MapID, float Team1StartLocX, float Team1StartLocY, float Team1StartLocZ, float Team1StartLocO, float Team2StartLocX, float Team2StartLocY, float Team2StartLocZ, float Team2StartLocO)
{
    // Create the BG
    cBattleGround *bg = NULL;
    switch(bgTypeId)
    {
        //case BATTLEGROUND_AV: bg = new cBattleGroundAV; break;
        case BATTLEGROUND_WS: bg = new cBattleGroundWS; break;
        case BATTLEGROUND_AB: bg = new cBattleGroundAB; break;
        case BATTLEGROUND_NA: bg = new cBattleGroundNA; break;
        case BATTLEGROUND_BE: bg = new cBattleGroundBE; break;
        case BATTLEGROUND_AA: bg = new cBattleGroundAA; break;
        //case BATTLEGROUND_EY: bg = new cBattleGroundEY; break;
        case BATTLEGROUND_RL: bg = new cBattleGroundRL; break;
        case BATTLEGROUND_SA: bg = new cBattleGroundSA; break;
        case BATTLEGROUND_DS: bg = new cBattleGroundDS; break;
        case BATTLEGROUND_RV: bg = new cBattleGroundRV; break;
        //case BATTLEGROUND_IC: bg = new cBattleGroundIC; break;
        case BATTLEGROUND_RB: bg = new cBattleGroundABG; break;
        default:bg = new cBattleGround;   break;             // placeholder for non implemented BG
    }
...
HolidayIds cBattleGroundMgr::BGTypeToWeekendHolidayId(BattleGroundTypeId bgTypeId)
{
    switch (bgTypeId)
    {
...
BattleGroundTypeId cBattleGroundMgr::WeekendHolidayIdToBGType(HolidayIds holiday)
{
    switch (holiday)
    {
...
bool cBattleGroundMgr::IsArenaType(BattleGroundTypeId bgTypeId)
{
...
BattleGroundTypeId cBattleGroundMgr::BGTemplateId(BattleGroundQueueTypeId bgQueueTypeId)
{
    switch(bgQueueTypeId)
    {
        ...
        case BATTLEGROUND_QUEUE_RANDOM:
...
uint8 cBattleGroundMgr::BGArenaType(BattleGroundQueueTypeId bgQueueTypeId)
{
    switch(bgQueueTypeId)
    {
        case BATTLEGROUND_QUEUE_2v2:
...
bool cBattleGroundMgr::IsBGWeekend(BattleGroundTypeId bgTypeId)
{
    return /*IsHolidayActive(BGTypeToWeekendHolidayId(bgTypeId))*/ false;
}


void cBattleGroundMgr::LoadBattleMastersEntry()
{
...
void cBattleGroundMgr::LoadBattleEventIndexes()
{
...
void cBattleGroundMgr::SendToBattleGround(Player *pl, uint32 instanceId, BattleGroundTypeId bgTypeId)
{
    cBattleGround *bg = GetBattleGround(instanceId, bgTypeId);
    if (bg)
    {
...
void cBattleGroundMgr::CreateInitialBattleGrounds()
{
    uint32 count = 0;
    ...
    do
    {
        ...
        if (!CreateBattleGround(bgTypeID, IsArena, MinPlayersPerTeam, MaxPlayersPerTeam, bl->minLevel, bl->maxLevel, bl->mapid[0], AStartLoc[0], AStartLoc[1], AStartLoc[2], AStartLoc[3], HStartLoc[0], HStartLoc[1], HStartLoc[2], HStartLoc[3]))
            continue;
...
void cBattleGroundMgr::InitAutomaticArenaPointDistribution()
{
...
void cBattleGroundMgr::DistributeArenaPoints()
{
