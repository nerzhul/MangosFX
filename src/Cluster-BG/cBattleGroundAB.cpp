...
cBattleGroundAB::cBattleGroundAB()
{
    SetBuffChange(true);
...
void cBattleGroundAB::Update(uint32 diff)
{
...
void cBattleGroundAB::StartingEventCloseDoors()
{
    // despawn buffs
...
void cBattleGroundAB::StartingEventOpenDoors()
{
...
void cBattleGroundAB::AddPlayer(Player *plr)
{
...
void cBattleGroundAB::RemovePlayer(Player * /*plr*/, uint64 /*guid*/)
{
}

void cBattleGroundAB::HandleAreaTrigger(Player *Source, uint32 Trigger)
{
    switch(Trigger)
    {
...
void cBattleGroundAB::_CreateBanner(uint8 node, uint8 type, uint8 teamIndex, bool delay)
{
...
int32 cBattleGroundAB::_GetNodeNameId(uint8 node)
{
    switch (node)
    {
...
void cBattleGroundAB::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
...
void cBattleGroundAB::_SendNodeUpdate(uint8 node)
{
...
void cBattleGroundAB::_NodeOccupied(uint8 node,Team team)
{
...
void cBattleGroundAB::EventPlayerClickedOnFlag(Player *source, GameObject* target_obj)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
bool cBattleGroundAB::SetupBattleGround()
{
    //buffs
    for (int i = 0; i < BG_AB_NODES_MAX; ++i)
    {
...
void cBattleGroundAB::EndBattleGround(uint32 winner)
{
    //win reward
	if (winner)
	{
...
WorldSafeLocsEntry const* cBattleGroundAB::GetClosestGraveYard(Player* player)
{
...
void cBattleGroundAB::UpdatePlayerScore(Player *Source, uint32 type, uint32 value)
{
...
bool cBattleGroundAB::IsAllNodesConrolledByTeam(uint32 team) const
{
    uint32 count = 0;
    for (int i = 0; i < BG_AB_NODES_MAX; ++i)
