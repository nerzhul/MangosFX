...
struct MANGOS_DLL_DECL boss_twinemperorsAI : public ScriptedAI
{
    ...
    void TryHealBrother(uint32 diff)
    {
        ...
        {
            Unit *pOtherBoss = GetOtherBoss();
            if (pOtherBoss && pOtherBoss->IsWithinDist(me, 60.0f))
            {
    ...
    Unit *GetAnyoneCloseEnough(float dist, bool totallyRandom)
    {
        ...
        for (i = me->getThreatManager().getThreatList().begin();i != me->getThreatManager().getThreatList().end(); ++i)
        {
            ...
            if (me->IsWithinDistInMap(pUnit, dist))
            {
                if (!totallyRandom)
                    return pUnit;
        ...
        i = candidates.begin();
        Unit *ret = Unit::GetUnit((*me), (*i)->getUnitGuid());
        candidates.clear();
        return ret;
    }
    ...
    Unit *PickNearestPlayer()
    {
        ...
        for (i = me->getThreatManager().getThreatList().begin();i != me->getThreatManager().getThreatList().end(); ++i)
        {
            ...
            if (!pUnit)
                continue;
            float pudist = pUnit->GetDistance((const Creature *)me);
    ...
    void SetAfterTeleport()
    {
        ...
        DoStopAttack();
        DoResetThreat();
        DoCastMe( SPELL_TWIN_TELEPORT_VISUAL);
    ...
    Creature *RespawnNearbyBugsAndGetOne()
    {
        ...
        GetCreatureListWithEntryInGrid(lUnitList,me,15316,150.0f);
        GetCreatureListWithEntryInGrid(lUnitList,me,15317,150.0f);
