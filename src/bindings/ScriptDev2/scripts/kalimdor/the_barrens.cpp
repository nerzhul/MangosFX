...
struct MANGOS_DLL_DECL npc_giltharesAI : public npc_escortAI
{
    ...
    void Aggro(Unit* pWho)
    {
        ...
        if (pWho->GetTypeId() != TYPEID_PLAYER && me->GetAreaId() == AREA_MERCHANT_COAST)
        {
            ...
            switch(urand(0, 3))
            {
                case 0: DoScriptText(SAY_GIL_AGGRO_1, me, pWho); break;
                case 1: DoScriptText(SAY_GIL_AGGRO_2, me, pWho); break;
                case 2: DoScriptText(SAY_GIL_AGGRO_3, me, pWho); break;
                case 3: DoScriptText(SAY_GIL_AGGRO_4, me, pWho); break;
            }
        }
    }
};
...
struct MANGOS_DLL_DECL npc_twiggy_flatheadAI : public ScriptedAI
{
    ...
    bool CanStartEvent(Player* pPlayer)
    {
        if (!EventInProgress)
        {
            ...
            DoScriptText(SAY_TWIGGY_BEGIN, me, pPlayer);
            return true;
        }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (ChallengerDeath_Timer)
        {
            if (ChallengerDeath_Timer <= diff)
            {
                for(uint8 i = 0; i < 6; ++i)
                {
                    Creature *challenger = (Creature*)Unit::GetUnit(*me,AffrayChallenger[i]);
                    if (challenger && !challenger->isAlive() && challenger->isDead())
                    {
                        DoScriptText(SAY_TWIGGY_DOWN, me);
        ...
        if (Event_Timer < diff)
        {
            ...
            switch(Step)
            {
                case 0:
                    ...
                    break;
                case 1:
                    DoScriptText(SAY_TWIGGY_FRAY, me);
                    if (Unit *challenger = Unit::GetUnit(*me,AffrayChallenger[Challenger_Count]))
                    ...
                    break;
                case 2:
                    if (Unit *temp = me->SummonCreature(NPC_BIG_WILL,-1713.79,-4342.09,6.05,6.15,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,300000))
                    {
                    ...
                    break;
                case 3:
                    if (Unit *will = Unit::GetUnit(*me,BigWillGUID))
                    {
                        will->setFaction(32);
                    ...
                    break;
                case 4:
                    Unit *will = Unit::GetUnit(*me,BigWillGUID);
                    if (will && will->isDead())
                    {
                        DoScriptText(SAY_TWIGGY_OVER, me);
                        Reset();
                    } else if (!will)
                        Reset();
                    Event_Timer = 5000;
        ...
        switch(uiPointId)
        {
            case 0:
                if (Player* pPlayer = GetPlayerForEscort())
                    DoScriptText(SAY_STARTUP1, me, pPlayer);
                break;
            case 9:
                SetRun(false);
                break;
