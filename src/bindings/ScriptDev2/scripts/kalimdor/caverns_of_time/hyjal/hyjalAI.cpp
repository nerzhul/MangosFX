...
void hyjalAI::SpawnCreatureForWave(uint32 uiMobEntry)
{
    ...
    if (pSpawn)
        me->SummonCreature(uiMobEntry, pSpawn->m_fX, pSpawn->m_fY, pSpawn->m_fZ, 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 120000);
}
...
void hyjalAI::DoTalk(YellType pYellType)
{
    ...
    for (uint32 i = 0; i < uiMaxCount; ++i)
    {
        if (m_aHyjalYell[i].uiCreatureEntry == me->GetEntry() && m_aHyjalYell[i].m_pYellType == pYellType)
        {
    ...
    if (pYell)
        DoScriptText(pYell->m_iTextId, me);
}
...
void hyjalAI::UpdateAI(const uint32 diff)
{
            ...
            if (!lWaveMobGUIDList.empty())
            {
                for(std::list<uint64>::iterator itr = lWaveMobGUIDList.begin(); itr != lWaveMobGUIDList.end(); ++itr)
                {
                    if (Creature* pTemp = m_pInstance->instance->GetCreature(*itr))
                    {
                        ...
                        pTemp->GetMotionMaster()->MovePoint(1, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ());
                    }
                }
            }
    ...
    if (m_uiCheckTimer < diff)
    {
        for(uint8 i = 0; i < 2; ++i)
        {
            if (m_uiBossGUID[i])
            {
                Unit* pUnit = Unit::GetUnit(*me, m_uiBossGUID[i]);
