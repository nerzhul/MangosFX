...
struct MANGOS_DLL_DECL npc_thrall_old_hillsbradAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        ...
        switch(i)
        {
            case 8:
                SetRun(false);
                me->SummonCreature(ENTRY_ARMORER,2181.87,112.46,89.45,0.26,TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,5000);
                break;
            case 9:
            ...
            case 106:
                ...
                if (Creature* pTaretha = pInstance->instance->GetCreature(pInstance->GetData64(DATA_TARETHA)))
                {
                ...
                if (!players.isEmpty() && map->IsDungeon())
                {
                    for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    {
                        if (Player* pPlayer = itr->getSource())
                            pPlayer->KilledMonsterCredit(NPC_THRALL_QUEST_TRIGGER, me->GetGUID());
                    }
                }
    ...
    void JustDied(Unit *slayer)
    {
        ...
        DoScriptText(urand(0, 1) ? SAY_TH_RANDOM_DIE1 : SAY_TH_RANDOM_DIE2, me);
    }
    ...
    void UpdateEscortAI(const uint32 diff)
    {
        ...
        {
            DoScriptText(urand(0, 1) ? SAY_TH_RANDOM_LOW_HP1 : SAY_TH_RANDOM_LOW_HP2, me);
            LowHp = true;
        }
