    ...
    void DamageTaken(Unit* pDoneBy, uint32 &uiDamage)
    {
        ...
        if (Player* pPlayer = pDoneBy->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            if (pPlayer->GetQuestStatus(QUEST_WHATS_HAUNTING_WITCH_HILL) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                me->CastSpell(pDoneBy, SPELL_SUMMON_RESTLESS_APPARITION, true);
            }
        }
    }
...
struct MANGOS_DLL_DECL npc_restless_apparitionAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiTalk_Timer <= diff)
        {
            switch(urand(0, 7))
            {
                case 0: DoScriptText(SAY_RAND_1, me); break;
                case 1: DoScriptText(SAY_RAND_2, me); break;
                case 2: DoScriptText(SAY_RAND_3, me); break;
                case 3: DoScriptText(SAY_RAND_4, me); break;
                case 4: DoScriptText(SAY_RAND_5, me); break;
                case 5: DoScriptText(SAY_RAND_6, me); break;
                case 6: DoScriptText(SAY_RAND_7, me); break;
                case 7: DoScriptText(SAY_RAND_8, me); break;
            }
