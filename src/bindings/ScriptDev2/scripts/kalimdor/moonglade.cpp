...
struct MANGOS_DLL_DECL npc_clintar_dw_spiritAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        ...
        switch(i)
        {
            ...
            case 13:
                ...
                break;
            case 14:
                DoScriptText(SAY_RELIC1, me, pPlayer);
                break;
            case 26:
                me->HandleEmoteCommand(EMOTE_STATE_USESTANDING_NOSHEATHE);
                break;
            case 27:
                DoScriptText(SAY_RELIC2, me, pPlayer);
                break;
            case 31:
                me->SummonCreature(NPC_ASPECT_OF_RAVEN, 7465.321, -3088.515, 429.006, 5.550, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
                break;
            case 35:
                me->HandleEmoteCommand(EMOTE_STATE_USESTANDING_NOSHEATHE);
                break;
            case 36:
                DoScriptText(SAY_RELIC3, me, pPlayer);
                break;
            case 49:
    ...
    void Reset()
    {
        ...
        //me are expected to always be spawned, but not visible for player
        ...
        if (CreatureInfo const* pTemp = GetCreatureTemplateStore(me->GetEntry()))
			me->SetDisplayId(pTemp->ModelId[2]);
...
bool EffectDummyCreature_npc_clintar_dw_spirit(Unit *pCaster, uint32 spellId, uint32 effIndex, Creature *pCreatureTarget)
{
    ...
    if (spellId == SPELL_EMERALD_DREAM && effIndex == 0)
    {
        ...
        if (CreatureInfo const* pTemp = GetCreatureTemplateStore(NPC_CLINTAR_SPIRIT))
			pCreatureTarget->SetDisplayId(pTemp->ModelId[2]);
        else
            return true;
