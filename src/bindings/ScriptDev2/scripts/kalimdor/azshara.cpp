...
struct MANGOS_DLL_DECL npc_rizzle_sprysprocketAI : public npc_escortAI
{
    ...
    void MoveInLineOfSight(Unit* pUnit)
    {
        ...
        {
            if (!HasEscortState(STATE_ESCORT_PAUSED) && me->IsWithinDistInMap(pUnit, INTERACTION_DISTANCE) && me->IsWithinLOSInMap(pUnit))
            {
                if (((Player*)pUnit)->GetQuestStatus(QUEST_MOONSTONE) == QUEST_STATUS_INCOMPLETE)
                    me->CastSpell(me,SPELL_SURRENDER,true);
            }
        }
    ...
    void WaypointReached(uint32 uiPointId)
    {
        ...
        {
            case 0:
                me->CastSpell(me,SPELL_PERIODIC_CHECK,true);
                break;
        }
    }
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* pSpell)
    {
        if (pTarget->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_FROST_GRENADE)
            DoScriptText(SAY_WHISPER_CHILL, me, pTarget);
    }

    //this may be wrong
    void JustSummoned(Creature* pSummoned)
    {
        //pSummoned->CastSpell(pSummoned,SPELL_PERIODIC_GRENADE,false,0,0,me->GetGUID());
    }
    ...
    void UpdateEscortAI(const uint32 diff)
    {
        if (m_bIsIntro)
        {
            ...
            switch(m_uiIntroPhase)
            {
                ...
                case 1:
                    ...
                    me->CastSpell(me,SPELL_ESCAPE,false);
                    break;
                case 2:
                    me->CastSpell(me,SPELL_SWIM_SPEED,false);
                    m_bIsIntro = false;
                    Start(false,true);
                    break;
            }
        ...
        if (m_uiDepthChargeTimer < diff)
        {
            if (!HasEscortState(STATE_ESCORT_PAUSED))
                me->CastSpell(me,SPELL_SUMMON_DEPTH_CHARGE,false);

            m_uiDepthChargeTimer = urand(10000, 15000);
        }else m_uiDepthChargeTimer -= diff;
    }
};
...
struct MANGOS_DLL_DECL npc_depth_chargeAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pUnit)
    {
        ...
        if (me->IsWithinDistInMap(pUnit, INTERACTION_DISTANCE) && me->IsWithinLOSInMap(pUnit))
            me->CastSpell(pUnit,SPELL_TRAP,false);
    }
