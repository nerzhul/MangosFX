...
struct MANGOS_DLL_DECL npc_draenei_survivorAI : public ScriptedAI
{
    ...
    {
        if (m_bCanSayHelp && pWho->GetTypeId() == TYPEID_PLAYER && me->IsFriendlyTo(pWho) &&
            me->IsWithinDistInMap(pWho, 25.0f))
        {
            //Random switch between 4 texts
            switch(urand(0, 3))
            {
                case 0: DoScriptText(SAY_HELP1, me, pWho); break;
                case 1: DoScriptText(SAY_HELP2, me, pWho); break;
                case 2: DoScriptText(SAY_HELP3, me, pWho); break;
                case 3: DoScriptText(SAY_HELP4, me, pWho); break;
            }
    ...
    {
        if (pSpell->GetSpellFamilyFlags2() & 0x080000000)
        {
    ...
    void UpdateAI(const uint32 diff)
    {
        if (m_uiSayThanksTimer)
        {
            if (m_uiSayThanksTimer <= diff)
            {
                ...
                if (Player* pPlayer = (Player*)Unit::GetUnit(*me,m_uiCaster))
                {
                    ...
                    switch(urand(0, 3))
                    {
                        case 0: DoScriptText(SAY_HEAL1, me, pPlayer); break;
                        case 1: DoScriptText(SAY_HEAL2, me, pPlayer); break;
                        case 2: DoScriptText(SAY_HEAL3, me, pPlayer); break;
                        case 3: DoScriptText(SAY_HEAL4, me, pPlayer); break;
                    }
...
struct MANGOS_DLL_DECL npc_engineer_spark_overgrindAI : public ScriptedAI
{
    ...
    void Aggro(Unit *who)
    {
        DoScriptText(SAY_ATTACK, me, who);
    }

    void UpdateAI(const uint32 diff)
    {
        if (!me->isInCombat() && !m_bIsTreeEvent)
        {
            if (m_uiEmoteTimer < diff)
            {
                DoScriptText(SAY_TEXT, me);
                DoScriptText(EMOTE_SHELL, me);
                m_uiEmoteTimer = urand(120000, 150000);
            }
    ...
    void WaypointReached(uint32 i)
    {
        ...
        switch(i)
        {
            ...
            case 29:
                ...
                pPlayer->GroupEventHappens(QUEST_A_CRY_FOR_HELP, me);
                break;
        }
    }
