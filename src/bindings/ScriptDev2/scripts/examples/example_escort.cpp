        ...
        switch (uiWP)
        {
            ...
            case 4:
                if (Player* pTmpPlayer = GetPlayerForEscort())
                {
                    ...
                    DoScriptText(SAY_WP_3, me, pTmpPlayer);
    ...
    void Aggro(Unit* pWho)
    {
        if (HasEscortState(STATE_ESCORT_ESCORTING))
        {
            if (Player* pTemp = GetPlayerForEscort())
                DoScriptText(SAY_AGGRO1, me, pTemp);
        }
        else
        ...
        if (HasEscortState(STATE_ESCORT_ESCORTING))
        {
            if (Player* pTemp = GetPlayerForEscort())
            {
                ...
                if (pKiller == me)
                {
                    ...
                    DoScriptText(SAY_DEATH_1, me, pTemp);
                }
                else
                    DoScriptText(SAY_DEATH_2, me, pTemp);
            }
        }
        else
            ...
            if (HasEscortState(STATE_ESCORT_ESCORTING))
            {
                if (m_uiChatTimer < uiDiff)
                {
                    if (me->HasAura(SPELL_ELIXIR_OF_FORTITUDE, 0))
                    {
                        ...
                        me->CastSpell(me, SPELL_BLUE_FIREWORK, false);
                    }
                    else
                    {
                        DoScriptText(SAY_RAND_2, me);
                        me->CastSpell(me, SPELL_ELIXIR_OF_FORTITUDE, false);
                    }
...
bool GossipHello_example_escort(Player* pPlayer, Creature* pCreature)
{
    ...
    //pPlayer->prepareGossipMenu(pPlayer, 0);
