...
struct MANGOS_DLL_DECL boss_kalecgosAI : public ScriptedAI
{
    ...
    {
        DoScriptText(urand(0, 1) ? SAY_EVIL_SLAY1 : SAY_EVIL_SLAY2, me);
    }
...
struct MANGOS_DLL_DECL boss_sathrovarrAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        me->CastSpell(me, SPELL_SPECTRAL_INVIS, true);
    }
    ...
    {
        DoScriptText(urand(0, 1) ? SAY_SATH_SLAY1 : SAY_SATH_SLAY2, me);
    }
...
struct MANGOS_DLL_DECL boss_kalecgos_humanoidAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (((me->GetHealth()*100 / me->GetMaxHealth()) < 20) && !HasYelled20Percent)
        {
            DoScriptText(SAY_GOOD_NEAR_DEATH, me);
            HasYelled20Percent = true;
        }

        if (((me->GetHealth()*100 / me->GetMaxHealth()) < 10) && !HasYelled10Percent)
        {
