...
struct MANGOS_DLL_DECL boss_malacrassAI : public ScriptedAI
{
    ...
    void JustReachedHome()
    {
        ...
        {
            if (Creature* pAdd = (Creature*)Unit::GetUnit(*me, m_auiAddGUIDs[i]))
    ...
    void InitializeAdds()
    {
        ...
        if (m_lAddsEntryList.empty())
        {
            ...
            {
                if (Creature* pAdd = me->SummonCreature((*itr), m_afAddPosX[j], ADD_POS_Y, ADD_POS_Z, ADD_ORIENT, TEMPSUMMON_CORPSE_DESPAWN, 0))
        ...
        else
        {
            for(std::list<uint32>::iterator itr = m_lAddsEntryList.begin(); itr != m_lAddsEntryList.end(); ++itr)
            {
                Unit* pAdd = Unit::GetUnit(*me, m_auiAddGUIDs[j]);
...
struct MANGOS_DLL_DECL boss_malacrass_addAI : public ScriptedAI
{
    ...
    bool IsEnemyPlayerInRangeForSpell(uint32 uiSpellId)
    {
       ...
       ThreatList const& lThreatList = me->getThreatManager().getThreatList();

        for(ThreatList::const_iterator iter = lThreatList.begin(); iter != lThreatList.end(); ++iter)
        {
            Unit* pTarget = Unit::GetUnit((*me), (*iter)->getUnitGuid());

            if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER)
            {
                //if target further away than maxrange or closer than minrange, statement is false
                if (me->IsInRange(pTarget, pSpellRange->minRange, pSpellRange->maxRange))
                    return true;
            }
        }
...
struct MANGOS_DLL_DECL mob_alyson_antilleAI : public boss_malacrass_addAI
{
    ...
    void AttackStart(Unit* pWho)
    {
        ...
        if (me->Attack(pWho, false))
        {
            ...
            me->GetMotionMaster()->MoveChase(pWho, 20.0f);
        }
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiFlashHealTimer < diff)
        {
            ...
            {
                if (pTarget->IsWithinDistInMap(me, RANGE_FRIENDLY_TARGET))
                {
                ...
                else
                {
                    ...
                    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == CHASE_MOTION_TYPE)
                    {
                        ...
                        me->GetMotionMaster()->MoveFollow(pTarget, 20.0f, 0.0f);
                    }
                }
            }
...
struct MANGOS_DLL_DECL mob_darkheartAI : public boss_malacrass_addAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiPsychicWailTimer < diff)
        {
            ...
            {
                DoCastVictim( SPELL_PSYCHIC_WAIL);
