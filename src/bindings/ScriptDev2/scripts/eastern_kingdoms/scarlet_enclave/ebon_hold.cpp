...
struct MANGOS_DLL_DECL npc_a_special_surpriseAI : public ScriptedAI
{
    ...
    {
        if (m_uiPlayerGUID || pWho->GetTypeId() != TYPEID_PLAYER || !pWho->IsWithinDist(me, INTERACTION_DISTANCE))
            return;
                ...
                switch(pPlayer->getRace())
                {
                    case RACE_HUMAN:
                        switch(m_uiExecuteSpeech_Counter)
                        {
                            ...
                            case 8:
                                if (Creature* pPlaguefist = GetClosestCreatureWithEntry(me, NPC_PLAGUEFIST, 85.0f))
...
struct MANGOS_DLL_DECL npc_death_knight_initiateAI : public ScriptedAI
{
    ...
    {
        if (m_bIsDuelInProgress && uiDamage > me->GetHealth())
        {
            uiDamage = 0;

            if (Unit* pUnit = Unit::GetUnit(*me, m_uiDuelerGUID))
                me->CastSpell(pUnit, SPELL_DUEL_VICTORY, true);
    ...
    void WaypointReached(uint32 uiPointId)
    {
        switch(uiPointId)
        {
            ...
            case 2:
                ...
                DoCastMe( SPELL_KOLTIRA_TRANSFORM);
                break;
            case 3:
                SetEscortPaused(true);
                me->SetStandState(UNIT_STAND_STATE_KNEEL);
                DoScriptText(SAY_BREAKOUT2, me);
                DoCastMe( SPELL_ANTI_MAGIC_ZONE);  // cast again that makes bubble up
                break;
            case 4:
                SetRun(true);
                break;
...
struct MANGOS_DLL_DECL npc_unworthy_initiate_anchorAI : public ScriptedAI
{
    ...
    {
        Creature* pInitiate = (Creature*)Unit::GetUnit(*me, m_uiMyInitiate);

        if (pInitiate && pSource)
        {
            pInitiate->SetLootRecipient(pSource);
            me->CastSpell(pInitiate,SPELL_CHAINED_PESANT_BREATH,true);
        }
    }
...
struct MANGOS_DLL_DECL npc_unworthy_initiateAI : public ScriptedAI
{
    ...
    void SetAnchor()
    {
        if (Creature* pAnchor = GetClosestCreatureWithEntry(me, NPC_ANCHOR, INTERACTION_DISTANCE*2))
        {
            ((npc_unworthy_initiate_anchorAI*)pAnchor->AI())->RegisterCloseInitiate(me->GetGUID());

            pAnchor->CastSpell(me, SPELL_CHAINED_PESANT_CHEST, false);
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        if (pSpell->Id == SPELL_CHAINED_PESANT_BREATH)
        {
            ...
            if (Player* pSource = me->GetLootRecipient())
                DoScriptText(GetTextId(), me, pSource);
        }
    }
        ...
        else
        {
            if (m_uiPhaseTimer < diff)
            {
                if (m_uiPhase == PHASE_DRESSUP)
                {
                    if (m_pToTransform)
                    {
                        me->CastSpell(me, m_pToTransform->m_uiSpellToNewDisplay, true);
                        me->CastSpell(me, SPELL_INITIATE_VISUAL, false);
                    }
                    else
                ...
                else
                {
                    ...
                    if (Player* pTarget = me->GetLootRecipient())
                    {
                        DoScriptText(GetTextId(), me, pTarget);
                        AttackStart(pTarget);
                    }
                }
...
struct MANGOS_DLL_DECL npc_crusade_persuadedAI : public ScriptedAI
{
    ...
    {
        if (caster->GetTypeId() == TYPEID_PLAYER && me->isAlive() && spell->Id == SPELL_PERSUASIVE_STRIKE && uiSpeech_counter == 0)
        {
            if(((Player*)caster)->GetQuestStatus(12720) == QUEST_STATUS_INCOMPLETE)
            {
                if (rand()%100 > 90) // chance
                {
                    ...
                    uiCrusade_faction = me->getFaction();
                    uiSpeech_timer = 1000;
                    uiSpeech_counter = 1;
                    me->setFaction(35);
                }
    ...
    void UpdateAI(const uint32 diff) 
    {
        if (uiSpeech_counter >= 1 && uiSpeech_counter <= 6)
            if (uiSpeech_timer < diff)
            {
                me->CombatStop(true);
                me->StopMoving();
                Unit* pPlayer = Unit::GetUnit(*me, uiPlayerGUID);

                switch(uiSpeech_counter)
                {
                    case 1: DoScriptText(SAY_PERSUADED1, me); uiSpeech_timer = 8000; uiSpeech_counter++; break;
                    case 2: DoScriptText(SAY_PERSUADED2, me); uiSpeech_timer = 8000; uiSpeech_counter++; break;
                    case 3: DoScriptText(SAY_PERSUADED3, me); uiSpeech_timer = 8000; uiSpeech_counter++; break;
                    case 4: DoScriptText(SAY_PERSUADED4, me); uiSpeech_timer = 8000; uiSpeech_counter++; break;
                    case 5: DoScriptText(SAY_PERSUADED5, pPlayer); uiSpeech_timer = 8000; uiSpeech_counter++; break;
                    case 6:
                        DoScriptText(SAY_PERSUADED6, me);
                        me->setFaction(uiCrusade_faction);
                        //me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
...
struct MANGOS_DLL_DECL mob_dark_rider_of_acherusAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff) 
    {
        ...
        {
            DoCastVictim( SPELL_PLAGUE_STRIKE2);
    ...
    {
        if (Unit* pPlayer = Unit::GetUnit(*me, uiPlayerGUID))
...
struct MANGOS_DLL_DECL mob_scarlet_minerAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        if (pCaster->GetTypeId() == TYPEID_PLAYER && me->isAlive() && pSpell->Id == SPELL_GIFT_OF_THE_HARVESTER_MISSILE)
        {
            if(((Player*)pCaster)->GetQuestStatus(12698) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                Unit* pGhoul = me->SummonCreature(NPC_SCARLET_GHOUL, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 60000);
...
struct MANGOS_DLL_DECL npc_highlord_darion_mograineAI : public npc_escortAI
{
    ...
    void Reset()
    {
        if (!IsBeingEscorted)
        {
            ...
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiTirionGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiKorfaxGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiMaxwellGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiEligorGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiRayneGUID)))
                pTemp->setDeathState(JUST_DIED);
            ...
            {
                if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiDefenderGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiDefenderGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_EARTHSHATTER_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiEarthshatterGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiEarthshatterGUID[i] = 0;
            }

            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiKoltiraGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiOrbazGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiThassarianGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiLichKingGUID)))
                pTemp->setDeathState(JUST_DIED);
            ...
            {
                if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiAbominationGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiAbominationGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_BEHEMOTH_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiBehemothGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiBehemothGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_GHOUL_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiGhoulGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiGhoulGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_WARRIOR_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiWarriorGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiWarriorGUID[i] = 0;
            }
        }
    ...
    void WaypointReached(uint32 i)
    {
        switch(i)
        {
            ...
            case 2:
                me->AddSplineFlag(SPLINEFLAG_WALKMODE);
                DoCastMe( SPELL_THE_LIGHT_OF_DAWN);
                break;
            case 3:
            {
                Creature* pTirion = ((Creature*)Unit::GetUnit((*me), uiTirionGUID));
            ...
            if (uiPhase_timer < diff)
            {
                ...
                switch(uiStep)
                {
                    ...
                    case 4: // summon ghoul
                        ...
                        DoCastMe( 33271); // shack effect
                        uiPhase_timer = 500;
                        if (uiSummon_counter < ENCOUNTER_GHOUL_NUMBER)
                        {
                            if (Creature* pTemp = me->SummonCreature(NPC_ACHERUS_GHOUL, (me->GetPositionX()-20)+rand()%40, (me->GetPositionY()-20)+rand()%40, me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                            {
                    ...
                    case 5: // summon abomination
                        ...
                        {
                            if (Creature* pTemp = me->SummonCreature(NPC_RAMPAGING_ABOMINATION, (me->GetPositionX()-20)+rand()%40, (me->GetPositionY()-20)+rand()%40, me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->setFaction(2084);
                    ...
                    case 6: // summon warrior
                        ...
                        {
                            if (Creature* pTemp = me->SummonCreature(NPC_WARRIOR_OF_THE_FROZEN_WASTES, (me->GetPositionX()-20)+rand()%40, (me->GetPositionY()-20)+rand()%40, me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->setFaction(2084);
                    ...
                    case 7: // summon warrior
                        ...
                        {
                            if (Creature* pTemp = me->SummonCreature(NPC_FLESH_BEHEMOTH, (me->GetPositionX()-20)+rand()%40, (me->GetPositionY()-20)+rand()%40, me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                            {
                                pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                pTemp->setFaction(2084);
                    ...
                    case 15: // summon gate
                        if (Creature* pTemp = me->SummonCreature(NPC_HIGHLORD_ALEXANDROS_MOGRAINE, LightofDawnLoc[22].x, LightofDawnLoc[22].y, LightofDawnLoc[22].z, LightofDawnLoc[22].o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                        {
                    ...
                    case 16: // Alexandros out
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiAlexandrosGUID)))
                        {
                    ...
                    case 18: // Darion's spirit out
                        if (Creature* pTemp = me->SummonCreature(NPC_DARION_MOGRAINE, LightofDawnLoc[24].x, LightofDawnLoc[24].y, LightofDawnLoc[24].z, LightofDawnLoc[24].o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                        {
                    ...
                    case 19: // runs to father
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiDarionGUID)))
                        {
                    ...
                    case 24:
                        ...
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiTirionGUID))) // Tirion moves forward here
                    ...
                    case 29: // lich king spawns
                        if (Creature* pTemp = me->SummonCreature(NPC_THE_LICH_KING, LightofDawnLoc[26].x, LightofDawnLoc[26].y, LightofDawnLoc[26].z, LightofDawnLoc[26].o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                        {
                            DoScriptText(SAY_LIGHT_OF_DAWN43, pTemp);
                            uiLichKingGUID = pTemp->GetGUID();
                            if (Creature* pAlex = ((Creature*)Unit::GetUnit((*me), uiAlexandrosGUID)))
                    ...
                    case 30:
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiAlexandrosGUID))) // remove him
                        {
...
// disable              if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiLichKingGUID)))
//  because mangos          DoCastMe( SPELL_MOGRAINE_CHARGE); // jumping charge
//   doesn't make it looks well, so workarounds, Darion charges, looks better
                    ...
                    case 41: // Lich king - Apocalypse
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiLichKingGUID)))
                        {
                            ...
                            if (Creature* pTirion = ((Creature*)Unit::GetUnit((*me), uiTirionGUID)))
                            {
                    ...
                    case 42: // Maxwell yells for attack
                        {
                            ...
                            {
                                Creature* pTemp;
                                if (pTemp = me->SummonCreature(NPC_DEFENDER_OF_THE_LIGHT, LightofDawnLoc[0].x+rand()%10, LightofDawnLoc[0].y+rand()%10, LightofDawnLoc[0].z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000))
                                {
                                ...
                                if (pTemp = me->SummonCreature(NPC_RIMBLAT_EARTHSHATTER, LightofDawnLoc[0].x+rand()%10, LightofDawnLoc[0].y+rand()%10, LightofDawnLoc[0].z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000))
                                {
                                    pTemp->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_ATTACK_UNARMED);
                                    pTemp->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                                    pTemp->SetSpeedRate(MOVE_RUN, 2.0f);
                    ...
                    case 43: // They all got kicked
                        ...
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiDefenderGUID[0])))
                        {
                            pTemp->SetSpeedRate(MOVE_RUN, 6.0f);
                            ((Unit*)pTemp)->SetStandState(UNIT_STAND_STATE_DEAD);
                            pTemp->GetMotionMaster()->MovePoint(0, LightofDawnLoc[0].x+rand()%10, LightofDawnLoc[0].y+rand()%10, LightofDawnLoc[0].z);
                        }
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiEarthshatterGUID[0])))
                        {
                            pTemp->SetSpeedRate(MOVE_RUN, 6.0f);
                            ((Unit*)pTemp)->SetStandState(UNIT_STAND_STATE_DEAD);
                            pTemp->GetMotionMaster()->MovePoint(0, LightofDawnLoc[0].x+rand()%10, LightofDawnLoc[0].y+rand()%10, LightofDawnLoc[0].z);
                    ...
                    case 59:
                        ...
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiTirionGUID))) // Tirion runs to Darion
                        {
                            pTemp->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_NONE);
                            pTemp->SetSpeedRate(MOVE_RUN, 1.0f);
                    ...
                    case 60:
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), uiLichKingGUID))) // Lich king disappears here
                        {
        ...
        else
        {
            ...
            {
                DoCastMe( SPELL_ANTI_MAGIC_ZONE1);
            ...
            {
                if (!uiTirionGUID)
                    if (Creature* pTemp = me->SummonCreature(NPC_HIGHLORD_TIRION_FORDRING, LightofDawnLoc[0].x, LightofDawnLoc[0].y, LightofDawnLoc[0].z, 1.528, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 600000))
                    {
                        pTemp->setFaction(me->getFaction());
    ...
    {
        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), ui_GUID)))
            if (pTemp->isAlive())
    ...
    void SpawnNPC()
    {
        ...
        for(uint8 i = 0; i < ENCOUNTER_GHOUL_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiGhoulGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_ABOMINATION_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiAbominationGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_WARRIOR_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiWarriorGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_BEHEMOTH_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiBehemothGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_DEFENDER_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiDefenderGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_EARTHSHATTER_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiEarthshatterGUID[i]))))
            {
        ...
        if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiKorfaxGUID))))
        {
        ...
        if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiMaxwellGUID))))
        {
        ...
        if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiEligorGUID))))
        {
        ...
        if (!(pTemp = ((Creature*)Unit::GetUnit((*me), uiRayneGUID))))
        {
    ...
    {
        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), pGUID)))
            if (pTemp->isAlive())
            {
