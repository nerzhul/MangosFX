...
struct MANGOS_DLL_DECL boss_vexallusAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (pInstance)
            SetInstanceData(DATA_VEXALLUS_EVENT, NOT_STARTED);
    }
    ...
    {
        if (pInstance)
            SetInstanceData(DATA_VEXALLUS_EVENT, DONE);
    }
    ...
    void Aggro(Unit *who)
    {
        ...
        if (pInstance)
            SetInstanceData(DATA_VEXALLUS_EVENT, IN_PROGRESS);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!Enraged)
        {
            ...
            if ((me->GetHealth()*100 / me->GetMaxHealth()) <= (100-(INTERVAL_MODIFIER*IntervalHealthAmount)))
            {
                ...
                if (m_bIsHeroic)
                {
                    me->CastSpell(me,H_SPELL_SUMMON_PURE_ENERGY1,false);
                    me->CastSpell(me,H_SPELL_SUMMON_PURE_ENERGY2,false);
                }
                else
                    me->CastSpell(me,SPELL_SUMMON_PURE_ENERGY,false);
            ...
            if (ChainLightningTimer < diff)
            {
                if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0))
                    DoCast(target, m_bIsHeroic ? SPELL_H_CHAIN_LIGHTNING : SPELL_CHAIN_LIGHTNING);
            ...
            {
                if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0))
                    DoCast(target, m_bIsHeroic ? SPELL_H_ARCANE_SHOCK : SPELL_ARCANE_SHOCK);
