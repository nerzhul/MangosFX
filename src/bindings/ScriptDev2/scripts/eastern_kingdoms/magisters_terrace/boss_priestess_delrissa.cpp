...
struct MANGOS_DLL_DECL boss_priestess_delrissaAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (RenewTimer < diff)
        {
            ...
            DoCast(target, m_bIsHeroic ? SPELL_RENEW_HEROIC : SPELL_RENEW_NORMAL);
            RenewTimer = 5000;
        ...
        if (SWPainTimer < diff)
        {
            if (Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0))
                DoCast(pTarget, m_bIsHeroic ? SPELL_SW_PAIN_HEROIC : SPELL_SW_PAIN_NORMAL);
...
struct MANGOS_DLL_DECL boss_yazzaiAI : public boss_priestess_lackey_commonAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Blink_Timer < diff)
        {
           ...
           ThreatList const& t_list = me->getThreatManager().getThreatList();
...
struct MANGOS_DLL_DECL boss_apokoAI : public boss_priestess_lackey_commonAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Totem_Timer < diff)
        {
            switch(urand(0, 2))
            {
                case 0: DoCastMe( SPELL_WINDFURY_TOTEM); break;
                case 1: DoCastMe( SPELL_FIRE_NOVA_TOTEM); break;
                case 2: DoCastMe( SPELL_EARTHBIND_TOTEM); break;
            }
            ++Totem_Amount;
        ...
        if (Healing_Wave_Timer < diff)
        {
            ...
            //   {
            DoCastMe( SPELL_LESSER_HEALING_WAVE);
            Healing_Wave_Timer = 5000;
            //    }
            // }
...
struct MANGOS_DLL_DECL boss_zelfanAI : public boss_priestess_lackey_commonAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            DoCastMe( SPELL_HIGH_EXPLOSIVE_SHEEP);
