...
struct MANGOS_DLL_DECL npc_kelerun_bloodmournAI : public ScriptedAI
{
    ...
    void DoSpawnChallengers()
    {
        for(uint8 i = 0; i < MAX_CHALLENGER; ++i)
        {
            if (Creature* pCreature = me->SummonCreature(uiChallengerId[i],
    ...
    void UpdateAI(const uint32 diff)
    {
        if (m_bIsEventInProgress)
        {
            ...
            {
                if (Unit* pChallenger = Unit::GetUnit(*me,uiChallengerGUID[m_uiChallengerCount]))
                {
                    if (!pChallenger->isAlive())
                    {
                        ...
                        {
                            if (pPlayer && pPlayer->isAlive())
                                pPlayer->GroupEventHappens(QUEST_SECOND_TRIAL,me);
            ...
            {
                Unit* pPlayer = Unit::GetUnit(*me,m_uiPlayerGUID);

                if (pPlayer && pPlayer->isAlive())
                {
                    if (Creature* pCreature = (Creature*)Unit::GetUnit(*me,uiChallengerGUID[m_uiChallengerCount]))
                    {
                        DoScriptText(uiSayId[m_uiChallengerCount], me, pPlayer);
