    ...
    void WaypointStart(uint32 uiPointId)
    {
        switch(uiPointId)
        {
            case 27:
                ...
                {
                    float fX, fY, fZ;
                    me->GetRandomPoint(147.927444f, -3851.513428f, 130.893f, 7.0f, fX, fY, fZ);

                    me->SummonCreature(NPC_MARAUDING_OWL, fX, fY, fZ, 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 25000);
                }
                break;
            case 44:
                for(uint8 i = 0; i < 3; ++i)
                {
                    float fX, fY, fZ;
                    me->GetRandomPoint(-141.151581f, -4291.213867f, 120.130f, 7.0f, fX, fY, fZ);

                    me->SummonCreature(NPC_VILE_AMBUSHER, fX, fY, fZ, 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 25000);
                }
                break;
        }
    }
...
struct MANGOS_DLL_DECL npc_rinjiAI : public npc_escortAI
{
    ...
    void Aggro(Unit* pWho)
    {
        if (HasEscortState(STATE_ESCORT_ESCORTING))
        {
            ...
            DoScriptText(urand(0, 1) ? SAY_RIN_HELP_1 : SAY_RIN_HELP_2, me);
        }
    }
