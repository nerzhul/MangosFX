...
struct MANGOS_DLL_DECL boss_moroesAI : public ScriptedAI
{
    ...
    {
        Unit* pUnit = Unit::GetUnit((*me), guid);
        if (pUnit)
        {
            if (!pUnit->isAlive())
    ...
    void SpawnAdds()
    {
        ...
        else
        {
            for(uint8 i = 0; i < 4; ++i)
            {
                ...
                {
                    case 0:
                        pCreature = me->SummonCreature(AddId[i], Locations[i][0], Locations[i][1], POS_Z, Locations[i][2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
                        if (pCreature)
                            AddGUID[i] = pCreature->GetGUID();
                        break;
                    case 1:
                        pCreature = ((Creature*)Unit::GetUnit((*me), AddGUID[i]));
                        if (pCreature)
                        {
                            pCreature->Respawn();
    ...
    void DeSpawnAdds()
    {
        for(uint8 i = 0; i < 4 ; ++i)
        {
            ...
            if (AddGUID[i])
            {
                Temp = Unit::GetUnit((*me),AddGUID[i]);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!Enrage)
        {
            ...
            if (InVanish)
            {
                if (Wait_Timer < diff)
                {
                    DoScriptText(urand(0, 1) ? SAY_SPECIAL_1 : SAY_SPECIAL_2, me);
...
struct MANGOS_DLL_DECL boss_baroness_dorothea_millstipeAI : public boss_moroes_guestAI
{
    ...
    void Reset()
    {
        ...
        DoCastMe(SPELL_SHADOWFORM, true);
