...
struct MANGOS_DLL_DECL boss_murmurAI : public ScriptedAI
{
    ...
    void SonicBoomEffect()
    {
        ...
        for(std::list<HostileReference *>::iterator itr = t_list.begin(); itr!= t_list.end(); ++itr)
        {
           ...
           if (target && target->GetTypeId() == TYPEID_PLAYER)
           {
               ...
               if (target->HasAura(SPELL_SONIC_BOOM_CAST,1) && me->IsWithinDistInMap(target, 34.0f))
               {
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (SonicBoom_Timer < diff)
        {
            if (CanSonicBoom)
            {
                DoCastMe( SPELL_SONIC_BOOM_CAST,true);
                SonicBoomEffect();
        ...
        if (MurmursTouch_Timer < diff)
        {
            ...
            DoCastMe( SPELL_MURMURS_TOUCH);
            MurmursTouch_Timer = urand(25000, 35000);
        }else MurmursTouch_Timer -= diff;

        //Resonance_Timer
        if (!CanSonicBoom && !me->IsWithinDistInMap(me->getVictim(), ATTACK_DISTANCE))
        {
            if (Resonance_Timer < diff)
            {
                DoCastVictim( SPELL_RESONANCE);
                Resonance_Timer = m_bIsHeroic ? 3000 : 5000;
        ...
        if (m_bIsHeroic)
        {
            ...
            {
                DoCastVictim( SPELL_THUNDERING_STORM);
        ...
        if (MagneticPull_Timer < diff)
        {
            ...
            {
                if (Unit* target = Unit::GetUnit(*me,pTarget))
