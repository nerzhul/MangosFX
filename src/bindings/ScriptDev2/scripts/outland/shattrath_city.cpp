...
struct MANGOS_DLL_DECL npc_dirty_larryAI : public ScriptedAI
{
    ...
    {
        Creature* pCreature = (Creature*)Unit::GetUnit(*me, guid);
        ...
        else
        {
            ...
            if (bAttack)
            {
                if (Unit* pUnit = Unit::GetUnit(*me, m_uiPlayerGUID))
                {
                    if (pUnit->isAlive())
    ...
    void StartEvent()
    {
        ...
        if (Creature* pCreepjack = GetClosestCreatureWithEntry(me, ENTRY_CREEPJACK, 20.0f))
            m_uiCreepjackGUID = pCreepjack->GetGUID();

        if (Creature* pMalone = GetClosestCreatureWithEntry(me, ENTRY_MALONE, 20.0f))
    ...
    {
        Unit* pUnit = Unit::GetUnit(*me, m_uiPlayerGUID);
        ...
        {
            case 1:
                DoScriptText(SAY_START, me, pUnit);
                SetRuffies(m_uiCreepjackGUID,false,false);
                SetRuffies(m_uiMaloneGUID,false,false);
                return 3000;
            case 2: DoScriptText(SAY_COUNT, me, pUnit); return 5000;
            case 3: DoScriptText(SAY_COUNT_1, me, pUnit); return 3000;
            case 4: DoScriptText(SAY_COUNT_2, me, pUnit); return 3000;
            case 5: DoScriptText(SAY_ATTACK, me, pUnit); return 3000;
            case 6:
                if (!me->isInCombat() && pUnit->isAlive())
                    AttackStart(pUnit);
    ...
    void DamageTaken(Unit* pDoneBy, uint32 &damage)
    {
        ...
        if (bEvent)
        {
            ...
            {
                DoScriptText(SAY_GIVEUP, me, pPlayer);
                pPlayer->GroupEventHappens(QUEST_WHAT_BOOK, me);
            }
...
struct MANGOS_DLL_DECL npc_khadgars_servantAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId)
    {
        ...
        {
            case 0:
                if (Creature* pKhadgar = GetClosestCreatureWithEntry(me, NPC_KHADGAR, 10.0f))
            ...
            case 75:
                ...
                break;
            case 34:
                if (Creature* pIzzard = GetClosestCreatureWithEntry(me, NPC_IZZARD, 10.0f))
                    DoScriptText(SAY_KHAD_MIND_YOU, pIzzard);
                break;
            case 35:
                if (Creature* pAdyria = GetClosestCreatureWithEntry(me, NPC_ADYRIA, 10.0f))
            ...
            if (m_uiTalkTimer <= diff)
            {
                ...
                switch(m_uiPointId)
                {
                    ...
                    case 24:                                //in lower city
                    {
                        ...
                        {
                            case 5:
                                if (Creature* pShanir = GetClosestCreatureWithEntry(me, NPC_SHANIR, 15.0f))
