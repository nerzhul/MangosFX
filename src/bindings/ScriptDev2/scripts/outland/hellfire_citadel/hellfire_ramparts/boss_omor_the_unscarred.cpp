    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (SummonedCount < 2)
        {
            if (Summon_Timer < diff)
            {
                ...
                DoCastMe(SPELL_SUMMON_FIENDISH_HOUND);
        ...
        if (Aura_Timer < diff)
        {
            ...
            {
                DoCast(target, m_bIsHeroic ? H_SPELL_BANE_OF_TREACHERY : SPELL_TREACHEROUS_AURA);
        ...
        if (Shadowbolt_Timer < diff)
        {
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
            {
                ...
                DoCast(target, m_bIsHeroic ? H_SPELL_SHADOW_BOLT : SPELL_SHADOW_BOLT);
