...
struct MANGOS_DLL_DECL npc_daranelleAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWho)
    {
        ...
        {
            if (pWho->HasAura(SPELL_LASHHAN_CHANNEL,0) && me->IsWithinDistInMap(pWho, 10.0f))
            {
                DoScriptText(SAY_SPELL_INFLUENCE, me, pWho);

                //TODO: Move the below to updateAI and run if this statement == true
                ((Player*)pWho)->KilledMonsterCredit(NPC_KALIRI_AURA_DISPEL, me->GetGUID());
