...
struct MANGOS_DLL_DECL mob_lumpAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit *done_by, uint32 & damage)
    {
        if (done_by->GetTypeId() == TYPEID_PLAYER && (me->GetHealth() - damage)*100 / me->GetMaxHealth() < 30)
        {
            if (!bReset && ((Player*)done_by)->GetQuestStatus(9918) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                DoScriptText(SAY_LUMP_DEFEAT, me, done_by);
    ...
    void Aggro(Unit *who)
    {
        ...
        DoScriptText(urand(0, 1) ? SAY_LUMP_0 : SAY_LUMP_1, me, who);
    }
...
struct MANGOS_DLL_DECL npc_creditmarker_visit_with_ancestorsAI : public ScriptedAI
{
    ...
    {
        if (pWho->GetTypeId() == TYPEID_PLAYER && me->IsWithinDistInMap(pWho, 30.0f))
        {
            if (((Player*)pWho)->GetQuestStatus(QUEST_VISIT_WITH_ANCESTORS) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                if ((creditMarkerId >= 18840) && (creditMarkerId <= 18843))
                {
                    ...
                    if (!((Player*)pWho)->GetReqKillOrCastCurrentCount(QUEST_VISIT_WITH_ANCESTORS, creditMarkerId))
                        ((Player*)pWho)->KilledMonsterCredit(creditMarkerId, me->GetGUID());
                }
            }
        }
    }
