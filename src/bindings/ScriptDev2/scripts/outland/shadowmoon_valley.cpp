    ...
    void UpdateAI(const uint32 diff)
    {
        if (bCanEat || bIsEating)
        {
            if (EatTimer < diff)
            {
                ...
                {
                    DoCastMe( SPELL_JUST_EATEN);
                    DoScriptText(SAY_JUST_EATEN, me);

                    if (Player* pPlr = (Player*)Unit::GetUnit((*me), uiPlayerGUID))
...
struct MANGOS_DLL_DECL mob_enslaved_netherwing_drakeAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* caster, const SpellEntry* spell)
    {
        ...
        if (caster->GetTypeId() == TYPEID_PLAYER && spell->Id == SPELL_HIT_FORCE_OF_NELTHARAKU && !Tapped)
        {
            ...
            if (Creature* Dragonmaw = GetClosestCreatureWithEntry(me, NPC_DRAGONMAW_SUBJUGATOR, 50.0f))
            {
    ...
    void UpdateAI(const uint32 diff)
    {
        if (!CanDoSomething())
        {
            if (Tapped)
            {
                ...
                {
                    Tapped = false;
                    if (Player* pPlayer = (Player*)Unit::GetUnit(*me, PlayerGUID))
                    {
                        if (pPlayer->GetQuestStatus(QUEST_FORCE_OF_NELT) == QUEST_STATUS_INCOMPLETE)
                        {
                            pPlayer->KilledMonsterCredit(NPC_ENSLAVED_DRAKE_KILL_CREDIT, me->GetGUID());
                            ...
                            if (Creature* EscapeDummy = GetClosestCreatureWithEntry(me, NPC_ESCAPE_DUMMY, 30.0f))
...
struct MANGOS_DLL_DECL mob_dragonmaw_peonAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        ...
        if (pCaster->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_SERVING_MUTTON && !m_bIsTapped)
        {
            ...
            float fX, fY, fZ;
			pCaster->GetClosePoint(fX, fY, fZ, me->GetObjectBoundingRadius());

            me->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
            me->GetMotionMaster()->MovePoint(POINT_DEST, fX, fY, fZ);
        }
    }
            ...
            if (m_uiPoisonTimer <= diff)
            {
                if (Player* pPlayer = (Player*)Unit::GetUnit(*me, m_uiPlayerGUID))
                {
                    if (pPlayer->GetQuestStatus(QUEST_SLOW_DEATH) == QUEST_STATUS_INCOMPLETE)
                        pPlayer->KilledMonsterCredit(NPC_DRAGONMAW_KILL_CREDIT, me->GetGUID());
                }
...
struct MANGOS_DLL_DECL npc_wildaAI : public npc_escortAI
{
    ...
    void DoRandomSay()
    {
        switch(urand(0, 2))
        {
            case 0: DoScriptText(SAY_WIL_PROGRESS2, me); break;
            case 1: DoScriptText(SAY_WIL_PROGRESS4, me); break;
            case 2: DoScriptText(SAY_WIL_PROGRESS5, me); break;
        }
    }
    ...
    void DoSpawnAssassin()
    {
        ...
        float fX, fY, fZ;
        me->GetRandomPoint(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 15.0f, fX, fY, fZ);

        me->SummonCreature(NPC_COILSKAR_ASSASSIN, fX, fY, fZ, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
    }
    ...
    void Aggro(Unit* pWho)
    {
        ...
        if (pWho->GetTypeId() != TYPEID_PLAYER)
        {
            ...
            switch(urand(0, 3))
            {
                case 0: DoScriptText(SAY_WIL_AGGRO1, me, pWho); break;
                case 1: DoScriptText(SAY_WIL_AGGRO2, me, pWho); break;
            }
        }
    }
...
struct MANGOS_DLL_DECL mob_torlothAI : public ScriptedAI
{
    ...
    void HandleAnimation()
    {
        ...
        {
            pCreature = ((Creature*)Unit::GetUnit(*me, m_uiLordIllidanGUID));
        ...
        switch(m_uiAnimationCount)
        {
            ...
            case 3:
                ...
                break;
            case 5:
                if (Unit* pTarget = Unit::GetUnit((*me), m_uiPlayerGUID))
                {
    ...
    void JustDied(Unit* pKiller)
    {
        if (Player* pPlayer = pKiller->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            ...
            if (Creature* pLordIllidan = ((Creature*)Unit::GetUnit(*me, m_uiLordIllidanGUID)))
            {
