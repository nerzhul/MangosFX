...
struct MANGOS_DLL_DECL npc_manaforge_control_consoleAI : public ScriptedAI
{
    ...
    void JustDied(Unit* killer)
    {
        ...
        if (goConsole)
        {
            if (GameObject* pGo = me->GetMap()->GetGameObject(goConsole))
    ...
    void UpdateAI(const uint32 diff)
    {
        if (Event_Timer < diff)
        {
            ...
            if (someplayer)
            {
                playertarget = Unit::GetUnit((*me),someplayer);
                if (!playertarget)
                    return;
            ...
            {
                case 1:
                    DoScriptText(EMOTE_START, me, playertarget);
                    Event_Timer = 60000;
                    Wave = true;
                    ++Phase;
                    break;
                case 2:
                    DoScriptText(EMOTE_60, me, playertarget);
                    Event_Timer = 30000;
                    ++Phase;
                    break;
                case 3:
                    DoScriptText(EMOTE_30, me, playertarget);
                    Event_Timer = 20000;
                    DoFinalSpawnForCreature(me);
                    ++Phase;
                    break;
                case 4:
                    DoScriptText(EMOTE_10, me, playertarget);
                    Event_Timer = 10000;
                    Wave = false;
                    ++Phase;
                    break;
                case 5:
                    DoScriptText(EMOTE_COMPLETE, me, playertarget);
                    ((Player*)playertarget)->KilledMonsterCredit(me->GetEntry(),me->GetGUID());
                    DoCastMe(SPELL_DISABLE_VISUAL);
                    if (goConsole)
                    {
                        if (GameObject* pGo = me->GetMap()->GetGameObject(goConsole))
                            pGo->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
...
struct MANGOS_DLL_DECL npc_commander_dawnforgeAI : public ScriptedAI
{
    ...
    void Turn_to_Pathaleons_Image()
    {
        ...
        me->SendCreateUpdateToPlayer(pPlayer);
    ...
    bool CanStartEvent(Player* pPlayer)
    {
        if (!isEvent)
        {
            Creature* ardonis = GetClosestCreatureWithEntry(me, uiCreatureEntry[0], 10.0f);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        switch (Phase)
        {
            ...
            case 4:
                ...
                me->SummonCreature(uiCreatureEntry[2], 2325.851563, 2799.534668, 133.084229, 6.038996, TEMPSUMMON_TIMED_DESPAWN, 90000);
                ++Phase;
                Phase_Timer = 500;
                break;
