...
struct MANGOS_DLL_DECL boss_harbinger_skyrissAI : public ScriptedAI
{
    ...
    void DoSplit()
    {
        ...
        DoCastMe( IsImage33 ? SPELL_33_ILLUSION : SPELL_66_ILLUSION);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (MindRend_Timer < diff)
        {
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,1))
                DoCast(target, m_bIsHeroic ? H_SPELL_MIND_REND : SPELL_MIND_REND);
            else
                DoCastVictim( m_bIsHeroic ? H_SPELL_MIND_REND : SPELL_MIND_REND);
        ...
        if (Fear_Timer < diff)
        {
            ...
            DoScriptText(urand(0, 1) ? SAY_FEAR_1 : SAY_FEAR_2, me);
        ...
        if (Domination_Timer < diff)
        {
            ...
            DoScriptText(urand(0, 1) ? SAY_MIND_1 : SAY_MIND_2, me);

            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,1))
                DoCast(target, m_bIsHeroic ? H_SPELL_DOMINATION : SPELL_DOMINATION);
            else
                DoCastVictim( m_bIsHeroic ? H_SPELL_DOMINATION : SPELL_DOMINATION);
