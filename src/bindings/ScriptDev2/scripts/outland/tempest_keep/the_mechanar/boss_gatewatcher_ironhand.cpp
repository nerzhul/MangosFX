...
struct MANGOS_DLL_DECL boss_gatewatcher_iron_handAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            DoCastMe( m_bIsHeroic ? H_SPELL_SHADOW_POWER : SPELL_SHADOW_POWER);
        ...
        if (Jackhammer_Timer < diff)
        {
            ...
            DoCastVictim( m_bIsHeroic ? H_SPELL_JACKHAMMER : SPELL_JACKHAMMER);

            //chance to yell, but not same time as emote (after spell in fact casted)
            if (urand(0, 4))
                DoScriptText(urand(0, 1) ? SAY_HAMMER_1 : SAY_HAMMER_2, me);
