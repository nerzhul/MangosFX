...
struct MANGOS_DLL_DECL advisorbase_ai : public ScriptedAI
{
    ...
    {
        if (!pWho || m_bFakeDeath || me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE))
            return;
    ...
    void Revive(Unit* Target)
    {
        ...
        DoCastMe( SPELL_RES_VISUAL, false);
    ...
    void UpdateAI(const uint32 diff)
    {
        if (m_uiDelayRes_Timer)
        {
            if (m_uiDelayRes_Timer <= diff)
            {
                ...
                Unit* pTarget = Unit::GetUnit((*me), m_uiDelayRes_Target);
                if (!pTarget)
...
struct MANGOS_DLL_DECL boss_kaelthasAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWho)
    {
            ...
            me->IsHostileTo(pWho) && pWho->isInAccessablePlaceFor(me))
        {
            ...
            {
                if (!me->getVictim() && m_uiPhase >= PHASE_4_SOLO)
                {
    ...
    void JustSummoned(Creature* pSummoned)
    {
        ...
        {
            pSummoned->CastSpell(pSummoned, SPELL_FLAME_STRIKE_DUMMY, false, NULL, NULL, me->GetGUID());
            return;
        }
        ...
        switch (m_uiPhase)
        {
                    ...
                    case 1:
                        if (m_uiPhase_Timer < diff)
                        {
                            ...
                            if (pAdvisor)
                            {
                                ...
                                pAdvisor->setFaction(me->getFaction());
            ...
            case PHASE_3_ADVISOR_ALL:
            {
                if (m_uiPhaseSubphase == 0)
                {
                    ...
                    {
                        pAdvisor = (Creature*)(Unit::GetUnit((*me), m_auiAdvisorGuid[i]));
            ...
            case 6:
            {
                    ...
                    if (m_uiFireball_Timer < diff)
                    {
                        if (!m_bIsCastingFireball)
                        {
                            if (!me->IsNonMeleeSpellCasted(false))
                            {
                                ...
                                me->CastCustomSpell(me->getVictim(), SPELL_FIREBALL, &uiDmg, 0, 0, false);
                ...
                if (m_uiPhoenix_Timer < diff)
                {
                    ...
                    DoScriptText(urand(0, 1) ? SAY_SUMMON_PHOENIX1 : SAY_SUMMON_PHOENIX2, me);
                ...
                if (m_uiPhase == PHASE_4_SOLO)
                {
                    if (me->GetHealth()*100 / me->GetMaxHealth() < 50)
                    {
                        ...
                        me->GetMap()->CreatureRelocation(me, afGravityPos[0], afGravityPos[1], afGravityPos[2], 0.0f);
                        me->SendMonsterMove(afGravityPos[0], afGravityPos[1], afGravityPos[2], SPLINETYPE_NORMAL, SPLINEFLAG_WALKMODE, 0);

                        me->InterruptNonMeleeSpells(false);
                        DoCastMe( SPELL_FULLPOWER);
                        me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    }
                ...
                if (m_uiPhase == 6)
                {
                    ...
                    if (m_uiGravityLapse_Timer < diff)
                    {
                        ...
                        switch(m_uiGravityLapse_Phase)
                        {
                            ...
                            case 1:
                                DoScriptText(urand(0, 1) ? SAY_GRAVITYLAPSE1 : SAY_GRAVITYLAPSE2, me);
...
struct MANGOS_DLL_DECL boss_grand_astromancer_capernianAI : public advisorbase_ai
{
    ...
    void AttackStart(Unit* pWho)
    {
        ...
        if (me->Attack(pWho, true))
        {
            ...
            me->GetMotionMaster()->MoveChase(pWho, CAPERNIAN_DISTANCE);
        }
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiConflagration_Timer < diff)
        {
            ...
            if (pTarget && me->IsWithinDistInMap(pTarget, 30.0f))
        ...
        if (m_uiArcaneExplosion_Timer < diff)
        {
           ...
           ThreatList const& m_threatlist = me->getThreatManager().getThreatList();
            for (ThreatList::const_iterator i = m_threatlist.begin(); i!= m_threatlist.end();++i)
            {
                ...
                //if in melee range
                if (pUnit && pUnit->IsWithinDistInMap(me, ATTACK_DISTANCE))
                {
...
struct MANGOS_DLL_DECL mob_phoenix_tkAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiCycle_Timer < diff)
        {
            ...
            if (me->GetHealth() > uiDmg)
                me->SetHealth(uint32(me->GetHealth()-uiDmg));
...
struct MANGOS_DLL_DECL mob_phoenix_egg_tkAI : public ScriptedAI
{
    ...
    void JustSummoned(Creature* pSummoned)
    {
        pSummoned->AddThreat(me->getVictim(), 0.0f);
