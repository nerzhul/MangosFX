...
struct MANGOS_DLL_DECL mob_treantAI  : public ScriptedAI
{
    ...
    void Reset()
    {
        me->SetSpeedRate(MOVE_RUN, 0.5f, true);
    }
...
struct MANGOS_DLL_DECL boss_warp_splinterAI : public ScriptedAI
{
    ...
    void SummonTreants()
    {
        for(int i = 0; i < 6; ++i)
        {
            ...
            //float Z = me->GetMap()->GetHeight(X,Y, me->GetPositionZ());
            //float Z = me->GetPositionZ();
            float O = - me->GetAngle(X,Y);

            Creature* pTreant = me->SummonCreature(CREATURE_TREANT,treant_pos[i][0],treant_pos[i][1],treant_pos[i][2],O,TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,40000);
            if (pTreant)
            {
                //pTreant->GetMotionMaster()->Mutate(new TargetedMovementGenerator<Creature>(*me));
                pTreant->AddThreat(me, 0.1f);
                Treant_GUIDs[i] = pTreant->GetGUID();
                ((mob_treantAI*)pTreant->AI())->WarpGuid = me->GetGUID();
            }
        }

        DoScriptText(urand(0, 1) ? SAY_SUMMON_1 : SAY_SUMMON_2, me);
    }
    ...
    void EatTreant()
    {
        for(int i=0; i<6; ++i)
        {
            Unit *pTreant = Unit::GetUnit(*me, Treant_GUIDs[i]);

            if (pTreant)
            {
                if (me->IsWithinDistInMap(pTreant, 5))
                {
                    // 2) Heal Warp Splinter
                    int32 CurrentHP_Treant = (int32)pTreant->GetHealth();
                    me->CastCustomSpell(me,SPELL_HEAL_FATHER,&CurrentHP_Treant, 0, 0, true,0 ,0, me->GetGUID());
