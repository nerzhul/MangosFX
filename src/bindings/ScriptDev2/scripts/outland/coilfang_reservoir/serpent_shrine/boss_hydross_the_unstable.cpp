...
enum
{
    ...
    SPELL_PURIFY_ELEMENTAL      = 36461,                    //(not in use) visualize the line by tainting/purifying mobs
...
struct MANGOS_DLL_DECL boss_hydross_the_unstableAI : public ScriptedAI
{
    ...
    {
        if (m_bCorruptedForm)
            DoScriptText(urand(0,1) ? SAY_CORRUPT_SLAY1 : SAY_CORRUPT_SLAY2, me);
        else
            DoScriptText(urand(0,1) ? SAY_CLEAN_SLAY1 : SAY_CLEAN_SLAY2, me);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_bCorruptedForm)
        {
            ...
            if (m_uiPosCheck_Timer < uiDiff)
            {
                ...
                if (me->IsWithinDist2d(fPosX, fPosY, SWITCH_RADIUS))
                {
