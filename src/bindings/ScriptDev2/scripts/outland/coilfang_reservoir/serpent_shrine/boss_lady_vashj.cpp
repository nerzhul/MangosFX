...
struct MANGOS_DLL_DECL boss_lady_vashjAI : public ScriptedAI
{
    ...
    void RemoveAllShieldGenerators()
    {
        ...
        {
            if (Unit* pTemp = Unit::GetUnit(*me,m_auiShieldGeneratorChannel[i]))
            {
                if (pTemp->isAlive())
    ...
    void MovementInform(uint32 uiType, uint32 uiPointId)
    {
        ...
        if (uiPointId == POINT_MOVE_CENTER)
        {
            ...
            for(uint8 i = 0; i < MAX_SHIELD_GEN; ++i)
            {
                if (Creature* pCreature = me->SummonCreature(NPC_SHIELD_GENERATOR, afShieldGeneratorChannelPos[i][0],  afShieldGeneratorChannelPos[i][1],  afShieldGeneratorChannelPos[i][2],  afShieldGeneratorChannelPos[i][3], TEMPSUMMON_CORPSE_DESPAWN, 0))
    ...
    void JustSummoned(Creature* pSummoned)
    {
        ...
        if (uiEntry == NPC_SHIELD_GENERATOR)
        {
            ...
            pSummoned->CastSpell(me,SPELL_MAGIC_BARRIER,true);
        }
    }

    //called when any summoned (by me) despawns
    ...
    void CastShootOrMultishot()
    {
        ...
        DoCastVictim( urand(0,1) ? SPELL_SHOOT : SPELL_MULTI_SHOT);
    ...
    void UpdateAI(const uint32 diff)
    {
            ...
            if (m_uiPhase == PHASE_1)
            {
                ...
                if ((me->GetHealth()*100 / me->GetMaxHealth()) <= 70)
                {
                    ...
                    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == CHASE_MOTION_TYPE)
                    {
                        ...
                        me->GetMotionMaster()->MovePoint(POINT_MOVE_CENTER, afMiddlePos[0], afMiddlePos[1], afMiddlePos[2]);
                    }
        ...
        else
        {
            ...
            if (m_uiTaintedElemental_Timer < diff)
            {
                ...
                me->SummonCreature(NPC_TAINTED_ELEMENTAL,
            ...
            if (m_uiCoilfangElite_Timer < diff)
            {
                ...
                me->SummonCreature(NPC_COILFANG_ELITE,
            ...
            if (m_uiCoilfangStrider_Timer < diff)
            {
                ...
                me->SummonCreature(NPC_COILFANG_STRIDER,
            ...
            if (Unit* pVashj = Unit::GetUnit((*me), pInstance->GetData64(DATA_LADYVASHJ)))
            {
                if (pVashj->IsWithinDistInMap(me, ATTACK_DISTANCE))
                {
                    ...
                    if (pVashj->isAlive() && pVashj->isInCombat())
                        me->CastSpell(pVashj, SPELL_SURGE, false, 0, 0, pVashj->GetGUID());
                    else
...
struct MANGOS_DLL_DECL mob_tainted_elementalAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiPoisonBolt_Timer < diff)
        {
            ...
            if (pTarget && pTarget->IsWithinDistInMap(me, 30.0f))
