...
struct MANGOS_DLL_DECL mob_illidari_councilAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        for(uint8 i = 0; i < 4; ++i)
        {
            if (pMember = ((Creature*)Unit::GetUnit((*me), Council[i])))
            {
                if (!pMember->isAlive())
                {
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (EndEventTimer)
        {
            if (EndEventTimer <= diff)
            {
                ...
                Creature* pMember = ((Creature*)Unit::GetUnit(*me, Council[DeathCount]));
        ...
        if (CheckTimer)
        {
            if (CheckTimer <= diff)
            {
                ...
                for(uint8 i = 0; i < 4; ++i)
                {
                    if (Council[i])
                    {
                        if (Creature* Member = ((Creature*)Unit::GetUnit((*me), Council[i])))
                        {
...
struct MANGOS_DLL_DECL boss_gathios_the_shattererAI : public boss_illidari_councilAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (SealTimer < diff)
        {
            DoCastMe( urand(0, 1) ? SPELL_SEAL_OF_COMMAND : SPELL_SEAL_OF_BLOOD);
            SealTimer = 40000;
        }else SealTimer -= diff;
...
struct MANGOS_DLL_DECL boss_lady_malandeAI : public boss_illidari_councilAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (CircleOfHealingTimer < diff)
        {
            ...
            //DoCastMe( SPELL_CIRCLE_OF_HEALING);
        ...
        {
            DoCastMe( SPELL_REFLECTIVE_SHIELD);
