...
struct MANGOS_DLL_DECL boss_reliquary_of_soulsAI : public ScriptedAI
{
    ...
    void DespawnEssences()
    {
        ...
        if (SufferingGUID)
            pEssence = (Creature*)Unit::GetUnit((*me), SufferingGUID);
        else if (DesireGUID)
            pEssence = (Creature*)Unit::GetUnit((*me), DesireGUID);
        else if (AngerGUID)
            pEssence = (Creature*)Unit::GetUnit((*me), AngerGUID);
    ...
    void SummonSoul()
    {
        ...
        Creature* Soul = me->SummonCreature(CREATURE_ENSLAVED_SOUL, x, y, me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 45000);
    ...
    void MergeThreatList(Creature* target)
    {
       ...
       ThreatList const& m_threatlist = target->getThreatManager().getThreatList();
        ...
        for(; itr != m_threatlist.end(); ++itr)
        {
            ...
            if (pUnit)
            {
                me->AddThreat(pUnit, 1.0f);         // This is so that we make sure the unit is in Reliquary's threat list before we reset the unit's threat.
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Phase && me->getThreatManager().getThreatList().empty())
            EnterEvadeMode();
        ...
        if (Phase == 1)
        {
            ...
            {
                // Release the cube
                me->SetUInt32Value(UNIT_NPC_EMOTESTATE,374);
                AnimationTimer = 8300;
            ...
            if (SummonEssenceTimer < diff)
            {
                ...
                EssenceSuffering = me->SummonCreature(23418, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 1.57, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000);
        ...
        if (Phase == 2)
        {
            ...
            {
                Creature* EssenceSuffering = NULL;
                EssenceSuffering = ((Creature*)Unit::GetUnit((*me), SufferingGUID));
                ...
                if (!EndingPhase)
                {
                    if (EssenceSuffering)
                    {
                        if (EssenceSuffering->GetHealth() < (EssenceSuffering->GetMaxHealth()*0.1))
                        {
                            ...
                            EssenceSuffering->GetMotionMaster()->MoveFollow(me,0.0f,0.0f);
        ...
        if (Phase == 3)
        {
            ...
            if (SoulDeathCount >= SoulCount)
            {
                ...
                if (SummonEssenceTimer < diff)
                {
                    ...
                    EssenceDesire = me->SummonCreature(23419, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 1.57, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000);
        ...
        if (Phase == 4)
        {
            ...
            {
                Creature* EssenceDesire = NULL;
                EssenceDesire = ((Creature*)Unit::GetUnit((*me), DesireGUID));
                ...
                if (!EndingPhase && EssenceDesire)
                {
                    if (EssenceDesire->GetHealth() < (EssenceDesire->GetMaxHealth()*0.1))
                    {
                        ...
                        EssenceDesire->GetMotionMaster()->MoveFollow(me,0.0f,0.0f);
        ...
        if (Phase == 5)
        {
            ...
            if (SoulDeathCount >= SoulCount)
            {
                ...
                if (SummonEssenceTimer < diff)
                {
                    ...
                    EssenceAnger = me->SummonCreature(23420, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 1.57, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 45000);
        ...
        if (Phase == 6)
        {
            ...
            {
                Creature* EssenceAnger = NULL;
                EssenceAnger = ((Creature*)Unit::GetUnit((*me), AngerGUID));

                if (!EssenceAnger)
                    EnterEvadeMode();

                if (me->isAlive() && EssenceAnger)
                {
...
struct MANGOS_DLL_DECL boss_essence_of_sufferingAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit *done_by, uint32 &damage)
    {
        if ((damage >= me->GetHealth()) && (done_by != me))
        {
            ...
            if (StatAuraGUID)
            {
                Unit* pUnit = Unit::GetUnit((*me), StatAuraGUID);
                if (pUnit)
    ...
    void Aggro(Unit* pWho)
    {
        ...
        DoCastMe( ESSENCE_OF_SUFFERING_PASSIVE, true);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (me->GetHealth() <= (me->GetMaxHealth()*0.1))
        {
            if (StatAuraGUID)
            {
                Unit* pUnit = NULL;
                pUnit = Unit::GetUnit((*me), StatAuraGUID);
                if (pUnit)
                    pUnit->RemoveAurasDueToSpell(AURA_OF_SUFFERING_ARMOR);
            }
        }
        ...
        {
            if (me->getVictim())
                me->DeleteThreatList();             // Delete our threatlist if below 10% as we should no longer attack.
            return;
        }
...
struct MANGOS_DLL_DECL boss_essence_of_angerAI : public ScriptedAI
{
    ...
    {
        me->SetInCombatWithZone();
        DoCastVictim( AURA_OF_ANGER, true);
    }
    ...
    void KilledUnit(Unit *victim)
    {
        DoScriptText(urand(0, 1) ? ANGER_SAY_SLAY1 : ANGER_SAY_SLAY2, me);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (AggroYellTimer)
        {
            if (AggroYellTimer <= diff)
            {
                DoScriptText(ANGER_SAY_FREED2, me);
                AggroYellTimer = 0;
            }else AggroYellTimer -= diff;
        }
        ...
        if (CheckTankTimer < diff)
        {
            if (me->getVictim()->GetGUID() != AggroTargetGUID)
            {
                ...
                DoCastMe( SPELL_SELF_SEETHE);
                DoCastVictim( SPELL_ENEMY_SEETHE, true);
                AggroTargetGUID = me->getVictim()->GetGUID();
            }
            CheckTankTimer = 2000;
...
void npc_enslaved_soulAI::JustDied(Unit *killer)
{
    if (ReliquaryGUID)
    {
        Creature* Reliquary = ((Creature*)Unit::GetUnit((*me), ReliquaryGUID));
        if (Reliquary)
