    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!me->HasAura(SPELL_BERSERK, 0))
        {
            if (EnrageTimer < diff)
            {
                ...
                DoScriptText(urand(0, 1) ? SAY_ENRAGE1 : SAY_ENRAGE2, me);
        ...
        if (Phase1)
        {
            if (BewilderingStrikeTimer < diff)
            {
                ...
                float mt_threat = me->getThreatManager().getThreat(me->getVictim());
            ...
            if (EjectTimer < diff)
            {
                ...
                me->getThreatManager().modifyThreatPercent(me->getVictim(), -40);
                EjectTimer = 15000;
        ...
        if (PhaseChangeTimer < diff)
        {
            if (Phase1)
            {
                ...
                if (target && target->isAlive())
                {
                    ...
                    TargetThreat = me->getThreatManager().getThreat(target);
                    ...
                    DoCastMe( SPELL_INSIGNIFIGANCE, true);
