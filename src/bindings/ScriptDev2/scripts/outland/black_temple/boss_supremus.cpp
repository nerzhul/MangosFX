...
struct MANGOS_DLL_DECL molten_flameAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit *who)
    {
        ...
        if (who && (who != me) && (me->IsWithinDistInMap(who, 10)))
            StalkTarget(who);
    }
    ...
    void StalkTarget(Unit* target)
    {
        ...
        DoCastMe( SPELL_DEMON_FIRE, true);
        // DoCastMe( SPELL_MOLTEN_FLAME, true); // This spell damages self, so disabled for now
        TargetLocked = true;
    }

    void UpdateAI(const uint32 diff)
    {
        if (!me->SelectHostileTarget())
            return;
        ...
        if (me->getVictim() && me->isAlive())
        {
            if (CheckTimer < diff)
            {
                ...
                {
                    Unit* Supremus = NULL;
                    Supremus = Unit::GetUnit((*me), SupremusGUID);
                    if (Supremus && (!Supremus->isAlive()))
                        me->DealDamage(me, me->GetHealth(), 0, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
                }
                CheckTimer = 2000;
...
struct MANGOS_DLL_DECL npc_volcanoAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (GeyserTimer < diff)
        {
            DoCastMe( SPELL_VOLCANIC_GEYSER);
            GeyserTimer = 18000;
...
struct MANGOS_DLL_DECL boss_supremusAI : public ScriptedAI
{
    ...
    Creature* SummonCreature(uint32 entry, Unit* target)
    {
        if (target && entry)
        {
            Creature* Summon = me->SummonCreature(entry, CalculateRandomCoord(target->GetPositionX()), CalculateRandomCoord(target->GetPositionY()), target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 20000);
            if (Summon)
            {
    ...
    Unit* CalculateHurtfulStrikeTarget()
    {
       ...
       ThreatList const& m_threatlist = me->getThreatManager().getThreatList();
        ...
        for (i = m_threatlist.begin(); i!= m_threatlist.end();++i)
        {
            ...
            if (pUnit && me->IsWithinDistInMap(pUnit, ATTACK_DISTANCE))
            {
        ...
        if (SummonFlameTimer < diff)
        {
            ...
            if (target)
            {
                ...
                if (MoltenFlame)
                {
                    ...
                    ((molten_flameAI*)MoltenFlame->AI())->SetSupremusGUID(me->GetGUID());
            ...
            if (SummonVolcanoTimer < diff)
            {
                ...
                if (target)
                {
                    ...
                    if (Volcano)
                    {
                        ...
                        ((npc_volcanoAI*)Volcano->AI())->SetSupremusGUID(me->GetGUID());
                    }
