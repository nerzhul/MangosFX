...
bool GOHello_go_trouble_int(Player* pPlayer, GameObject* pGo)
{
    if (pPlayer->GetQuestStatus(11317) == QUEST_STATUS_INCOMPLETE)
	{
		Creature *trouble;
		trouble = pGo->SummonCreature(27959,pPlayer->GetPositionX() + 2.0f,pPlayer->GetPositionY() + 2.0f,pPlayer->GetPositionZ() + 0.5f,pPlayer->GetAngle(pPlayer),TEMPSUMMON_TIMED_DESPAWN,300000);
		trouble->AddThreat(pPlayer);
	}
	else 
		return false;
    return true;
...
{
	if(!pPlayer->GetBattleGround() && (pPlayer->IsWintergraspPortalActive() || !pPlayer->IsWintergraspPortalActive() && pPlayer->IsWintergraspDefender()))
	{
	...
	{
		if(bg->GetTypeID(false) == BATTLEGROUND_SA && !pPlayer->HasAura(54643))
		{
...
bool GoHello_wintergrasp_dalaran_teleporter( Player *pPlayer, GameObject *pGO )
{
	if(pPlayer->IsWintergraspPortalActive())
	{
		if(pPlayer->IsWintergraspDefender())
			pPlayer->TeleportTo(571,5365.641f,2841.748f,410.0f,3.0f);
		else
		{
			if(pPlayer->GetTeam() == ALLIANCE)
				pPlayer->TeleportTo(571,5096.821f,2179.695f,366.0f,1.9f);
			else
				pPlayer->TeleportTo(571,5023.263f,3676.656f,363.0f,4.25f);
		}
	}
    return true;
}
...
void AddSC_go_scripts()
{
    ...
    newscript->Name = "go_touble_int";
    newscript->pGOHello =           &GOHello_go_trouble_int;
	...
	newscript = new Script;
    newscript->Name = "wintergrasp_teleporter";
    newscript->pGOHello = &GoHello_wintergrasp_teleporter;
    newscript->RegisterSelf();

	newscript = new Script;
    newscript->Name = "wintergrasp_dalaran_teleporter";
    newscript->pGOHello = &GoHello_wintergrasp_dalaran_teleporter;
    newscript->RegisterSelf();
}
