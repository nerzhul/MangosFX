...
#define WANT_BEER "Voulez vous goûter cette pinte de bière, c'est de la bonne ?"
enum
{
	QUEST_BEER	=	12645,
	SPELL_BEER	=	51962
...
bool GossipHello_npc_beer_quest(Player* pPlayer, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        pPlayer->PrepareQuestMenu(pCreature->GetGUID());

    if (pPlayer->GetQuestStatus(QUEST_BEER) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, WANT_BEER, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

    pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(pCreature), pCreature->GetGUID());
    return true;
}

bool GossipSelect_npc_beer_quest(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
			pPlayer->CastSpell(pCreature,SPELL_BEER,false);
            break;
    }
...
void AddSC_sholazar_basin()
{
	...
	newscript = new Script;
    newscript->Name = "npc_beer_quest";
    newscript->pGossipHello = &GossipHello_npc_beer_quest;
    newscript->pGossipSelect = &GossipSelect_npc_beer_quest;
    newscript->RegisterSelf();
}
