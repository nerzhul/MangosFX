            ...
            for(std::list<Creature*>::iterator itr = m_lKaddrakGUIDList.begin(); itr != m_lKaddrakGUIDList.end(); ++itr)
            {
                if ((*itr)->isAlive())
                {
                    if (uiPositionCounter == 0)
                    {
                        ...
                        (*itr)->SendMonsterMove(927.265, 333.200, 218.780, 0, (*itr)->GetMonsterMoveFlags(), 1);
                    }
                    else
                    {
                        (*itr)->GetMap()->CreatureRelocation((*itr), 921.745, 328.076, 218.780, (*itr)->GetOrientation());
                        (*itr)->SendMonsterMove(921.745, 328.076, 218.780, 0, (*itr)->GetMonsterMoveFlags(), 1);
                    }
                }
                ++uiPositionCounter;
            }
    ...
    void UpdateEscortAI(const uint32 uiDiff)
    {
        if (m_uiPhase_timer < uiDiff)
        {
            switch(m_uiStep)
            {
                ...
                case 5:
                    if (m_pInstance)
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), m_pInstance->GetData64(DATA_ABEDNEUM))))
                ...
                case 7:
                    if (m_pInstance)
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), m_pInstance->GetData64(DATA_KADDRAK))))
                ...
                case 11:
                    if (m_pInstance)
                        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*me), m_pInstance->GetData64(DATA_MARNAK))))
