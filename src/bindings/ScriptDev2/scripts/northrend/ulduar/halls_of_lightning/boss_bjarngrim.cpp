...
struct MANGOS_DLL_DECL boss_bjarngrimAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        for(uint8 i = 0; i < 2; ++i)
        {
            if (Creature* pStormforgedLieutenant = ((Creature*)Unit::GetUnit((*me), m_auiStormforgedLieutenantGUID[i])))
            {
        ...
        if (m_uiStance != STANCE_DEFENSIVE)
        {
            ...
            DoCastMe( SPELL_DEFENSIVE_STANCE);
            m_uiStance = STANCE_DEFENSIVE;
        }
