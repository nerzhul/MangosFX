...
bool GossipHello_npc_icegiant(Player* pPlayer, Creature* pCreature)
{
	if (pPlayer->GetQuestStatus(12075) == QUEST_STATUS_INCOMPLETE)
	{
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Recolter un echantillon", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
		pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(pCreature), pCreature->GetGUID());
	}
    return true;
}
...
bool GossipSelect_npc_icegiant(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        ItemPosCountVec dest;
        uint8 msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 11522, 1, false);
        if (msg == EQUIP_ERR_OK)
          pPlayer->StoreNewItem(dest, 36765, 1, true);
		else
			return false;
    }
...
void AddSC_dragonblight()
{
    ...
    newscript->Name = "npc_dragonblight_icegiant";
    newscript->pGossipHello = &GossipHello_npc_icegiant;
    newscript->pGossipSelect = &GossipSelect_npc_icegiant;
