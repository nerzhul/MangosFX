...
void guardAI::Aggro(Unit *who)
{
    if (me->GetEntry() == 15184)
    {
        switch(urand(0, 2))
        {
            case 0: DoScriptText(SAY_GUARD_SIL_AGGRO1, me, who); break;
            case 1: DoScriptText(SAY_GUARD_SIL_AGGRO1, me, who); break;
            case 2: DoScriptText(SAY_GUARD_SIL_AGGRO1, me, who); break;
        }
    }
...
void guardAI::UpdateAI(const uint32 diff)
{
    ...
    else
    {
        ...
        if (!me->IsNonMeleeSpellCasted(false))
        {
            ...
            if (me->GetHealth()*100 / me->GetMaxHealth() < 30 && !urand(0, 2))
                info = SelectSpell(me, -1, -1, SELECT_TARGET_ANY_FRIEND, 0, 0, 0, 0, SELECT_EFFECT_HEALING);

            //No healing spell available, See if we can cast a ranged spell (Range must be greater than ATTACK_DISTANCE)
            if (info) Healing = true;
            else info = SelectSpell(me->getVictim(), -1, -1, SELECT_TARGET_ANY_ENEMY, 0, 0, ATTACK_DISTANCE, 0, SELECT_EFFECT_DONTCARE);
